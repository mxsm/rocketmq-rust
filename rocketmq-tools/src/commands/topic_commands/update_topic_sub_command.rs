...
impl CommandExecute for UpdateTopicSubCommand {
    /// Create or update a topic configuration on a single broker or across a cluster.
    ///
    /// Validates input, builds a TopicConfig from the command options (queues, permissions,
    /// ordering, unit flags), starts an MQAdminExt, and applies the configuration to either
    /// the provided broker address or to every master broker in the specified cluster.
    /// On success the operation returns `Ok(())`; on failure it returns a `RocketmqError::SubCommand` with details.
    ///
    /// # Examples
    ///
    /// ```
    /// // Parse a CLI invocation and execute the command (async context required).
    /// // This example demonstrates the typical usage pattern; adjust arguments as needed.
    /// let cmd = UpdateTopicSubCommand::parse_from(&[
    ///     "prog", "-t", "example-topic", "-b", "127.0.0.1:10911", "-r", "8", "-w", "8", "-p", "6",
    /// ]);
    /// // In an async runtime:
    ...
    async fn execute(&self, _rpc_hook: Option<Arc<dyn RPCHook>>) -> RocketMQResult<()> {
        ...
        let operation_result = async {
            ...
            if let Some(perm) = &self.perm {
                topic_config.perm = perm.parse::<u32>().unwrap();
            } else {
                topic_config.perm = 6;
            }
