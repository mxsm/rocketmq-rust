    ...
    async fn execute(&self, _rpc_hook: Option<Arc<dyn RPCHook>>) -> RocketMQResult<()> {
        ...
        let operation_result = async {
            MQAdminExt::start(&mut default_mqadmin_ext).await.map_err(|e| {
                RocketMQError::Internal(format!("WipeWritePermSubCommand: Failed to start MQAdminExt: {}", e))
            })?;
            ...
            for namesrv_addr in default_mqadmin_ext.get_name_server_address_list().await {
                match default_mqadmin_ext
                    ...
                    .await
                    .map_err(|e| {
                        RocketMQError::Internal(format!("WipeWritePermSubCommand: Failed to wipe write perm: {}", e))
                    }) {
                    Ok(add_topic_count) => {
                        println!(
                            "wipe write perm of broker[{broker_name}] in name server[{namesrv_addr}] OK, \
                             {add_topic_count}"
                        );
                    }
                    Err(e) => {
                        println!("wipe write perm of broker[{broker_name}] in name server[{namesrv_addr}] Failed",);
                        println!("{e}")
                    }
                }
            }
