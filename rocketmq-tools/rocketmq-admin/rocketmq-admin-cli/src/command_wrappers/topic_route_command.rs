...
use crate::command_wrappers::CommandExecute;
use crate::command_wrappers::CommonArgs;
        ...
        match format {
            OutputFormat::Json => {
                let json = route_data.serialize_json().map_err(|e| {
                    rocketmq_admin_core::core::ToolsError::internal(format!("Failed to serialize JSON: {e}"))
                })?;
                println!("{json}");
            }
            OutputFormat::Yaml => {
