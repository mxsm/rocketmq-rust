...
impl GetNamesrvConfigCommand {
    ...
    fn validate(&self) -> RocketMQResult<()> {
        if !["table", "json", "yaml"].contains(&self.format.as_str()) {
            return Err(rocketmq_admin_core::core::ToolsError::validation_error(
                "format",
        ...
        match self.format.as_str() {
            "json" => {
                let json = serde_json::to_string_pretty(&config).map_err(|e| {
                    rocketmq_admin_core::core::ToolsError::internal(format!("Failed to serialize to JSON: {e}"))
                })?;
                println!("{json}");
            }
            "yaml" => {
                let yaml = serde_yaml::to_string(&config).map_err(|e| {
                    rocketmq_admin_core::core::ToolsError::internal(format!("Failed to serialize to YAML: {e}"))
                })?;
                println!("{yaml}");
            }
            "table" => {
