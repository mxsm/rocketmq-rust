...
use rocketmq_model::common::message::message_property::MessageProperties;
...
use rocketmq_model::utils::message_utils::build_message_id;
...
pub fn decode(
    ...
    let frame = crate::protocol::body::message_codec::decode_message_frame(byte_buffer).ok()?;
...
pub fn decode_message_id(msg_id: &str) -> Result<MessageId, String> {
    ...
    let bytes = string_to_bytes(msg_id).ok_or_else(|| "Failed to decode message ID: invalid hex string".to_string())?;
