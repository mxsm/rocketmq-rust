...
/// Generation observed after a broker configuration update attempt.
///
...
mod tests {
    ...
    use super::UpdateBrokerConfigResponseHeader;
    use crate::protocol::command_custom_header::CommandCustomHeader;
    use crate::protocol::command_custom_header::FromMap;

    #[test]
    fn config_generation_round_trips_through_wire_map() {
        let header = UpdateBrokerConfigResponseHeader { config_generation: 43 };
        let map = header.to_map().expect("header should encode");
        assert_eq!(map.get("configGeneration").map(CheetahString::as_str), Some("43"));

        let decoded = <UpdateBrokerConfigResponseHeader as FromMap>::from(&map).expect("header should decode");
