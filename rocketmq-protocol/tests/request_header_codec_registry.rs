...
#[derive(Default, RequestHeaderCodec)]
#[header(
...
fn typed_registry_contains_every_migrated_v3_header_exactly_once() {
    // This is an archived migration inventory: `currentCodec = "v3"` records the
    // implementation used when the inventory was created, before its rename.
    ...
    assert_eq!(
        actual, expected,
        "typed registry must cover the archived V3 migration inventory"
    );
}
