...
> 盘点日期：2026-07-22
> 代码基线：Issue #8537 / M11-12bc62 完成后
...
Issue #8537 后 reviewed ArcMut baseline 为 220 identities / 697 occurrences：production 90/253、test
116/404、compatibility 14/40。production 全部分布在 Broker 与 Store。

| owner | 剩余 identity / occurrence | 完成条件 |
|---|---:|---|
| Broker | 8 / 19 | NameServer 全量/增量注册已迁入显式 `BrokerRegistrationRuntime`，producer/consumer state observer 仅持共享 Topic/Client manager；Admin dispatcher 与 controller 周期也不再捕获完整 runtime root；Ack、完整 POP processor/buffer/revive、send/reply、pull、admin config、offset/failover、pre-online 及其余 processor/admin/service leaf 已完成既有收口；仅剩 R01 BrokerRuntime 与 Local/Rocks/Store accessor 组合根 carrier |
...
1. Broker aggregate：收窄 `BrokerRuntimeInner` 组合根（Broker 当前为 8/19）；Admin dispatcher、controller 周期、NameServer 注册与 producer/consumer state observer 均已改持显式 context/runtime/manager，不再保活或解引用完整 runtime root；Ack、POP processor/buffer/revive、send/reply、pull、offset/failover、pre-online 及其他 admin/processor/service leaf 的完整 runtime owner 已拆除。继续删除 BrokerRuntime 与 Local/Rocks/Store accessor 组合根中的最后 8/19。
...
> 本清单给出当前可执行下界。`identity / occurrence` 来自 Issue #8537 后通过
> `python scripts/arc_mut_guard.py` 验证的 reviewed baseline；production 16 项精确合计 Broker 8/19、Store
> 82/234。test 条目会随相邻 production 切片同步下降，因此 R17 只在 production 收口后处理真实余量。

### Production owner（16 项）

- [ ] R01 Broker runtime aggregate：`broker_runtime.rs` 与 Local/Rocks/Store accessor 构造边界（8/19；bc60 已删除 Admin root carrier；bc61 已将 controller 周期迁入显式 `BrokerControllerRuntime`；bc62 已将 NameServer 注册迁入显式 `BrokerRegistrationRuntime`，producer/consumer state observer 改持共享 manager；bc58 归并的单一 Store 兼容 owner 仍由 R01/R09 后续切片删除）。
...
- [ ] R17 Test/bench caller 迁移：在 R01～R16 后重新盘点并清理真实余量；当前上界为 116 identities / 404 occurrences（Store 90/296、Broker 20/29、Client 4/71、runtime-foundation 2/8）。
