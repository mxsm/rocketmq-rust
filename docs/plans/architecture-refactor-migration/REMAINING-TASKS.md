...
> 盘点日期：2026-07-23
> 代码基线：Issue #8583 / M11-12bc83 完成后
...
Issue #8583 后 reviewed ArcMut baseline 为 88 identities / 366 occurrences：production 24/51、test
...
> 本清单给出当前可执行下界。`identity / occurrence` 来自 Issue #8583 后通过
> `python scripts/arc_mut_guard.py` 验证的 reviewed baseline；production 16 项精确合计 Broker 4/8、Store
> 20/43。test 条目会随相邻 production 切片同步下降，因此 R17 只在 production 收口后处理真实余量。
...
- [ ] R15 Store default HA：Default HA service/client/connection 的 service/actor ownership（5/16；bc75 将 DefaultHAClient runtime root 改为标准 Arc，删除从未安装的 stream 与重复 buffer/dispatch/report 状态，实际连接状态保持 task-local；bc82 以共享 generation/allocator/put lock/runtime/checkpoint 的 replica-store handle 删除 DefaultHAClient/ReaderTask 完整 LocalStore carrier；bc83 使 connection registry 直接拥有连接值并只用 runtime handle 跨锁观察状态）。
