...
> 盘点日期：2026-07-23
> 代码基线：Issue #8625 / M11-12bc102 完成后
...
Issue #8625 / M11-12bc102 后 reviewed ArcMut baseline 降至 28 identities / 67 occurrences：production
...
1. Broker aggregate：`BrokerRuntime.inner` 已改为独占 `Box<BrokerRuntimeInner>`（Broker 当前为 2/2），production 与 test 均不能再克隆完整 runtime root；Admin dispatcher、controller 周期、NameServer 注册、producer/consumer state observer、observability 与后台维护任务均持显式 context/runtime/manager/capability，MessageStore accessor 仅返回普通借用；Local/Rocks backend 已由 owned composition 直接拥有，fast-failure 已改持 `PutMessagePreflight`，Admin 与 EscapeBridge 均只持弱 Store provider，Broker runtime 是唯一长期 Store lifecycle owner。剩余私有 legacy owner 需由下一组窄 Store lifecycle/read/write capability bundle 替代。
...
> 本清单给出当前可执行下界。`identity / occurrence` 来自 Issue #8625 / M11-12bc102 后通过
