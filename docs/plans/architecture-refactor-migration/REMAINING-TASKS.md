...
> 盘点日期：2026-07-22
> 代码基线：Issue #8527 / M11-12bc57 完成后
...
| 最小可审查执行单元 | 5 | 26 | 31 项清单中的 R02、R03、R05、R06、R08 已完成 |
...
> 本清单给出当前可执行下界。`identity / occurrence` 来自 Issue #8527 后通过
> `python scripts/arc_mut_guard.py` 验证的 reviewed baseline；production 16 项精确合计 Broker 20/60、Store
> 82/234。test 条目会随相邻 production 切片同步下降，因此 R17 只在 production 收口后处理真实余量。

### Production owner（16 项）

- [ ] R01 Broker runtime aggregate：`broker_runtime.rs` 的 root carrier、controller 与 Local/Rocks 构造边界（8/35；processor 构造及 Ack handle 的 5 occurrences 已由 bc52 删除，无调用方的 start helper occurrence 已由 bc54 删除；bc57 将 admin dispatcher 的 1/1 兼容 carrier 一对一归并到本单元）。
...
- [ ] R17 Test/bench caller 迁移：在 R01～R16 后重新盘点并清理真实余量；当前上界为 120 identities / 408 occurrences（Store 90/296、Broker 24/33、Client 4/71、runtime-foundation 2/8）。
