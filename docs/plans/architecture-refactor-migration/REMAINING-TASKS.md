...
Issue #8410 后 reviewed ArcMut baseline 为 418 identities / 1,051 occurrences：production 250/562、test
154/449、compatibility 14/40。production 全部分布在 Broker 与 Store。

| owner | 剩余 identity / occurrence | 完成条件 |
|---|---:|---|
| Broker | 128 / 254 | transaction bridge、BrokerRuntime aggregate carrier、其余 admin/processor/service 不再传播不安全共享可变 owner |
...
1. M11-12bc4：拆分 `TransactionalMessageBridge` 剩余 Store/offset capability（当前 2/3）。
2. Broker aggregate：收窄 `BrokerRuntimeInner`、processor variant 和启动 carrier（热点文件 `broker_runtime.rs` 为 7/48）。
3. Broker leaf：完成其余 admin/processor/revive/slave/offset leaf owner（除 bridge 外约 126/251）。
4. Store WAL：收口 Local/Rocks MessageStore、CommitLog 与 Flush manager。
5. Store queue：收口 ConsumeQueue、queue store、index/mapped-file carrier。
6. Store timer/HA：收口 Timer、Default/General/AutoSwitch HA service、client 与 connection actor。
7. compatibility：迁移剩余测试/兼容调用方，删除公开 ArcMut facade 和不再需要的 nightly surface。
8. 候选快照 Gate：冻结同一 commit，执行 stable feature matrix、Miri/Loom 可用切片、soak/SLO fault、动态
   Kind/K3d/container、dashboard/runbook/rollback，并完成 `[ARCH]`、`[REV]`、`[TEST]`、`[HUMAN]` 签署。

上述 8 个批次是依据当前代码热点形成的执行计划，不是“还剩 8 个正式工作包”。实际 PR 数可因每个切片的风险与审查
