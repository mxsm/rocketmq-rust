...
> 盘点日期：2026-07-23
> 代码基线：Issue #8579 / M11-12bc81 完成后
...
Issue #8579 后 reviewed ArcMut baseline 为 96 identities / 381 occurrences：production 27/61、test
...
> 本清单给出当前可执行下界。`identity / occurrence` 来自 Issue #8579 后通过
> `python scripts/arc_mut_guard.py` 验证的 reviewed baseline；production 16 项精确合计 Broker 4/8、Store
> 23/53。test 条目会随相邻 production 切片同步下降，因此 R17 只在 production 收口后处理真实余量。
...
- [ ] R17 Test/bench caller 迁移：在 R01～R16 后重新盘点并清理真实余量；当前上界为 55 identities / 280 occurrences（Store 43/193、Broker 6/8、Client 4/71、runtime-foundation 2/8）。
