...
- [ ] 补齐 `sync-flush`、local/Rocks pull、tiered append/pull、connection soak 与 overload 的其余 8/11
  个真实变体采集器，以及四项 correctness runner。
...
- [ ] R19 M10 固定硬件 baseline/candidate、正确性优先性能 Gate 与 HUMAN 签署。
  ...
  - [x] Issue #8688 增加 `local-append` 三个变体的真实采集器；每个变体输出五个隔离进程样本和完整 core
    metric inventory。采集器就绪进度为 3/11，R19 仍保持未完成。
  - [ ] 补齐其余 8 个性能变体和 4 个 correctness runner。
