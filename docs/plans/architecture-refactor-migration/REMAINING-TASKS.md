...
> 盘点日期：2026-07-22
> 代码基线：Issue #8541 / M11-12bc64 完成后
...
Issue #8541 后 reviewed ArcMut baseline 为 220 identities / 690 occurrences：production 90/246、test
116/404、compatibility 14/40。production 全部分布在 Broker 与 Store。

| owner | 剩余 identity / occurrence | 完成条件 |
|---|---:|---|
| Broker | 8 / 12 | `BrokerRuntime.inner` 已改为独占 `Box<BrokerRuntimeInner>`，production 完整 root clone 与 test root clone 均已清零；Admin、ClientManage 与 Lite 测试只持 owned admin runtime 或窄 manager/registry handle。剩余 12 个 occurrence 全部属于 Local/Rocks 构造与单一 Store owner 组合根 carrier，随 R09～R16 删除 |
...
> 本清单给出当前可执行下界。`identity / occurrence` 来自 Issue #8541 后通过
> `python scripts/arc_mut_guard.py` 验证的 reviewed baseline；production 16 项精确合计 Broker 8/12、Store
> 82/234。test 条目会随相邻 production 切片同步下降，因此 R17 只在 production 收口后处理真实余量。

### Production owner（16 项）

- [ ] R01 Broker runtime aggregate：`broker_runtime.rs` 与 Local/Rocks/Store accessor 构造边界（8/12；bc60～bc63 已删除 Admin/controller/registration/state/background root carrier；bc64 将 `BrokerRuntime.inner` 改为独占 `Box`，43 处 test root clone 改为 owned admin runtime 或窄 manager/registry handle；bc58 归并的单一 Store 兼容 owner 仍由 R01/R09～R16 后续切片删除）。
