...
> 盘点日期：2026-07-22
> 代码基线：Issue #8535 / M11-12bc61 完成后
...
Issue #8535 后 reviewed ArcMut baseline 为 220 identities / 703 occurrences：production 90/259、test
116/404、compatibility 14/40。production 全部分布在 Broker 与 Store。

| owner | 剩余 identity / occurrence | 完成条件 |
|---|---:|---|
| Broker | 8 / 25 | Admin dispatcher 已删除 `BrokerAdminRuntimeHandle` 根兼容 owner；controller bootstrap、leader discovery、broker ID、heartbeat、replica 与 membership 周期已迁入显式 `BrokerControllerRuntime`，后台任务不再捕获完整 runtime root；Ack、完整 POP processor/buffer/revive、send/reply、pull、admin config、offset/failover、pre-online 及其余 processor/admin/service leaf 已完成既有收口；仅剩 R01 BrokerRuntime 注册/state getter 与 Local/Rocks 组合根 carrier |
...
1. Broker aggregate：收窄 `BrokerRuntimeInner` 组合根（Broker 当前为 8/25）；Admin dispatcher 与 controller bootstrap/heartbeat/replica/membership 后台任务均已改持显式 context/runtime，不再保活或解引用完整 runtime root；Ack、POP processor/buffer/revive、send/reply、pull、offset/failover、pre-online 及其他 admin/processor/service leaf 的完整 runtime owner 已拆除。继续删除注册、state getter 与 Local/Rocks 组合根中的最后 8/25。
...
> 本清单给出当前可执行下界。`identity / occurrence` 来自 Issue #8535 后通过
> `python scripts/arc_mut_guard.py` 验证的 reviewed baseline；production 16 项精确合计 Broker 8/25、Store
> 82/234。test 条目会随相邻 production 切片同步下降，因此 R17 只在 production 收口后处理真实余量。

### Production owner（16 项）

- [ ] R01 Broker runtime aggregate：`broker_runtime.rs` 的注册、state getter 与 Local/Rocks 构造边界（8/25；bc60 已删除 bc57 归并的 Admin root carrier；bc61 已将 controller bootstrap、leader discovery、broker ID、heartbeat、replica 与 membership 周期迁入显式 `BrokerControllerRuntime`，删除后台任务的完整 root capture；bc58 归并的单一 Store 兼容 owner 仍由 R01/R09 后续切片删除）。
