...
> 盘点日期：2026-07-22
> 代码基线：Issue #8557 / M11-12bc72 完成后
...
Issue #8557 后 reviewed ArcMut baseline 为 127 identities / 444 occurrences：production 56/120、test
...
> 本清单给出当前可执行下界。`identity / occurrence` 来自 Issue #8557 后通过
> `python scripts/arc_mut_guard.py` 验证的 reviewed baseline；production 16 项精确合计 Broker 4/8、Store
> 52/112。test 条目会随相邻 production 切片同步下降，因此 R17 只在 production 收口后处理真实余量。
