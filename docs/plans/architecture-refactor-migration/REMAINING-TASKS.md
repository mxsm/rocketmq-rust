...
> 盘点日期：2026-07-23
> 代码基线：Issue #8581 / M11-12bc82 完成后
...
Issue #8581 后 reviewed ArcMut baseline 为 89 identities / 371 occurrences：production 25/56、test
...
> 本清单给出当前可执行下界。`identity / occurrence` 来自 Issue #8581 后通过
> `python scripts/arc_mut_guard.py` 验证的 reviewed baseline；production 16 项精确合计 Broker 4/8、Store
> 21/48。test 条目会随相邻 production 切片同步下降，因此 R17 只在 production 收口后处理真实余量。
...
- [ ] R14 Store timer：`timer_message_store.rs` 的 LocalStore 回指与可变访问（2/5；bc79 使 CommitLog/LocalStore 消息追加在既有锁下接受共享引用，Timer rolled/due delivery 删除两处 `mut_from_ref`，剩余完整 LocalStore carrier）。
- [ ] R15 Store default HA：Default HA service/client/connection 的 service/actor ownership（6/21；bc75 将 DefaultHAClient runtime root 改为标准 Arc，删除从未安装的 stream 与重复 buffer/dispatch/report 状态，实际连接状态保持 task-local；bc82 以共享 generation/allocator/put lock/runtime/checkpoint 的 replica-store handle 删除 DefaultHAClient/ReaderTask 完整 LocalStore carrier）。
