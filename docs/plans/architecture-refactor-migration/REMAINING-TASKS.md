...
> 盘点日期：2026-07-23
> 代码基线：Issue #8575 / M11-12bc79 完成后
...
Issue #8575 后 reviewed ArcMut baseline 为 98 identities / 383 occurrences：production 29/63、test
...
> 本清单给出当前可执行下界。`identity / occurrence` 来自 Issue #8575 后通过
> `python scripts/arc_mut_guard.py` 验证的 reviewed baseline；production 16 项精确合计 Broker 4/8、Store
> 25/55。test 条目会随相邻 production 切片同步下降，因此 R17 只在 production 收口后处理真实余量。
