...
> 盘点日期：2026-07-23
> 代码基线：Issue #8621 / M11-12bc100 完成后
...
1. Broker aggregate：`BrokerRuntime.inner` 已改为独占 `Box<BrokerRuntimeInner>`（Broker 当前为 4/5），production 与 test 均不能再克隆完整 runtime root；Admin dispatcher、controller 周期、NameServer 注册、producer/consumer state observer、observability 与后台维护任务均持显式 context/runtime/manager/capability，MessageStore accessor 仅返回普通借用；Local/Rocks backend 已由 owned composition 直接拥有，fast-failure 已改持 `PutMessagePreflight`。剩余外层 Store owner 需继续拆分 Admin、EscapeBridge 与 lifecycle capability。
...
> 本清单给出当前可执行下界。`identity / occurrence` 来自 Issue #8621 / M11-12bc100 后通过
