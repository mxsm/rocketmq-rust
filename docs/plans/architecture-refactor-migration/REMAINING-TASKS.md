...
Issue #8421 后 reviewed ArcMut baseline 为 396 identities / 1,019 occurrences：production 232/534、test
...
| Store | 112 / 293 | BrokerStats observer、HA notification config capability 与 ConsumeQueueExt owner 已收窄；其余 MessageStore、CommitLog/Flush、queue、Rocks/Timer 与 HA 生命周期改为独占 owner、标准 Arc/Weak 或显式 actor/锁边界 |
...
5. Store timer/HA：BrokerStats observer 与 HA notification service 已退出完整 Store owner；继续收口 Timer、Default/General/AutoSwitch HA service、client 与 connection actor。
