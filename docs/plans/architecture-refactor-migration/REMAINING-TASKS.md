...
> 盘点日期：2026-07-22
> 代码基线：Issue #8547 / M11-12bc67 完成后
...
Issue #8547 后 reviewed ArcMut baseline 为 137 identities / 493 occurrences：production 62/159、test
61/294、compatibility 14/40。production 全部分布在 Broker 与 Store。
...
> 本清单给出当前可执行下界。`identity / occurrence` 来自 Issue #8547 后通过
> `python scripts/arc_mut_guard.py` 验证的 reviewed baseline；production 16 项精确合计 Broker 4/8、Store
> 58/151。test 条目会随相邻 production 切片同步下降，因此 R17 只在 production 收口后处理真实余量。
...
- [ ] R17 Test/bench caller 迁移：在 R01～R16 后重新盘点并清理真实余量；当前上界为 61 identities / 294 occurrences（Store 49/207、Broker 6/8、Client 4/71、runtime-foundation 2/8）。
