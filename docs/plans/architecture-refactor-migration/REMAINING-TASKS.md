...
> 盘点日期：2026-07-22
> 代码基线：Issue #8533 / M11-12bc60 完成后
...
Issue #8533 后 reviewed ArcMut baseline 为 220 identities / 713 occurrences：production 90/269、test
116/404、compatibility 14/40。production 全部分布在 Broker 与 Store。

| owner | 剩余 identity / occurrence | 完成条件 |
|---|---:|---|
| Broker | 8 / 35 | Admin dispatcher 已删除 `BrokerAdminRuntimeHandle` 根兼容 owner，改持显式 `BrokerAdminRuntime`、原子配置代际、controller/membership 状态与窄 Store/control-plane capability；Ack、完整 POP processor/buffer/revive、send/reply、pull、admin config、offset/failover、pre-online 及其余 processor/admin/service leaf 已完成既有收口；仅剩 R01 BrokerRuntime 启动、注册、后台任务与 Local/Rocks 组合根 carrier |
...
1. Broker aggregate：收窄 `BrokerRuntimeInner` 与启动 carrier（Broker 当前为 8/35）；Admin dispatcher 已改持显式 Admin/control-plane context，不再保活或解引用完整 runtime root；Ack、POP processor/buffer/revive、send/reply、pull、offset/failover、pre-online 及其他 admin/processor/service leaf 的完整 runtime owner 已拆除。继续删除启动、注册、后台任务与 Local/Rocks 组合根中的最后 8/35。
...
> 本清单给出当前可执行下界。`identity / occurrence` 来自 Issue #8533 后通过
> `python scripts/arc_mut_guard.py` 验证的 reviewed baseline；production 16 项精确合计 Broker 8/35、Store
> 82/234。test 条目会随相邻 production 切片同步下降，因此 R17 只在 production 收口后处理真实余量。

### Production owner（16 项）

- [ ] R01 Broker runtime aggregate：`broker_runtime.rs` 的启动、注册、后台任务与 Local/Rocks 构造边界（8/35；bc60 已删除 bc57 归并的 Admin root carrier，Admin dispatcher 改持显式 context/controller/config state；bc58 归并的单一 Store 兼容 owner 仍由 R01/R09 后续切片删除）。
