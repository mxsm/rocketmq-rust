...
> 盘点日期：2026-07-23
> 代码基线：Issue #8587 / M11-12bc85 完成后
...
Issue #8587 后 reviewed ArcMut baseline 为 84 identities / 351 occurrences：production 21/37、test
...
> 本清单给出当前可执行下界。`identity / occurrence` 来自 Issue #8587 后通过
> `python scripts/arc_mut_guard.py` 验证的 reviewed baseline；production 16 项精确合计 Broker 4/8、Store
> 17/29。test 条目会随相邻 production 切片同步下降，因此 R17 只在 production 收口后处理真实余量。
...
- [ ] R15 Store default HA：Default HA service/client/connection 的 service/actor ownership（2/2；bc75 将 DefaultHAClient runtime root 改为标准 Arc，删除从未安装的 stream 与重复 buffer/dispatch/report 状态，实际连接状态保持 task-local；bc82 以共享 generation/allocator/put lock/runtime/checkpoint 的 replica-store handle 删除 DefaultHAClient/ReaderTask 完整 LocalStore carrier；bc83 使 connection registry 直接拥有连接值并只用 runtime handle 跨锁观察状态；bc84 使 DefaultHAService 仅持 replica-store capability；bc85 使 accept/connection/read/write 任务只持窄 `DefaultHAConnectionContext`，并通过标准 `Weak` 访问 registry、group-transfer 与 state-notification owner。剩余 2 个 occurrence 均为 Default HA 组合根初始化 carrier，需与 General/AutoSwitch HA 根边界一起收口）。
