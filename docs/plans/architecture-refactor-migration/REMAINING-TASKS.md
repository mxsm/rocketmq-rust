...
> 盘点日期：2026-07-23
> 代码基线：Issue #8677 / R20 五服务容器动态验收完成后（source commit `13d50e2d33ddfc1142bba63431b339d07704a4f7`）
> 当前范围：Phase 1～3 的 R01～R18、R20、R22～R25；R19 固定目标硬件性能验收、
> R21 Docker/Kind/K3d 动态证据与 Phase 4 AI Native（R26～R31）为独立后续提案，
> 不计入本轮进度、剩余任务数或完成 Gate。
> 统计规则：当前范围 76 个顶层 `PR-Mxx-yy` 工作包与 M11-12 内部实施切片分开统计，禁止重复计数。

## 结论

当前范围的正式工作包已完成 75/76，尚余 1 个：正在实施的 PR-M11-12。
Phase 1、Phase 2 和 10 个目标边界 crate 已完成；Broker production ArcMut 已清零，当前主要工程量已不在 crate
拆分；Store facade/root carrier 与 compatibility 已按 HUMAN 提前移除授权删除，当前仅剩 R25 同一候选快照验收。

| 口径 | 已完成 | 剩余 | 说明 |
|---|---:|---:|---|
| 顶层 PR 工作包 | 75 | 1 | PR-M11-12 |
| 里程碑 | 9 | 2 个未关闭 | M10 待验收、M11 实施中 |
| Phase Gate | 2 | 1 | Phase 3 |
| 目标边界 crate | 10 | 0 | 根 workspace 已为目标 32 package |
| 最小可审查执行单元 | 22 | 1 | 23 项清单中的 R01～R18、R20、R22～R24 已完成，当前仅剩 R25 |

按当前代码热点、兼容窗口和候选快照 Gate 进一步拆分的清单共 **23 个最小可审查单元**：
R01～R18、R20、R22～R24 已完成，当前仅剩 R25。R19 已转为固定目标硬件性能后续项，
R21 已转为独立平台后续项。23 是执行估算，
不是新的正式工作包总数；
相邻单元可以合并到同一 PR，遇到高风险 owner 也可以继续拆分，但 1 个剩余正式工作包的统计口径保持不变。

## 当前完成定义与后续项边界

| 项目 | 类型 | 具体目标 | 前置条件 | 完成结果 |
|---|---|---|---|---|
| R09 | 已完成 | 删除 `GenericMessageStore` 共享可变根 facade 的 2 identities / 5 occurrences | Issue #8702，与 R18 同批删除 | `message_store.rs` 的 ArcMut import/type reference 清零 |
| R18 | 已完成 | 删除 ArcMut/WeakArcMut/SyncUnsafeCellWrapper 及 Store/Timer legacy 公开入口 | 2026-07-24 HUMAN 明确批准提前移除 | 20 identities / 58 occurrences 单调清零，canonical replacement 保留 |
| R19 | FOLLOW-UP | 在同一生产等价固定硬件采集不同 clean baseline/candidate 并执行性能 Gate | 本轮不计入完成 Gate | runner 保持 4/4 correctness、11/11 measurement 就绪 |
| R25 | 待完成 | 冻结重新划定范围的 Phase 3 候选快照并完成四方签署 | R09、R18 完成；R19、R21 不在依赖中 | `[ARCH]`、`[REV]`、`[TEST]`、`[HUMAN]` 对同一 commit 签署，当前范围关闭 |

### R09：Store root facade

- [x] 以 `scripts/arc-mut-baseline.json` 复核
  `rocketmq-store/src/message_store.rs` 仍只有 ArcMut import 1 identity / 1 occurrence，以及
  `GenericMessageStore` variant/constructor type reference 1 identity / 4 occurrences。
- [x] 删除已弃用的 `GenericMessageStore` ArcMut variant、constructor 与 adapter；内部活跃调用方不得恢复，
  Local/Rocks 组合根继续使用 `OwnedMessageStore`。
- [x] `rocketmq-store/src/base/message_store.rs` 保持中立 `MessageStore` capability contract，不新增完整可变
  Store root carrier。
- [x] R09 路径的 production ArcMut 精确降为 0/0；不通过 relocation 或重置 baseline 隐藏余量。

R09 的剩余表面本身属于公开兼容入口，因此实际删除必须与 R18 同一兼容阶段完成；在 R18 前只能继续保持弃用，
不能单独提前删除。

### R18：Public compatibility 删除

- [x] 2026-07-24 HUMAN 明确批准在原弃用窗口前移除 12 个已盘点表面；release plan 记录 override 与 v0.9.0 回滚点。
- [x] 删除 `rocketmq/src/arc_mut.rs` 的 ArcMut、WeakArcMut、SyncUnsafeCellWrapper、`mut_from_ref`
  实现与 facade 自测，并删除 `rocketmq/src/lib.rs` 的三个根 re-export。
- [x] 同一批次删除 R09 的 `GenericMessageStore`、LocalFile 的 `set_message_store_arc`，以及 Timer 的
  full-Store field、`new`、`new_with_config`、setter。
- [x] 保留并验证三个 canonical replacement：`OwnedMessageStore`、
  `LocalFileMessageStore::wire_owned_root_dependencies`、`TimerMessageStore::new_with_message_store_config`。
- [x] 将 ArcMut guard 从“20 identities / 58 occurrences 的弃用合同”迁移为“公开/production/test 零残留”
  合同；禁止通过清空或重置 baseline 伪造完成。
- [x] 记录预期 public API break、迁移说明和回滚版本；执行受影响 crate/consumer 的格式、Clippy、测试与
  public API 验证。

### R19：固定硬件性能 Gate（FOLLOW-UP，不计入本轮）

- [x] 目标硬件 sidecar、完整 manifest、原始 transcript/hash 绑定和 fail-closed 报告校验已由 Issue #8682 完成。
- [x] Issue #8688 增加 `local-append` 的真实 CommitLog 采集器，覆盖 `producers-1`、`producers-8`、
  `producers-32` 共 3/11 个变体；每个指标由 5 个隔离子进程样本组成，采集吞吐、P99、OS 峰值 RSS、
  allocation call 和编码写放大。该项只表示 runner 就绪，不表示已产生批准主机上的 baseline/candidate 证据。
- [x] Issue #8690 增加 `sync-flush/concurrency-64` 真实 group-commit 采集，并以成功 mapped-file flush
  增量计算 `fsync_per_ack`；runner 就绪进度为 4/11。
- [x] Issue #8692 增加 `local-pull/batch-32` 真实 append/reput/hot-read 采集，以 direct CommitLog 匹配控制
  隔离 CQ-unit allocation，并从 mapped-buffer source 统计 body copy；同时修复受限 batch iterator exhausted
  后空转，runner 就绪进度为 5/11。
- [x] Issue #8694 增加 `rocks-pull/batch-32` 真实 RocksDB CQ typed range read 与 Local WAL hot-read 采集，
  以 RocksDB point-read/range-scan counter 计算 native calls；runner 就绪进度为 6/11。
- [x] Issue #8696 增加 `tiered-append/batch-64`、`tiered-pull/cold-32` 与 `tiered-pull/warm-32` 的真实
  POSIX provider 采集器，以 measured-window provider/metadata 累计计数证明写入放大和 cold/warm
  read-ahead 行为；runner 就绪进度为 9/11。
- [x] Issue #8698 增加 `connection-soak/mixed-tls-churn` 与 `overload/bounded-rejection` 的真实
  TCP/TLS 采集器，以成功编码帧写入、pending table、server-owned task、admission rejection 和控制面成功率
  作为 measured-window 证据；真实性能 runner 就绪进度为 11/11。
- [x] Issue #8700 补齐四项 correctness runner：SyncFlush writer 在 durable ack 后由父进程强杀并重启校验，
  派生 CQ/Index/checkpoint 删除后由 CommitLog 重放且逐 offset 无空洞，真实 Transport 过载同时验证
  data rejection、控制面保留与资源收敛，Compaction Recovering 状态禁止 raw CommitLog fallback。
  sidecar 模板现生成仓库内统一 runner 的 4 个 correctness 与 11 个 measurement 具体命令；仅目标环境元数据
  保留待批准主机填写。该项只表示 runner 就绪，不表示 R19 固定硬件 Gate 已执行。
- [ ] 选择不同的 clean baseline/candidate commit，并在同一批准主机、OS/内核、CPU、内存、文件系统、
  Rust toolchain、release profile 和 feature 下分别运行。
- [ ] 先通过 `sync_flush_crash_recovery`、`derived_replay_no_holes`、`bounded_overload`、
  `no_raw_commitlog_fallback` 四项 correctness，再执行 8 profiles / 11 variants。
- [ ] 每项指标至少保留 5 个真实样本；验证 50 个 metric contract 展开的 66 个比较项，
  throughput/p99/RSS/allocation/I/O amplification 相对回归不超过 5%。
- [ ] 相对 MAD 不超过 5%，单样本相对中位数偏离不超过 15%；fixture、脏树、缺失 profile、环境漂移、
  同 commit 对比或未登记例外均 fail closed。
- [ ] `[HUMAN]` 审查真实 report、原始 hash、任何限期例外及回退配置并签署 M10 Gate。

### R25：当前范围最终签署

- [ ] 在 R09、R18 完成后冻结唯一 candidate commit；之后任何修复都产生新快照并使旧签署失效。
- [ ] Evidence index 绑定该 commit，并汇总 R09/R18 以及已完成 R20、R22、R23、R24 的证据和回滚点。
- [ ] 范围守卫证明当前执行集为 R01～R18、R20、R22～R25，R19、R21 与 R26～R31 不计入完成 Gate。
- [ ] `[ARCH]` 核对边界、兼容决策和无未批准例外；`[REV]` 独立审查实现与 API diff；
  `[TEST]` 在同一快照执行最终适用矩阵。
- [ ] `[HUMAN]` 审查证据与残余风险，明确批准关闭 Phase 3 和本轮架构重构。
- [ ] 签署后把当前范围更新为 76/76、执行项 23/23；不得把独立后续项写成已完成。
...
建议按以下最小可审查批次继续推进；它们是 PR-M11-12 的内部切片，不增加 76 个顶层工作包总数：
...
7. 候选快照 Gate：R24 已固化同一 commit 的六小时 soak/SLO、dashboard/alerts/runbook/rollback 与 evidence
   index 工程合同；继续完成当前范围的 `[ARCH]`、`[REV]`、`[TEST]`、`[HUMAN]` 签署。
   Kind/K3d 动态证据已转为独立 R21 后续项。
...
- [x] R09 Store root facade：Issue #8702 删除 `message_store.rs` 的 `GenericMessageStore`、ArcMut import、constructor 与 adapter；`OwnedMessageStore` 保持唯一 canonical composition root。
...
- [x] R18 Public compatibility 删除：Issue #8702 根据 2026-07-24 HUMAN 提前移除授权，删除 `rocketmq/src/arc_mut.rs`、根 re-export 与 Store/Timer legacy carrier；20/58 基线经 guard 单调裁剪为 0/0，三个 canonical replacement 保留。

### M10 / Phase 3 候选快照 Gate（6 项）

- [FOLLOW-UP] R19 M10 固定硬件 baseline/candidate、正确性优先性能 Gate 与 HUMAN 签署；不计入本轮完成 Gate。
  - [x] Issue #8682 增加仓库内目标硬件 sidecar runner，冻结完整 correctness/profile/variant manifest，
    顺序执行并把每个原始 transcript SHA-256 绑定到 guard measurement report；拒绝脏树、placeholder、
    fixture、部分 inventory、命令失败、输出协议漂移和 `target/architecture-refactor/M10` 外的证据路径。
  - [x] Issue #8688 增加 `local-append` 三个变体的真实采集器；每个变体输出五个隔离进程样本和完整 core
    metric inventory。
  - [x] Issue #8690 增加 `sync-flush/concurrency-64` 的真实采集器；采集器就绪进度为 4/11，R19 仍保持未完成。
  - [x] Issue #8692 增加 `local-pull/batch-32` 的真实采集器并修复受限 batch pull 的 iterator exhausted
    空转；采集器就绪进度为 5/11，R19 仍保持未完成。
  - [x] Issue #8694 增加 `rocks-pull/batch-32` 的真实 RocksDB typed-range 采集器；采集器就绪进度为
    6/11，R19 仍保持未完成。
  - [x] Issue #8696 增加 Tiered append/pull 三个真实 POSIX provider 采集器；采集器就绪进度为
    9/11，R19 仍保持未完成。
  - [x] Issue #8698 增加 connection soak 与 bounded overload 两个真实 TCP/TLS 采集器；采集器就绪进度为
    11/11，R19 仍保持未完成。
  - [x] Issue #8700 增加 4 个 correctness runner，并将 sidecar 的全部 correctness/measurement
    command 从占位符替换为仓库内统一 runner 的具体命令；R19 仍保持未完成。
  - [ ] 在批准的同一固定硬件上运行不同 clean baseline/candidate，保留原始数据并完成 `[HUMAN]` Gate。
- [x] R20 M11 五服务镜像动态构建/启动、non-root/read-only、SBOM、签名与漏洞策略验证。Issue #8677
  通过 [Container Foundation run `30011167537`](https://github.com/mxsm/rocketmq-rust/actions/runs/30011167537)
  在 main commit `13d50e2d33ddfc1142bba63431b339d07704a4f7` 上完成 foundation 与 Broker、
  NameServer、Controller、Proxy、MCP 五服务动态验证；artifact `container-foundation`（ID `8565842850`）
  archive digest 为 `sha256:bc8172178a0527a049a79d7c6be0d0811501067acb7336df94f50b5447d32a7f`，
  本地完整性复核确认 5 个服务、15 个 SBOM/Trivy/bundle hash、零 CRITICAL 且未保留 private key。
...
- [x] R24 Soak/SLO fault、dashboard、alert、runbook、rollback 与 evidence index 对齐。Issue #8649 /
  ...
  9 项正向/故意违规测试通过。fixture 明确非动态；R20 容器动态已随后关闭，真实集群执行转入独立 R21，
  R25 只签署重新划定后的当前范围。
- [ ] R25 冻结 Phase 3 候选快照，完成 `[ARCH]`、`[REV]`、`[TEST]`、`[HUMAN]` 签署。
  - 只签署重新划定后的当前范围；R21 已排除，不再是本项依赖。

### 独立性能与平台后续项（不计入上述 23 项）

- [FOLLOW-UP] R19：在批准的固定目标硬件执行已就绪的 4 个 correctness 与 11 个 measurement runner。
- [FOLLOW-UP] R21：在真实 Kind/K3d 集群执行七类 fault/rolling 场景并冻结持久化证据。

### 独立后续提案：Phase 4 / M12（不计入上述 23 项）

- [FOLLOW-UP] R26 / PR-M12-01：Evidence normalization 与 Knowledge Graph。
- [FOLLOW-UP] R27 / PR-M12-02：受控 RAG。
- [FOLLOW-UP] R28 / PR-M12-03：多领域确定性诊断。
- [FOLLOW-UP] R29 / PR-M12-04：冻结 Plan contract 并证明无副作用。
- [FOLLOW-UP] R30 / PR-M12-05：仅在 HUMAN 单独批准后实现独立 Apply；若拒绝实施，以签署的 no-Apply 决策关闭条件分支。
- [FOLLOW-UP] R31 / PR-M12-06：Eval、red-team 与离线 fallback，并关闭独立 Phase 4 Gate。
...
Docker/Kind/K3d/Kubectl/Helm/真实镜像/Secret/Prometheus endpoint，因此该候选当时未虚报动态 PASS。
R20 已由后续 Issue #8677 的五服务容器 run 关闭；R21 已从本轮目标排除并转为独立平台后续项。
R24 与 R20 完成后，当前范围执行清单为完成 20 项、剩余 4 项，正式进度为 75/76。

## 独立后续提案：PR-M12 工作包

以下工作包不属于本轮架构重构范围，不计入当前剩余数或完成 Gate：
...
75/76 更新为 76/76。六个 PR-M12 工作包及其独立 Phase 4 Gate 后续单独立项，不改变本轮统计。
