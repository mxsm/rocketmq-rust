...
> 盘点日期：2026-07-22
> 代码基线：Issue #8521 / M11-12bc54 完成后
...
| 最小可审查执行单元 | 2 | 29 | 31 项清单中的 R02、R08 已完成 |
...
> `python scripts/arc_mut_guard.py` 验证的 reviewed baseline；production 16 项精确合计 Broker 34/86、Store
> 82/234。test 条目会随相邻 production 切片同步下降，因此 R17 只在 production 收口后处理真实余量。

### Production owner（16 项）

- [ ] R01 Broker runtime aggregate：`broker_runtime.rs` 的 root carrier、controller 与 Local/Rocks 构造边界（7/34；processor 构造及 Ack handle 的 5 occurrences 已由 bc52 删除，无调用方的 start helper occurrence 已由 bc54 删除）。
