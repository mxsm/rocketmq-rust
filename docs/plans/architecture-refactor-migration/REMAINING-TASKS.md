...
> 盘点日期：2026-07-23
> 代码基线：Issue #8629 / M11-12bc104 完成后
...
Issue #8629 / M11-12bc104 后 reviewed ArcMut baseline 保持 28 identities / 67 occurrences：production
...
1. Broker aggregate：`BrokerRuntime.inner` 已改为独占 `Box<BrokerRuntimeInner>`（Broker 当前为 2/2），production 与 test 均不能再克隆完整 runtime root；Admin dispatcher、controller 周期、NameServer 注册、producer/consumer state observer、observability 与后台维护任务均持显式 context/runtime/manager/capability，MessageStore accessor 仅返回普通借用；Local/Rocks backend 已由 owned composition 直接拥有，fast-failure 已改持 `PutMessagePreflight`，Admin 与 EscapeBridge 均只持弱 Store provider，Broker runtime 是唯一长期 Store lifecycle owner；Admin/processor 的完整可变 Store lease 已删除，普通单条、批量和 Admin append 已通过私有强类型共享端口执行。剩余私有 legacy owner 需继续提取 role/admin control 和 read/lifecycle capability 后替代。
...
> 本清单给出当前可执行下界。`identity / occurrence` 来自 Issue #8629 / M11-12bc104 后通过
...
- [ ] R17 Test/bench caller 迁移：当前真实余量为 3 identities / 9 occurrences（Store 1/1、Broker 0/0、Client 0/0、runtime-foundation 2/8）。Issue #8615 已删除 CommitLog benchmark 的 4 identities / 16 occurrences，Issue #8625 又随 Broker owner 收口删除最后 1/1 test glob；Store 仅剩 RocksDB semantics 的 Generic Store 兼容 helper 1/1，Client 与 Broker 已清零，R17 仍等待 Store/runtime-foundation 余量及最终兼容夹具删除。
