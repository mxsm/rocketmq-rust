...
> 盘点日期：2026-07-22
> 代码基线：Issue #8539 / M11-12bc63 完成后
...
Issue #8539 后 reviewed ArcMut baseline 为 220 identities / 693 occurrences：production 90/249、test
116/404、compatibility 14/40。production 全部分布在 Broker 与 Store。

| owner | 剩余 identity / occurrence | 完成条件 |
|---|---:|---|
| Broker | 8 / 15 | observability、scheduled maintenance 与 metadata refresh 已改为窄能力捕获，production `self.inner.clone()` 已清零；NameServer 全量/增量注册已迁入显式 `BrokerRegistrationRuntime`，producer/consumer state observer 仅持共享 Topic/Client manager，Admin dispatcher 与 controller 周期也不再捕获完整 runtime root；仅剩 R01 BrokerRuntime、Local/Rocks 构造与 Store owner 组合根 carrier |
...
1. Broker aggregate：收窄 `BrokerRuntimeInner` 组合根（Broker 当前为 8/15）；Admin dispatcher、controller 周期、NameServer 注册、producer/consumer state observer、observability 与后台维护任务均已改持显式 context/runtime/manager/capability，不再保活或解引用完整 runtime root；MessageStore accessor 仅返回普通借用。下一切片迁移 test caller 并将 `BrokerRuntime.inner` 改为独占值，最终 Local/Rocks/Store owner 随 R09～R16 安全边界删除。
...
> 本清单给出当前可执行下界。`identity / occurrence` 来自 Issue #8539 后通过
> `python scripts/arc_mut_guard.py` 验证的 reviewed baseline；production 16 项精确合计 Broker 8/15、Store
> 82/234。test 条目会随相邻 production 切片同步下降，因此 R17 只在 production 收口后处理真实余量。

### Production owner（16 项）

- [ ] R01 Broker runtime aggregate：`broker_runtime.rs` 与 Local/Rocks/Store accessor 构造边界（8/15；bc60 已删除 Admin root carrier；bc61 已将 controller 周期迁入显式 `BrokerControllerRuntime`；bc62 已将 NameServer 注册迁入显式 `BrokerRegistrationRuntime`，producer/consumer state observer 改持共享 manager；bc63 已清除 observability/maintenance/metadata 的 20 处完整 root clone 并收窄 Store accessor；bc58 归并的单一 Store 兼容 owner 仍由 R01/R09 后续切片删除）。
