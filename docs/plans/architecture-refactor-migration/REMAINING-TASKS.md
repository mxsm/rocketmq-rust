...
> 盘点日期：2026-07-22
> 代码基线：Issue #8543 / M11-12bc65 完成后
...
Issue #8543 后 reviewed ArcMut baseline 为 138 identities / 506 occurrences：production 62/168、test
62/298、compatibility 14/40。production 全部分布在 Broker 与 Store。
...
> 本清单给出当前可执行下界。`identity / occurrence` 来自 Issue #8543 后通过
> `python scripts/arc_mut_guard.py` 验证的 reviewed baseline；production 16 项精确合计 Broker 4/8、Store
> 58/160。test 条目会随相邻 production 切片同步下降，因此 R17 只在 production 收口后处理真实余量。
...
- [ ] R10 Store LocalFile root：`local_file_message_store.rs` 的内部 owner 与 `mut_from_ref`（6/38）。
...
- [ ] R17 Test/bench caller 迁移：在 R01～R16 后重新盘点并清理真实余量；当前上界为 62 identities / 298 occurrences（Store 50/211、Broker 6/8、Client 4/71、runtime-foundation 2/8）。
