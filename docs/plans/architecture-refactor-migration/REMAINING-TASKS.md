...
> 盘点日期：2026-07-22
> 代码基线：Issue #8545 / M11-12bc66 完成后
...
Issue #8545 后 reviewed ArcMut baseline 为 138 identities / 501 occurrences：production 62/164、test
62/297、compatibility 14/40。production 全部分布在 Broker 与 Store。
...
1. Broker aggregate：`BrokerRuntime.inner` 已改为独占 `Box<BrokerRuntimeInner>`（Broker 当前为 8/12），production 与 test 均不能再克隆完整 runtime root；Admin dispatcher、controller 周期、NameServer 注册、producer/consumer state observer、observability 与后台维护任务均持显式 context/runtime/manager/capability，MessageStore accessor 仅返回普通借用。剩余 Local/Rocks/Store owner carrier 随 R09～R16 安全边界删除。
...
> 本清单给出当前可执行下界。`identity / occurrence` 来自 Issue #8545 后通过
> `python scripts/arc_mut_guard.py` 验证的 reviewed baseline；production 16 项精确合计 Broker 4/8、Store
> 58/156。test 条目会随相邻 production 切片同步下降，因此 R17 只在 production 收口后处理真实余量。
...
- [ ] R10 Store LocalFile root：`local_file_message_store.rs` 的内部 owner 与 `mut_from_ref`（6/34；bc66 已删除 Reput reader/dispatcher/one-shot 与 scheduled self-check 的完整 Local root 参数/捕获，改持显式运行上下文、live listener capability 与窄 child handle）。
...
- [ ] R13 Store WAL/flush：`commit_log.rs` 与 `default_flush_manager.rs`（5/21）。
...
- [ ] R17 Test/bench caller 迁移：在 R01～R16 后重新盘点并清理真实余量；当前上界为 62 identities / 297 occurrences（Store 50/210、Broker 6/8、Client 4/71、runtime-foundation 2/8）。
...
R10 还有一个 scanner 分类风险：`CleanCommitLogService::run` 的两处运行期 `mut_from_ref` 因同一 struct 含
`#[cfg(test)]` 字段而被当前 baseline 归入 test，但该服务由 production scheduler 调用。后续 WAL/cleanup 切片必须
清理这两个入口并修正 guard 分类，不能据当前 category 提前宣称 Local production `mut_from_ref` 已清零。
