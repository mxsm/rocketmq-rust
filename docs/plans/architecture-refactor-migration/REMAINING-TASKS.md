...
> 盘点日期：2026-07-23
> 代码基线：Issue #8585 / M11-12bc84 完成后
...
Issue #8585 后 reviewed ArcMut baseline 为 88 identities / 364 occurrences：production 24/49、test
50/275、compatibility 14/40。production 全部分布在 Broker 与 Store。
...
> 本清单给出当前可执行下界。`identity / occurrence` 来自 Issue #8585 后通过
> `python scripts/arc_mut_guard.py` 验证的 reviewed baseline；production 16 项精确合计 Broker 4/8、Store
> 20/41。test 条目会随相邻 production 切片同步下降，因此 R17 只在 production 收口后处理真实余量。
...
- [ ] R15 Store default HA：Default HA service/client/connection 的 service/actor ownership（5/14；bc75 将 DefaultHAClient runtime root 改为标准 Arc，删除从未安装的 stream 与重复 buffer/dispatch/report 状态，实际连接状态保持 task-local；bc82 以共享 generation/allocator/put lock/runtime/checkpoint 的 replica-store handle 删除 DefaultHAClient/ReaderTask 完整 LocalStore carrier；bc83 使 connection registry 直接拥有连接值并只用 runtime handle 跨锁观察状态；bc84 使 DefaultHAService 仅持 replica-store capability，传输、confirm、epoch 与 replica progress 继续共享原状态）。
- [ ] R16 Store general/auto-switch HA：General 与 AutoSwitch HA service 的剩余 carrier（5/9）。

### Caller 与 compatibility（2 项）

- [ ] R17 Test/bench caller 迁移：在 R01～R16 后重新盘点并清理真实余量；当前上界为 50 identities / 275 occurrences（Store 38/188、Broker 6/8、Client 4/71、runtime-foundation 2/8）。
