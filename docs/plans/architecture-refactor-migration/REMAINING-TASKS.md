...
> 盘点日期：2026-07-22
> 代码基线：Issue #8549 / M11-12bc68 完成后
...
Issue #8549 后 reviewed ArcMut baseline 为 133 identities / 476 occurrences：production 60/149、test
...
| Store | 56 / 141 | BrokerStats observer、ConsumeQueueExt owner、HA notification/connection registry capability、未共享 HA child、commit-to-flush 窄唤醒能力、HA confirm/epoch 原子发布、HA connection runtime handle、CommitLog shared disk-flush、标准 Arc cleanup capability 与 mapped-file flush capability、auto-switch replication-state/client construction、单一 delegate Store owner、共享引用可变 CommitLog facade 与 Local Reput/scheduled self-check 直接 root owner 已收窄；CommitLog 已独占 MappedFileQueue，DefaultFlushManager production `ArcMut` 已清零，其余 MessageStore、CommitLog/dispatcher、queue、Timer 与 HA service/actor 改为独占 owner、标准 Arc/Weak 或显式 actor/锁边界 |
...
3. Store WAL：commit worker 已只持 `Notify` 唤醒能力，CommitLog disk-flush 已通过共享 receiver enqueue，CommitLog 已独占 MappedFileQueue 且 flush worker 只持窄 capability，transaction 的直接 Store 兼容 owner 已删除；继续收口 Local/Rocks MessageStore 与 CommitLog/dispatcher owner。
...
> 本清单给出当前可执行下界。`identity / occurrence` 来自 Issue #8549 后通过
> `python scripts/arc_mut_guard.py` 验证的 reviewed baseline；production 16 项精确合计 Broker 4/8、Store
> 56/141。test 条目会随相邻 production 切片同步下降，因此 R17 只在 production 收口后处理真实余量。
...
- [ ] R10 Store LocalFile root：`local_file_message_store.rs` 的内部 owner 与 `mut_from_ref`（6/29；bc66 已删除 Reput reader/dispatcher/one-shot 与 scheduled self-check 的完整 Local root 参数/捕获；bc67 将三个 cleanup/offset service 改为标准 Arc 窄句柄，删除 6 个 production CommitLog owner occurrence）。
...
- [ ] R13 Store WAL/flush：`commit_log.rs` 与 dispatcher/flush-manager owner（3/11；bc68 使 CommitLog 独占 MappedFileQueue，四个 flush worker 仅持窄 `MappedFileQueueFlushHandle`，`default_flush_manager.rs` production ArcMut 已清零）。
