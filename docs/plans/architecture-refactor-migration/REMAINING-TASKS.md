...
> 盘点日期：2026-07-23
> 代码基线：Issue #8641 / M11-12bc110 候选完成后
...
Issue #8641 / M11-12bc110 候选后 reviewed ArcMut baseline 保持 22 identities / 60 occurrences：production
...
属性从 8 个降至 4 个，精确余量由 stable-surface guard 冻结。
...
6. compatibility/stable：production Store `WeakArcMut` 与活跃 test/bench caller 已清零；Remoting/Controller
   的 4 个过时 nightly gate 已删除。按 next-major/HUMAN 窗口处理公开 facade 及其自测夹具，并替换 Runtime
   `async_fn_traits`/`unboxed_closures` 与 R18 `sync_unsafe_cell` 两处残留；baseline guard 通过但 target
   模式保持 fail closed。
...
- [ ] R22 同一冻结 commit 的 stable default、完整 feature matrix 与 nightly surface 删除验证。Issue #8641 /
  ...
  仍待 Runtime 2 个 feature、R18 兼容面 2 个 feature 与完整 stable matrix。
