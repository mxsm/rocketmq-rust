...
> 盘点日期：2026-07-22
> 代码基线：Issue #8555 / M11-12bc71 完成后
...
Issue #8555 后 reviewed ArcMut baseline 为 128 identities / 447 occurrences：production 57/123、test
...
| Store | 53 / 115 | BrokerStats observer、ConsumeQueueExt owner、HA notification/connection registry capability、未共享 HA child、commit-to-flush 窄唤醒能力、HA confirm/epoch 原子发布、HA connection runtime handle、CommitLog shared disk-flush、标准 Arc cleanup/mapped-file flush/dispatcher/store-context/read capability、auto-switch replication-state/client construction、单一 delegate Store owner、共享引用可变 CommitLog facade 与 Local Reput/scheduled self-check 直接 root owner 已收窄；CommitLog 已独占 MappedFileQueue、DefaultFlushManager 与 recovery completion，长期 Reput/index/self-check/compaction/tiered reader 只持 `CommitLogReadHandle`，LocalFileMessageStore 独占 dispatcher registry，其余 MessageStore、queue、Timer 与 HA service/actor 改为独占 owner、标准 Arc/Weak 或显式 actor/锁边界 |
...
> 本清单给出当前可执行下界。`identity / occurrence` 来自 Issue #8555 后通过
> `python scripts/arc_mut_guard.py` 验证的 reviewed baseline；production 16 项精确合计 Broker 4/8、Store
> 53/115。test 条目会随相邻 production 切片同步下降，因此 R17 只在 production 收口后处理真实余量。
