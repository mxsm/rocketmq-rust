...
> 盘点日期：2026-07-23
> 代码基线：Issue #8599 / M11-12bc91 完成后
...
Issue #8599 后 reviewed ArcMut baseline 为 62 identities / 234 occurrences：production 13/24、test
35/170、compatibility 14/40。production 全部分布在 Broker 与 Store。
...
> 本清单给出当前可执行下界。`identity / occurrence` 来自 Issue #8599 后通过
...
- [ ] R17 Test/bench caller 迁移：在其余 production owner 收口后重新盘点并清理真实余量；当前上界为 35 identities / 170 occurrences（Store 27/154、Broker 6/8、Client 0/0、runtime-foundation 2/8）。Issue #8593 已将 LitePull 测试的 68 个配置 wrapper 改为标准 Arc，Issue #8595 以标准 `Arc<MessageExt>` 替代最后的 Client hot-path ArcMut benchmark，Issue #8597 将 RocksDB semantics fixture 默认改为独占 Store root 并只在 Generic Store 兼容 helper 保留 1/1，Issue #8599 将共享 HA fixture 改为独占 LocalFile root；Client test/bench caller 已清零，R17 仍等待 Store/Broker/runtime-foundation 余量。
