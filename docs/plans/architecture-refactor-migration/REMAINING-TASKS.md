...
> 盘点日期：2026-07-22
> 代码基线：Issue #8567 / M11-12bc75 完成后
...
Issue #8567 后 reviewed ArcMut baseline 为 126 identities / 437 occurrences：production 55/113、test
...
> 本清单给出当前可执行下界。`identity / occurrence` 来自 Issue #8567 后通过
> `python scripts/arc_mut_guard.py` 验证的 reviewed baseline；production 16 项精确合计 Broker 4/8、Store
> 51/105。test 条目会随相邻 production 切片同步下降，因此 R17 只在 production 收口后处理真实余量。
