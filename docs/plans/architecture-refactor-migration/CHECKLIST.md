...
PR-M11-12 的内部子切片不重复计入 82 个顶层工作包。Issue #8421 的 M11-12bc8 子切片完成后，当前 ArcMut reviewed
baseline 为 396 identities / 1,019 occurrences，其中 production 为 232/534、test 为 150/445、compatibility
...
| Store | 112 / 293 | TopicConfig 只读代际 carrier、BrokerStats observer、HA notification config capability 与 ConsumeQueueExt 显式锁 owner 已完成；继续完成 message store、CommitLog/Flush、其余 queue、Rocks/Timer 与其他 HA owner/actor 安全化 |
...
- [ ] PR-M11-12：完成 ArcMut、stable 与 SLO Phase 3 收口
  ...
  - [ ] M11-12bc9 及后续：Broker aggregate/leaf、Store WAL/其余 queue/timer/HA、compatibility 删除、stable/Miri/Loom/soak/SLO 与同一候选快照 Gate 仍待完成
