...
M10/Phase 3 动态验收与签署、6 个 M12；R01～R08、R10～R17 已完成，当前剩余 15 个。该数字不替代 82 个顶层工作包口径；逐项 checklist
及每组精确 baseline 见 [`REMAINING-TASKS.md`](REMAINING-TASKS.md#执行层最小审查清单31-项)。

PR-M11-12 的内部子切片不重复计入 82 个顶层工作包。Issue #8643 的 M11-12bc111 候选完成后，当前 ArcMut reviewed
baseline 保持 22 identities / 60 occurrences，其中 production 为 6/12、test 为 2/8、compatibility
...
M11-12bc110～bc111 已将 nightly feature 属性从 8 个降至 2 个，并以
`scripts/stable_surface_guard.py` 冻结精确 baseline；Runtime 已通过 stable all-target 检查，剩余两个
`sync_unsafe_cell` 均属于 R18 公开兼容面，未按 next-major/HUMAN 窗口清零前 stable target 模式保持 fail closed。
...
- [ ] PR-M11-12：完成 ArcMut、stable 与 SLO Phase 3 收口
  ...
  - [ ] M11-12bc112 及后续：完成 R09/R18 compatibility、Miri/Loom、soak/SLO 与同一候选快照 Gate
