...
PR-M11-12 的内部子切片不重复计入 82 个顶层工作包。Issue #8541 的 M11-12bc64 子切片完成后，当前 ArcMut reviewed
baseline 为 220 identities / 690 occurrences，其中 production 为 90/246、test 为 116/404、compatibility
...
| Broker | 8 / 12 | `BrokerRuntime.inner` 已改为独占 `Box<BrokerRuntimeInner>`，production/test 完整 root clone 均已清零；Admin、ClientManage 与 Lite 测试只持 owned admin runtime 或窄 manager/registry handle。剩余 Local/Rocks/Store owner carrier 随 Store 重构删除 |
...
- [ ] PR-M11-12：完成 ArcMut、stable 与 SLO Phase 3 收口
  ...
  - [ ] M11-12bc65 及后续：完成 Local-Rocks/Store owner、Store WAL/其余 queue/timer/HA、compatibility 删除、stable/Miri/Loom/soak/SLO 与同一候选快照 Gate
