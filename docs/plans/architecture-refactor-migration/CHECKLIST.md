...
PR-M11-12 的内部子切片不重复计入 82 个顶层工作包。Issue #8533 的 M11-12bc60 子切片完成后，当前 ArcMut reviewed
baseline 为 220 identities / 713 occurrences，其中 production 为 90/269、test 为 116/404、compatibility
...
| Broker | 8 / 35 | Admin dispatcher 已删除完整 runtime root handle，改持显式 Admin/control-plane context、原子 broker/store 配置代际、共享 controller/membership/role 状态和窄 Store capability；Ack、完整 POP processor/buffer/revive、send/reply、pull、offset/failover、pre-online 及其余 processor/admin/service leaf 已完成既有收口；继续删除启动、注册、后台任务与 Local/Rocks 组合根 carrier |
...
- [ ] PR-M11-12：完成 ArcMut、stable 与 SLO Phase 3 收口
  ...
  - [ ] M11-12bc63 及后续：BrokerRuntime/Local-Rocks/Store accessor aggregate、Store WAL/其余 queue/timer/HA、compatibility 删除、stable/Miri/Loom/soak/SLO 与同一候选快照 Gate 仍待完成
