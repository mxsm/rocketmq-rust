...
PR-M11-12 的内部子切片不重复计入 82 个顶层工作包。Issue #8629 的 M11-12bc104 子切片完成后，当前 ArcMut reviewed
...
| Broker | 2 / 2 | `BrokerRuntime.inner` 已改为独占 `Box<BrokerRuntimeInner>`，production/test 完整 runtime root clone 均已清零；Broker runtime 是唯一 Store 生命周期强 owner，EscapeBridge/Admin 只保存弱 provider；Admin 通用可变租约已删除，普通单条/批量/Admin append 已经私有强类型共享端口，不再克隆完整可变 Store carrier。剩余私有 legacy owner constructor/field 随 role/admin control 与 read/lifecycle capability 提取删除 |
...
- [ ] PR-M11-12：完成 ArcMut、stable 与 SLO Phase 3 收口
  ...
  - [ ] M11-12bc105 及后续：继续提取 role/admin control 与 read/lifecycle capability，移除 R01 私有 legacy Store owner，再处理其余 Store/runtime-foundation test caller、公开 compatibility 删除、stable/Miri/Loom/soak/SLO 与同一候选快照 Gate
