...
PR-M11-12 的内部子切片不重复计入 82 个顶层工作包。Issue #8555 的 M11-12bc71 子切片完成后，当前 ArcMut reviewed
baseline 为 128 identities / 447 occurrences，其中 production 为 57/123、test 为 57/284、compatibility
...
| Store | 53 / 115 | TopicConfig 只读代际 carrier、BrokerStats observer、ConsumeQueueExt 显式锁 owner、HA notification/connection registry 窄能力、未共享 HA child 直接 ownership、commit-to-flush 窄唤醒能力、HA confirm/epoch 原子发布、HA connection runtime handle、CommitLog shared disk-flush、标准 Arc cleanup/mapped-file flush/dispatcher/store-context/read capability、auto-switch replication-state/client construction、单一 delegate Store owner、共享引用可变 CommitLog facade 与 Local Reput/scheduled self-check 直接 root owner 已退出；CommitLog 已独占 MappedFileQueue、DefaultFlushManager 与 recovery completion，长期 Reput/index/self-check/compaction/tiered reader 只持 `CommitLogReadHandle`，LocalFileMessageStore 独占 dispatcher registry，继续完成 MessageStore、其余 queue、Timer 与 HA service/actor 安全化 |
...
- [ ] PR-M11-12：完成 ArcMut、stable 与 SLO Phase 3 收口
  ...
  - [ ] M11-12bc72 及后续：完成 Local-Rocks/Store owner、其余 queue/timer/HA、compatibility 删除、stable/Miri/Loom/soak/SLO 与同一候选快照 Gate
