...
PR-M11-12 的内部子切片不重复计入 82 个顶层工作包。Issue #8539 的 M11-12bc63 子切片完成后，当前 ArcMut reviewed
baseline 为 220 identities / 693 occurrences，其中 production 为 90/249、test 为 116/404、compatibility
...
| Broker | 8 / 15 | Admin、controller、registration/state observer、observability、scheduled maintenance 与 metadata refresh 均已退出完整 runtime root；MessageStore accessor 仅暴露普通借用，production `self.inner.clone()` 已清零；继续迁移 test caller、将 `BrokerRuntime.inner` 改为独占值并随 Store 重构删除 Local/Rocks/Store owner carrier |
...
- [ ] PR-M11-12：完成 ArcMut、stable 与 SLO Phase 3 收口
  ...
  - [ ] M11-12bc64 及后续：迁移 test caller 并将 `BrokerRuntime.inner` 改为独占值；随后完成 Local-Rocks/Store owner、Store WAL/其余 queue/timer/HA、compatibility 删除、stable/Miri/Loom/soak/SLO 与同一候选快照 Gate
