...
M10/Phase 3 动态验收与签署、6 个 M12；R02、R08 已完成，当前剩余 29 个。该数字不替代 82 个顶层工作包口径；逐项 checklist
及每组精确 baseline 见 [`REMAINING-TASKS.md`](REMAINING-TASKS.md#执行层最小审查清单31-项)。

PR-M11-12 的内部子切片不重复计入 82 个顶层工作包。Issue #8521 的 M11-12bc54 子切片完成后，当前 ArcMut reviewed
baseline 为 255 identities / 775 occurrences，其中 production 为 116/320、test 为 125/415、compatibility
...
- [ ] PR-M11-12：完成 ArcMut、stable 与 SLO Phase 3 收口
  ...
  - [ ] M11-12bc55 及后续：Broker aggregate/leaf、Store WAL/其余 queue/timer/HA、compatibility 删除、stable/Miri/Loom/soak/SLO 与同一候选快照 Gate 仍待完成
