...
在 M11-09 合并快照中，资产故意不包含 `readinessProbe`、`livenessProbe`、`startupProbe`、`preStop`、
`lifecycle` 或 `terminationGracePeriodSeconds`，当时的 guard 将其作为禁止项，防止用 TCP-open 假 readiness
提前替代 M11-10 的统一 drain 语义。当前 `main` 已由 M11-10 在同一 canonical chart/base 上加入真实 HTTP
probe/preStop/统一 deadline；该后续状态以 `11-probe-prestop-drain-evidence.md` 为准，不改写本文件的历史结论。
