...
该目标尚未完成。本文件记录 M11-12a～g 子切片的真实下降，不把子切片计为第 76 个工作包，也不刷新
...
1. Controller/NameServer/remoting client owner 与 NameServer V1 table owner；NameServer runtime/KV/V2 route/batch/housekeeping/request-processor shell 已完成。
2. Remoting client/server/protocol/RPC 中剩余 21 条 owner；Channel/ConnectionHandlerContext 已完成并定向清零。
