...
LocalFile lifecycle probe 的 shared-owner 构造随 Issue #8577 完成以下收口：

- 新增 crate-private `local_file_shared_owner` owner 模块，集中完成兼容 `LocalFileMessageStore` shared owner 构造
  与一次性 `set_message_store_arc` wiring；opaque return 只暴露 `DerefMut + Clone` 能力，不向调用方传播具体指针类型。
- `bench_support::run_store_local_file_scheduled_lifecycle_probe` 删除 `ArcMut` import、直接 constructor 与重复
  self-wiring，只调用 owner factory；既有 init/start、四项 scheduled snapshot 与 shutdown 验证路径保持不变。
- 新增 source contract 固定 factory 中唯一兼容 constructor、一次性 wiring、probe 无 `ArcMut`/直接 wiring；复用既有
  lifecycle 行为测试证明 scheduled task 数量、运行统计与 clean shutdown 保持。
- reviewed baseline 从 98/383 降至 97/382：production 从 29/63 降至 28/62，test 保持 55/280，
  compatibility 保持 14/40；Broker production 保持 4/8，Store production 从 25/55 降至 24/54。
  旧 probe constructor identity 一对一迁入专用 owner 模块，旧 import identity 真实删除；临时 ADR-013 approval
  仅位于忽略的 `target/`，不提交仓库。
- R09 从 4/7 降至 2/5，R10 因接收唯一兼容 constructor 从 3/4 调整为 4/5；净减少 1 个 production
  identity/1 occurrence。31 项执行清单保持完成 9 项、剩余 22 项，正式进度仍为 75/82。

## M11-12bc80 验证

| 命令 | 结果 |
|---|---|
| affected behavior / source | Store `--lib` 精确过滤后 source contract 与既有 LocalFile scheduled lifecycle 回归 2/2 通过；首轮 source contract 仍断言移动前的 `Target = Self` 而失败，修正为 owner module 的 `Target = LocalFileMessageStore` 并删除移动后未使用 import 后重跑通过，先前非零结果未计作通过 |
| reviewed baseline / fixtures | 正式 baseline 与 `--promote-baseline` candidate 均为 97/382；直接 guard、带临时 identity-relocation approval 的 candidate compare、27/27 fixtures 与 78/78 guard tests 通过；净删除 1 个 production identity/1 occurrence，无提交态 approval |
| root final gates | `cargo fmt --all -- --check`、workspace all-target/all-feature strict Clippy 与 `git diff --check` 通过；Windows linker stdout 与既有 future-incompatibility note 不受 `-D warnings` 管辖 |
| reduced validation scope | 未重复 package strict Clippy（最终 workspace Clippy 已覆盖）、额外 `cargo check`（定向 lib tests 已完成编译）、Store/Broker 全量 lib、RocksDB 六项专项、runtime audit、dependency/release/performance/telemetry 矩阵、Rustdoc 或 AGENTS routing；未修改 Store 数据路径、RocksDB feature/backend、TaskGroup/runtime/lifecycle、公开 API、构建或路由边界，变更由 2 项定向回归、所有权 guard 与最终 workspace strict Clippy 覆盖 |

## M11-12bc81 实现

LocalFile lifecycle probe 的临时 shared-owner factory 随 Issue #8579 完成以下收口：

- `bench_support::run_store_local_file_scheduled_lifecycle_probe` 直接独占 `LocalFileMessageStore`，不再通过
  opaque compatibility owner 间接构造完整 Store root；原 `local_file_shared_owner` 模块和唯一 production
  `ArcMut` constructor identity 删除。
- 新增 crate-private `wire_owned_root_dependencies`。该入口仅在 MessageStore 与 Broker 的 duplication mode
  同时开启且 Timer 关闭时安装 ConsumeQueue context；任一条件不满足均返回 `InvalidState`，不会把缺失的 Timer/HA
  shared-root 依赖伪装成已完成 wiring。
- probe 显式选择 duplication/no-timer 配置，既有 init/start、四项 scheduled task snapshot、运行/重叠/失败统计
  与 clean shutdown 路径保持不变；source contract 固定 probe 无 `ArcMut`/`set_message_store_arc` 和 factory import。
- reviewed baseline 从 97/382 降至 96/381：production 从 28/62 降至 27/61，test 保持 55/280，
  compatibility 保持 14/40；Broker production 保持 4/8，Store production 从 24/54 降至 23/53。
  相对 bc80 净删除 1 个 production identity/1 occurrence，无 relocation、新增 identity 或临时 approval。
- R10 从 4/5 降至 3/4；31 项执行清单保持完成 9 项、剩余 22 项，正式进度仍为 75/82。

## M11-12bc81 验证

| 命令 | 结果 |
|---|---|
| affected behavior / source | `cargo test -p rocketmq-store --lib owned_root_wiring` 在最终源码快照 5/5 通过，覆盖 duplication 配置不一致、Timer 开启时 fail closed、允许的 owned wiring、无 shared owner source contract 和真实 scheduled lifecycle clean shutdown；首次运行行为用例与 lifecycle 4/4 通过，仅 source contract 因 `rustfmt` 换行导致精确字符串断言失败，放宽为方法名断言后重跑通过，先前非零结果未计作通过 |
| reviewed baseline / fixtures | 正式 baseline 与 `--prune-resolved` candidate 的 semantic set 均为 96/381；直接 guard、candidate compare、27/27 fixtures 与 78/78 guard tests 通过；净删除 1 个 production identity/1 occurrence，无 relocation、新增 identity 或临时 approval |
| root final gates | `cargo fmt --all -- --check` 与 workspace all-target/all-feature strict Clippy 通过；Windows linker stdout 与既有 future-incompatibility note 不受 `-D warnings` 管辖；`git diff --check` 在最终文档更新后执行 |
| reduced validation scope | 未重复 package strict Clippy（最终 workspace Clippy 已覆盖）、额外 `cargo check`（定向 lib tests 已完成编译）、Store/Broker 全量 lib、RocksDB 六项专项、runtime audit、dependency/release/performance/telemetry 矩阵、Rustdoc 或 AGENTS routing；未修改 Store 数据读写语义、RocksDB feature/backend、TaskGroup/runtime owner、公开 API、构建或路由边界，变更由 5 项定向回归、所有权 guard 与最终 workspace strict Clippy 覆盖；磁盘空间充足，保留 Cargo cache 且未执行 `cargo clean` |

## M11-12bc82 实现

DefaultHAClient 的完整 LocalStore carrier 随 Issue #8581 完成以下收口：

- `MappedFileQueueAppendHandle` 只克隆既有 `ArcSwap` generation、store path/file size、同一
  `AllocateMappedFileService` 与 `MappedFileQueueRuntimeState`；原 facade 与新 handle 复用同一 create/
  preallocation helper，segment 发布继续由同一 maintenance lock 串行化，不暴露 recovery/truncate/destroy。
- `CommitLogReplicaHandle` 组合 read/append handle、同一个 `put_message_lock`、`CommitLogRuntimeState` 与
  `StoreCheckpoint`；raw append 继续在 put lock 内完成 segment 获取/创建和 mmap append，confirm offset
  继续同时发布 runtime state 与 checkpoint。旧 `CommitLog::append_data` 委托该实现，避免语义分叉。
- `HAReplicaStoreHandle` 仅补充 `Arc<MessageStoreConfig>` 与 shutdown 原子状态；shutdown 后 replica append
  fail closed。DefaultHAClient 的 `Inner` 与 `ReaderTask` 只持该 handle，不再获得 ConsumeQueue、index、
  dispatcher、stats 或完整 Store lifecycle 能力。
- Default/AutoSwitch client 测试 helper 改为 duplication/no-timer owned Store，不把删除的 production
  `ArcMut` import 搬入测试；M06 contract 固定三层 capability 组合和 DefaultHAService 注入边界。
- reviewed baseline 从 96/381 降至 89/371：production 从 27/61 降至 25/56，test 从 55/280 降至
  50/275，compatibility 保持 14/40；Broker production 保持 4/8，Store production 从 23/53 降至 21/48。
  相对 bc81 净删除 7 identities/10 occurrences（production 2/5、test 5/5），无 relocation、新增 identity
  或临时 approval。
- R15 从 8/26 降至 6/21；31 项执行清单保持完成 9 项、剩余 22 项，正式进度仍为 75/82。

## M11-12bc82 验证

| 命令 | 结果 |
|---|---|
| affected behavior / source | `replica_handle` 2/2、DefaultHAClient 4/4、AutoSwitch client 2/2、MappedFileQueue module 15/15，共 23 项定向 Rust 回归通过，覆盖共享 put lock、并发单 generation、confirm runtime/checkpoint 双发布、shutdown、confirm clamp、offset mismatch、client 构造以及原 facade create/preallocation/flush/read 行为；M06 contract 2/2 通过，首轮 1/2 因字段计数同时命中构造参数而失败，收窄为字段行计数后重跑通过，先前非零结果未计作通过 |
| reviewed baseline / fixtures | 正式 baseline 与 `--prune-resolved` candidate 的 semantic set 均为 89/371；直接 guard、candidate compare、27/27 fixtures 与 78/78 guard tests 通过；净删除 7 identities/10 occurrences，无 relocation、新增 identity 或临时 approval |
| runtime / root final gates | enforcing runtime audit、`cargo fmt --all -- --check` 与 workspace all-target/all-feature strict Clippy 通过；Windows linker stdout 与既有 future-incompatibility note 不受 `-D warnings` 管辖；`git diff --check` 在最终文档更新后执行 |
| reduced validation scope | 未重复 package strict Clippy（最终 workspace Clippy 已覆盖）、额外 `cargo check`（定向 lib tests 已完成编译）、Store/Broker 全量 lib、RocksDB 六项专项、dependency/release/performance/telemetry 矩阵、Rustdoc 或 AGENTS routing；未修改 RocksDB feature/backend、公开 API、构建或路由边界，raw append 与生命周期风险由 23 项定向回归、M06、enforcing runtime audit、所有权 guard 和最终 workspace strict Clippy 覆盖；磁盘空间充足，保留 Cargo cache 且未执行 `cargo clean` |

## M11-12bc83 实现

Default HA connection registry 的共享完整连接根随 Issue #8583 完成以下收口：

- `DefaultHAService.connections` 直接拥有 `GeneralHAConnection`；accept path 在移动 owner 入表前完成
  `start`，`build_connection` 不再构造 `ArcMut<GeneralHAConnection>`。
- add 只转移唯一 owner；显式 remove 与 shutdown 都先从注册表取出/drain，再在锁外执行状态通知或
  connection shutdown，保留原有避免锁跨网络/任务等待的顺序。
- Default/AutoSwitch/General connection 暴露 crate-private `HAConnectionRuntimeHandle` 投影；
  `connection_state` 只克隆该投影后再 await，既不克隆完整 connection root，也不持连接表 guard 跨 await。
- ACK、transfer offset、flow monitor、auto-switch slave id 和 task-local read/write service 行为保持不变；
  source contract 禁止恢复 connection `ArcMut` registry/constructor。
- reviewed baseline 从 89/371 降至 88/366：production 从 25/56 降至 24/51，test 保持 50/275，
  compatibility 保持 14/40；Broker production 保持 4/8，Store production 从 21/48 降至 20/43。
  净删除 1 个 production identity/5 occurrences，无 relocation、新增 identity 或临时 approval。
- R15 从 6/21 降至 5/16；31 项执行清单保持完成 9 项、剩余 22 项，正式进度仍为 75/82。

## M11-12bc83 验证

| 命令 | 结果 |
|---|---|
| affected behavior / source | Default HA service 定向模块 12/12 通过，覆盖 owned registry source contract、Default/AutoSwitch build、ACK/caught-up/remove callback、drain-before-shutdown、connection state 与 in-sync replica 观察；定向测试已承担编译检查，不再重复额外 `cargo check` |
| reviewed baseline / fixtures | 正式 baseline 与 reviewed candidate 的 semantic set 均为 88/366；直接 guard、candidate compare、27/27 fixtures 与 78/78 guard tests 通过；净删除 1 identity/5 occurrences，无 relocation、新增 identity 或临时 approval |
| runtime / root final gates | enforcing runtime audit、M06 HA/Local contract 2/2、`cargo fmt --all -- --check`、workspace all-target/all-feature strict Clippy 与 `git diff --check` 通过；首轮将四项组合在 60 秒工具窗口内，Clippy 输出完成后整条命令仍因窗口超时返回 124，未计作通过，随后拆分命令均取得 0 退出码；Windows linker stdout 与既有 future-incompatibility note 不受 `-D warnings` 管辖 |
| reduced validation scope | 未重复 package strict Clippy、额外 `cargo check`、Store/Broker 全量 lib、RocksDB 六项专项、dependency/release/performance/telemetry 矩阵、Rustdoc 或 AGENTS routing；连接生命周期由 12 项定向回归、M06 contract、enforcing runtime audit、所有权 guard 与最终 workspace strict Clippy 覆盖；D 盘剩余 59.75 GiB，保留 Cargo cache且未执行 `cargo clean` |

## M11-12bc84 实现

Default HA service 的完整 LocalFile Store carrier 随 Issue #8585 完成以下收窄：

- `DefaultHAService` 只持 `HAReplicaStoreHandle`，不再持有或暴露
  `ArcMut<LocalFileMessageStore>`；Default/AutoSwitch service 与 connection 仅获得复制所需能力。
- `CommitLogReadHandle` 与 `CommitLogReplicaHandle` 复用原 bulk read、segment selection 和 direct confirm
  算法；原 CommitLog facade 委托同一实现，避免 HA transfer 与普通 facade 语义分叉。
- replica handle 共享原 `master_flushed_offset`、`alive_replica_num_in_group`、`state_machine_version` 与
  `controller_epoch_start_offset` 原子状态，confirm publication 继续同时更新 CommitLog runtime state/checkpoint。
- Default/AutoSwitch 的角色切换、sync-state-set、confirm、epoch 与 in-sync replica 计算改从窄 capability
  读取；shutdown 状态和 connection transfer 仍观察同一 Store 状态，不获得 queue、index、dispatcher 或完整 lifecycle。
- reviewed baseline 从 88/366 降至 88/364：production 从 24/51 降至 24/49，test 保持 50/275，
  compatibility 保持 14/40；Broker production 保持 4/8，Store production 从 20/43 降至 20/41。
  净删除 2 个 production occurrences；22 个保留 test occurrence 经临时 ADR-013 同 item 一对一 relocation
  审核，approval 仅位于忽略的 `target/`，无新增 identity 或提交态 approval。
- R15 从 5/16 降至 5/14；31 项执行清单保持完成 9 项、剩余 22 项，正式进度仍为 75/82。

## M11-12bc84 验证

| 命令 | 结果 |
|---|---|
| affected behavior / source | `replica_handle` 3/3、Default HA service 12/12、AutoSwitch HA service 12/12、MappedFileQueue bulk read 1/1、CommitLog segment boundary 1/1，共 29 项定向 Rust 回归通过；M06 HA/Local contract 2/2 通过。首轮编译暴露 AutoSwitch 9 条间接完整 Store 访问与 2 个失效 import，完成 capability 迁移后重跑通过，先前非零结果未计作通过 |
| reviewed baseline / fixtures | 正式 baseline 与 reviewed promoted candidate 的 semantic set 均为 88/364；直接 guard、candidate compare、27/27 fixtures 与 78/78 guard tests 通过；净删除 2 个 production occurrences，22 个同 item test relocation 由临时 ADR-013 approval 审核，无新增 identity 或提交态 approval |
| runtime / final gates | enforcing runtime audit、`cargo fmt --all -- --check` 与 workspace all-target/all-feature strict Clippy 通过；Windows linker stdout 与既有 future-incompatibility note 不受 `-D warnings` 管辖；`git diff --check` 在最终文档快照执行 |
| reduced validation scope | 不重复 package strict Clippy、额外 `cargo check`、Store/Broker 全量 lib、RocksDB 六项专项、dependency/release/performance/telemetry 矩阵、Rustdoc 或 AGENTS routing；定向 lib tests 已承担受影响 Rust 编译与传输/confirm/epoch/进度行为，M06 固定 capability source contract，runtime audit 覆盖 HA 生命周期，最终 workspace Clippy 覆盖全 workspace 编译/lint；磁盘空间充足，保留 Cargo cache且未执行 `cargo clean` |

## M11-12bc85 实现

Default HA connection service roots 随 Issue #8587 完成以下收窄：

- 新增窄 `DefaultHAConnectionContext`，仅共享 connection count、replica-store capability、replication progress、
  transfer metrics 与连接生命周期所需 callback；accept socket、connection、reader 与 writer task 均不再持有或导入
  完整 `ArcMut<DefaultHAService>`。
- connection registry 通过 `Weak<Mutex<HashMap<...>>>` 访问；group-transfer 与 state-notification service 由
  `OnceLock<Weak<_>>` 晚绑定。注册连接不会反向保活 service graph，owner 释放后 context 不会恢复 registry。
- AutoSwitch 的 add/ack/caught-up/remove/confirm callback 与运行时快照计算迁入 context，并继续共享 bc84 的
  `HAReplicaStoreHandle` 与 replication-state root，不复制或分叉 HA 状态机。
- GroupTransferService 与 HAConnectionStateNotificationService 的 start 入口改用共享借用，实际 lifecycle 仍由各自
  `ServiceManager` 和 Default HA shutdown 顺序拥有；accept task 继续由父 `TaskGroup` 管理。
- reviewed baseline 从 88/364 降至 84/351：production 从 24/49 降至 21/37，test 从 50/275 降至
  49/274，compatibility 保持 14/40；Broker production 保持 4/8，Store production 从 20/41 降至 17/29。
  净删除 4 identities/13 occurrences（production 3/12、test 1/1），无 relocation、新增 identity 或临时 approval。
- R15 从 5/14 降至 2/2，剩余仅 Default HA 组合根初始化 carrier；31 项执行清单保持完成 9 项、剩余 22 项，
  正式进度仍为 75/82。
...
| 命令 | 结果 |
|---|---|
| affected behavior / source | Default HA service 13/13、Default HA connection 6/6、AutoSwitch HA service 12/12，共 31 项定向 Rust 回归通过；M06 HA/Local contract 2/2 通过。首次单测编译发现 `Option<Arc<_>>` 方法引用类型不匹配和 private-interface warning，修正后所有定向命令取得 0 退出码，先前非零结果未计作通过 |
| reviewed baseline / fixtures | 正式 baseline 与 reviewed candidate 的 semantic set 均为 84/351；直接 guard、candidate compare、27/27 fixtures 与 78/78 guard tests 通过；净删除 4 identities/13 occurrences，无 relocation、新增 identity 或临时 approval |
| runtime / final gates | enforcing runtime audit、`cargo fmt --all -- --check`、workspace all-target/all-feature strict Clippy 与 `git diff --check` 通过；Windows linker stdout 与既有 future-incompatibility note 不受 `-D warnings` 管辖 |
| reduced validation scope | 不重复 package strict Clippy、额外 `cargo check`、Store/Broker 全量 lib、RocksDB 六项专项、dependency/release/performance/telemetry 矩阵、Rustdoc 或 AGENTS routing；31 项定向 Rust 回归承担受影响编译与 ACK/caught-up/remove/registry 生命周期行为，M06 固定 capability/Weak source contract，runtime audit 覆盖 HA TaskGroup/shutdown，最终 workspace Clippy 覆盖全 workspace 编译/lint；D 盘剩余 59.79 GiB，保留 Cargo cache 且未执行 `cargo clean` |

## M11-12bc86 实现

Default/AutoSwitch HA initialization 与 delegate ownership 随 Issue #8589 完成以下收窄：

- `DefaultHAService::init` 从 `&mut ArcMut<Self>` 改为独占 `&mut Self`；一次性 wiring 不再要求共享可变
  service wrapper，General HA 外层兼容根仍可通过现有可变借用完成初始化。
- `AutoSwitchHAService` 以唯一 `Box` 直接拥有 `DefaultHAService` delegate，不再在已有外层 root 内创建第二个
  `ArcMut<DefaultHAService>`；AutoSwitch 初始化通过独占借用完成 Default child wiring 与 client installation。
- start、mutable client access、role transition、replication callback、runtime info 与 shutdown 继续委托同一个
  owned Default delegate，不复制 connection registry、replication progress 或 replica-store state。
- reviewed baseline 从 84/351 降至 81/347：production 从 21/37 降至 18/33，test 保持 49/274，
  compatibility 保持 14/40；Broker production 保持 4/8，Store production 从 17/29 降至 14/25。
  净删除 3 个 production identities/4 occurrences；2 个保留 import occurrence 经临时 ADR-013 同 item 指纹审核，
  approval 仅位于忽略的 `target/`，无新增 identity 或提交态 approval。
- R15 从 2/2 降至 1/1、R16 从 5/9 降至 3/6；剩余 import-only debt 与 General HA 外层组合根一起收口。
...
| 命令 | 结果 |
|---|---|
| affected behavior / source | Default HA service 13/13、AutoSwitch HA service 13/13，共 26 项定向 Rust 回归通过；M06 HA/Local contract 2/2 通过 |
| reviewed baseline / fixtures | 正式 baseline 与 reviewed candidate 的 semantic set 均为 81/347；直接 guard、candidate compare、27/27 fixtures 与 78/78 guard tests 通过；净删除 3 个 production identities/4 occurrences，2 个保留 import occurrence 经临时 ADR-013 同 item 指纹审核，无新增 identity 或提交态 approval |
| runtime / final gates | enforcing runtime audit、`cargo fmt --all -- --check`、workspace all-target/all-feature strict Clippy 与 `git diff --check` 通过；首轮 Clippy 暴露两个 test-only import lint 与 direct delegate 导致的 pending enum size lint，改为带原因的最窄 import lint 说明和唯一 `Box` delegate 后重跑取得 0 退出码，先前非零结果未计作通过；Windows linker stdout 与既有 future-incompatibility note 不受 `-D warnings` 管辖 |
| reduced validation scope | 不重复 package strict Clippy、额外 `cargo check`、Store/Broker 全量 lib、RocksDB 六项专项、dependency/release/performance/telemetry 矩阵、Rustdoc 或 AGENTS routing；26 项定向 Rust 回归承担受影响编译与 client/role/replication/delegate 生命周期行为，M06 固定 exclusive-init/direct-delegate source contract，runtime audit 覆盖 HA TaskGroup/shutdown，最终 workspace Clippy 覆盖全 workspace 编译/lint；D 盘剩余 59.86 GiB，保留 Cargo cache 且未执行 `cargo clean` |

## M11-12bc87 实现

General HA composition root 随 Issue #8591 完成以下收口：

- `GeneralHAService` 的 Default/AutoSwitch variant 从 `ArcMut` 改为标准 `Arc`；一次性 `init` 仅在根尚未共享时
  通过 `Arc::get_mut` 完成，提前 clone 会 fail closed，而不是恢复共享可变入口。
- 新增晚绑定 `GeneralHAServiceReference`，内部仅保存标准 `Weak<DefaultHAService>` 或
  `Weak<AutoSwitchHAService>`；group-transfer 与 state-notification child 每次使用前 upgrade，根释放后请求按
  timeout/failure 结束，child 不再反向保活完整 HA graph。
- Default/AutoSwitch client/service 的 `start` 改用共享借用；accept worker 的重启 cancellation token 由异步锁
  保护，TaskGroup ownership、shutdown cancel-and-await 与连接 drain 顺序不变。
- LocalFile 初始化直接把 pending owned service 移入 General root，不再构造外层 `ArcMut`；Default、AutoSwitch、
  Group 三套重复的 LocalFile 测试根构造收口为一个 cfg(test) fixture，保留兼容 Store root 的测试边界但不传播到
  HA production module。
- reviewed baseline 从 81/347 降至 71/312：production 从 18/33 降至 13/24，test 从 49/274 降至
  44/248，compatibility 保持 14/40；Broker production 保持 4/8，Store production 从 14/25 降至 9/16。
  净删除 10 identities/35 occurrences（production 5/9、test 5/26）；3 个 test identity relocation 仅使用忽略的
  临时 ADR-013 approval，无新增 governed debt 或提交态 approval。
- R15、R16 均降至 0/0 并完成；31 项执行清单现为完成 11 项、剩余 20 项，正式进度仍为 75/82。

## M11-12bc87 验证

| 命令 | 结果 |
|---|---|
| affected behavior / source | Default HA service 13/13、AutoSwitch HA service 13/13、GroupTransfer 6/6、state notification 5/5，共 37 项定向 Rust 回归通过；另有 Local root source contract 与 Weak root-release 回归各 1/1 通过；M06 HA/Local contract 2/2 通过 |
| M06 Store contract | 完整大型变异脚本在 124 秒工具窗口到期，未计作通过；随后只运行本切片触及的 recovery/root-wiring 正向与 mutation 两项 contract，2/2 在 81.363 秒通过，并运行 Local root Rust source contract 1/1 通过，避免重复执行无关的数百项 Store 变异矩阵 |
| reviewed baseline / fixtures | 正式 baseline 与 reviewed candidate 的 semantic set 均为 71/312；直接 guard、candidate compare、27/27 fixtures 与 78/78 guard tests 通过；3 个 test fixture identity relocation 仅使用忽略的临时 ADR-013 approval，无新增 governed debt 或提交态 approval |
...
真实 test/bench caller 随 Issue #8639 完成以下收口：

- RocksDB semantics helper 不再以 `ArcMut` 包装独占的 `RocksDBMessageStore`，而是直接通过
  `OwnedMessageStore::rocksdb` 建立与生产组合根一致的 owner；recovery、dispatch、query、lifecycle 与
  compatibility mirror 行为测试保持。
- 16 个 Broker Store-capability 测试模块的 type-only `GenericMessageStore` 参数统一迁到
  `OwnedMessageStore`，覆盖 Ack、POP/Pull/Peek/Notification/Recall/Query/Transaction、failover、
  long-polling、slave sync、pre-online 与 admin config 等边界。
- `store_api_legacy_adapter`、RocksDB foundation 的 Generic trait contract，以及
  `rocketmq/src/arc_mut.rs` 自测继续作为明确 public compatibility fixture 保留，不以测试迁移提前删除公开 API。
- reviewed baseline 从 23/61 单调降至 22/60：production 保持 6/12，test 3/9→2/8，
  compatibility 保持 14/40。净删除 1 个 test identity/occurrence，无 relocation、新 governed debt 或临时
  approval。
- R17 完成；31 项最小审查清单现为完成 16 项、剩余 15 项，正式进度仍为 75/82；余下 2/8 test 仅为
  ArcMut facade 自测，转由 R18 next-major/HUMAN 兼容窗口关闭。

## M11-12bc109 验证

| 命令 | 结果 |
|---|---|
| affected behavior | RocksDB semantics 9/9 通过；`cargo check -p rocketmq-broker --tests --all-features` 通过并覆盖全部 16 个 type-only 测试迁移的编译与 trait bound |
| reviewed baseline | `python scripts/arc_mut_guard.py` 通过；reviewed baseline 只删除 RocksDB semantics helper 的 1 个 test identity/occurrence，23/61→22/60，无 relocation、新 debt 或临时 approval |
| final gates | `cargo fmt --all -- --check`、workspace all-target/all-feature strict Clippy 与 `git diff --check` 通过；最终 Clippy 复用增量缓存并在 15 秒内完成，Windows linker stdout 和既有 future-incompatibility note 不受 `-D warnings` 管辖 |
| reduced validation scope | 不重复 RocksDB foundation、Broker 全量测试执行、Store/Broker package Clippy、RocksDB 专项矩阵、runtime audit、M06 mutation、dependency/release/performance/telemetry、Rustdoc 或 AGENTS routing；本切片只改变测试 owner 类型，行为由 semantics 9/9 覆盖，全部 Broker test target 由 focused check 与最终 workspace Clippy 覆盖 |

## M11-12bc110 实现

stable nightly surface 随 Issue #8641 完成以下阶段性收口：

- 保留公开 `RequestProcessorV2::Fut` GAT API；Remoting 三个 core built-in processor 改为 concrete
  `Ready` future，继续保持静态分发与零堆分配。integration/example-only 异步 processor 使用显式 boxed
  future，不把分配引入 production core path。
- 删除 Remoting crate、integration test、complete example 的三处 `impl_trait_in_assoc_type`，并删除
  Controller 已无实际语法依赖的 `arbitrary_self_types`；all-target/all-feature compile 证明现有
  `Arc<Self>` lifecycle receiver 无需该 gate。
- 新增 `stable_surface_guard.py`、精确 policy 和 7 项正负/源码合同；baseline 模式拒绝未登记 feature、
  stale/duplicate/malformed policy，target 模式在任何 nightly feature 仍存在时 fail closed。root architecture
  CI 在 Windows/Linux 执行 baseline。
- nightly feature 属性从 8 个降至 4 个。剩余 Runtime 的 `async_fn_traits`/`unboxed_closures` 与 R18
  `sync_unsafe_cell` crate/benchmark 均有 owner 与退出窗口；ArcMut baseline 保持 22/60。
- R22 仍未完成，31 项最小审查清单保持完成 16 项、剩余 15 项，正式进度仍为 75/82。

## M11-12bc110 验证

| 命令 | 结果 |
|---|---|
| affected compile / behavior | Remoting 与 Controller all-target/all-feature check 通过；Processor V2 integration 7 passed / 1 manual allocation check ignored、library dispatcher 1/1 通过，complete example check 通过 |
| public API path | 同一工具链分别刷新 main/candidate rustdoc JSON，Remoting 与 Controller 的 named public-path 差异均为 0；已签名全局 baseline 的历史/toolchain 漂移未误归因于本切片 |
| stable-surface guard | baseline 精确报告 4 个登记 feature，7/7 guard/source-contract tests 通过；target 模式按预期失败并只列出四个登记余量 |
| stable Cargo probe | 清除主机 nightly-only `-Zthreads=4` 影响后，stable 编译推进到 `rocketmq-runtime` 并在已登记 `async_fn_traits` 失败；Remoting/Controller 不再产生更早的 stable feature error，不把该命令记作 PASS |
| final gates | `cargo fmt --all -- --check`、AGENTS routing drift guard、workspace all-target/all-feature strict Clippy 与 `git diff --check` 通过；最终 Clippy 35 秒完成，Windows linker stdout 和既有 future-incompatibility note 不受 `-D warnings` 管辖 |
| reduced validation scope | 不重复 Store/Broker/RocksDB 行为矩阵、runtime audit、M06 mutation、dependency/release/performance/telemetry 或 Rustdoc；本切片仅改变 Remoting example future 表达、无行为的 Controller gate 与治理脚本，focused processor tests、routing gate 和最终 workspace Clippy 覆盖受影响边界 |

## M11-12bc111 实现

Runtime scheduler stable surface 随 Issue #8643 完成以下阶段性收口：

- 保留 `ScheduledTaskManager` 四个公开 `_async` 方法名，将 nightly lending `AsyncFnMut` 约束改为稳定的
  `FnMut(CancellationToken) -> Fut`，其中 `Fut: Send + 'static`；兼容转发层持有内部 Tokio mutex 直到
  future 完成，继续保持旧入口的独占调用语义。
- `_async` 方法复用既有 stable scheduler，删除约 150 行重复的 fixed-rate、fixed-delay 与 no-overlap
  driver；取消、TaskGroup owner、shutdown report 与 task-id 行为继续由同一实现承担。
- Runtime 测试、Client 与 Broker 共 22 个捕获型调用点在返回 `async move` future 前克隆 owned
  capability；slave 同步时间戳改为标准 `Arc<AtomicU64>`，不新增完整 Store/runtime root 捕获或 ArcMut。
- 删除 Runtime crate 的 `async_fn_traits` 与 `unboxed_closures`，stable-surface policy 从 4 个精确余量
  降至 2 个；剩余均为 R18 `sync_unsafe_cell` 公开兼容面，ArcMut reviewed baseline 保持 22/60。
- R22 仍未完成，31 项最小审查清单保持完成 16 项、剩余 15 项，正式进度仍为 75/82。
...
| 命令 | 结果 |
|---|---|
| affected compile | Runtime all-target、Client all-target/all-feature、Broker all-target/all-feature check 通过；22 个捕获型仓内调用点全部覆盖编译 |
| scheduler behavior | owned-future 独占调用 1/1、fixed-rate/fixed-delay/no-overlap 3/3 通过 |
| stable Runtime / surface | 清除主机 nightly-only `-Zthreads=4` 后，Runtime `+stable` all-target 通过；baseline 精确报告 2 个登记 feature，8/8 guard/source-contract tests 通过；target 按预期仅列出两处 R18 `sync_unsafe_cell` 并 fail closed |
| runtime / proxy regression | enforcing runtime ownership audit 通过；Proxy Local all-target/all-feature check 通过，既有 `recursion_limit = "256"` 继续覆盖完整 Broker worker future |
| final gates | `cargo fmt --all -- --check`、workspace all-target/all-feature strict Clippy 与 `git diff --check` 通过 |
| reduced validation scope | 不重复执行 Store/Broker 全量行为套件、RocksDB 专项、M06 mutation、dependency/release/performance/telemetry、Rustdoc 或 AGENTS routing；本切片由四项调度语义、全部受影响 caller compile、stable Runtime、runtime audit、Proxy recursion 与最终 workspace Clippy 覆盖 |

## M11-12bc112 实现

ArcMut compatibility 与 R22 stable surface 随 Issue #8645 完成以下收口：

- `ArcMut`、`WeakArcMut` 与 `SyncUnsafeCellWrapper` 的 backing 从 nightly
  `std::cell::SyncUnsafeCell` 改为 workspace 已有的 stable `parking_lot::RwLock`；公开名称、constructor、
  downgrade/upgrade、unwrap、Serde、Deref、equality/hash 等 legacy 行为保持。
- `ArcMut::get_inner()` 改为返回 stable `Arc<RwLock<T>>`；仓库内没有旧签名 caller，新代码可通过
  read/write guard 访问。所有绕过 guard 的 legacy reference escape 均保留局部 `SAFETY` 说明和外部同步
  责任，因此本切片不提前关闭 R18。
- 删除零调用、仅比较 nightly cell 的 `syncunsafecell_mut` benchmark 及 manifest entry；没有把它改写成
  新的 ArcMut benchmark，避免恢复 bc109 已清零的活跃 test/bench caller。
- 删除最后两处 `sync_unsafe_cell` feature gate，stable-surface policy 清空；baseline/target guard 均报告
  0 feature。reviewed ArcMut baseline 从 22/60 降至 20/58：production 保持 6/12、test 2/8→1/7、
  compatibility 14/40→13/39，无新增 governed debt。
- stable default 与 workspace all-target/all-feature 矩阵在同一候选快照通过，R22 完成；31 项最小审查
  清单现为完成 17 项、剩余 14 项，正式进度仍为 75/82。

## M11-12bc112 验证

| 命令 | 结果 |
|---|---|
| ArcMut compatibility | RocketMQ all-target check、ArcMut facade 7/7 测试和 Broker all-target/all-feature check 通过；公开 facade 保持可编译，过时 benchmark manifest 已退出 |
| ArcMut governance | `python scripts/arc_mut_guard.py`、27/27 fixtures 与 79/79 guard tests 通过；reviewed baseline 精确为 20/58（production 6/12、test 1/7、compatibility 13/39） |
| stable surface | baseline/target guard 均以 0 feature 通过，9/9 stable source-contract tests 通过 |
| stable workspace matrix | `RUSTFLAGS=-Cdebuginfo=0 cargo +stable check --workspace` 与 `RUSTFLAGS=-Cdebuginfo=0 CARGO_INCREMENTAL=0 cargo +stable check --workspace --all-targets --all-features` 通过；后者从清理后的 target 完整执行 5 分 22 秒 |
| consumer / final gates | Example、Tauri Rust、Web backend 三个 standalone consumer 的本地 fmt/strict Clippy 通过，Web backend 额外 all-target/all-feature build 通过；dependency target、AGENTS routing、root format、workspace all-target/all-feature strict Clippy 与 diff check 全部通过 |
| reduced validation scope | 不重复 Store/Broker 行为套件、RocksDB 专项、runtime audit、M06 mutation、telemetry、performance、release 或 Rustdoc；本切片只改变 ArcMut backing、删除过时 benchmark 与 stable 治理，聚焦 facade 测试、所有 target 编译、完整 stable matrix、standalone consumer 和最终 workspace Clippy 覆盖受影响边界 |

## M11-12bc113 实现

R23 ArcMut soundness audit 随 Issue #8647 完成以下收口：

- 新增 `arc_mut_soundness_probe.py`，按 RocketMQ manifest 与 ArcMut source fingerprint 在忽略的
  `target/architecture-refactor/` 下生成隔离 Cargo probe；probe 直接依赖当前 checkout，不把新的 ArcMut
  caller 写入仓库 baseline。
- Miri 正向路径只使用 `get_inner().read/write` guard 并通过；负向路径让两个 clone 仅通过 safe
  `DerefMut` 建立两个可变引用，Miri 以 Stacked Borrows/Unique retag 报告预期 UB。
- runner 对负向结果 fail closed：退出码为 0、缺少 `Undefined Behavior`，或仅发生下载/工具失败都不能记作
  soundness evidence；4 项 Python 单测固定分类合同。
- 新增 Loom 0.7.2 模型，2/2 验证 `Arc<RwLock<T>>` 两 writer 串行和 worker 强 owner 释放后 root
  `try_unwrap`。该版本不暴露 `Weak`，证据明确不声称覆盖 weak-upgrade/drop 交错。
- 技术审计拒绝长期保留 ArcMut safe reference escape；R18 仍须等待 next-major 两轮弃用与
  Release Manager/HUMAN 批准后删除 facade。ArcMut baseline 保持 20/58，stable feature 保持 0。
- R23 完成；31 项最小审查清单现为完成 18 项、剩余 13 项，正式进度仍为 75/82。

## M11-12bc113 验证

| 命令 | 结果 |
|---|---|
| classifier | `python -m unittest scripts.tests.test_arc_mut_soundness_probe -v` 4/4 通过 |
| Loom replacement | `cargo test -p rocketmq-rust --test arc_mut_replacement_loom` 2/2 通过；首次使用 unsupported `loom::Arc::downgrade` 的非零编译未计作 PASS，模型收窄为可实际证明的 worker strong-owner release 后通过 |
| Miri audit | `python scripts/arc_mut_soundness_probe.py` 通过：fingerprint `096500ef5369c6d3`、guarded PASS、alias expected UB、Loom PASS |
| governance | ArcMut guard 20/58 通过，stable target guard 以 0 feature 通过 |
| final gates | RocketMQ stable all-target、AGENTS routing、format、workspace all-target/all-feature strict Clippy 与 diff check 全部通过 |
| reduced validation scope | 不重复 stable workspace matrix、Store/Broker 行为套件、RocksDB、runtime audit、standalone consumer、dependency/telemetry/performance/release 或 Rustdoc；本切片只新增 soundness runner、Loom dev model 和证据，Miri 正负合同、Loom 2/2、治理守卫与一次最终 workspace 门禁覆盖变更 |

## 剩余切片与 Gate

2026-07-23 盘点将执行工作固化为 31 个最小可审查单元：16 个 production owner、2 个
test/compatibility、7 个 M10/Phase 3 动态验收与签署、6 个 M12；R01～R08、R10～R17、R20、R22～R24
已完成，当前剩余 11 个，正式进度仍为 75/82。
...
3. Production `WeakArcMut` 与活跃 test/bench caller 已清零；1/7 test 余量仅为 ArcMut facade 自身兼容性
   测试。公开 `arc_mut.rs`/re-export 的 destructive 删除受 next-major 两轮弃用与 Release Manager/HUMAN
   Gate 约束，不能静默重置 public API baseline。
4. nightly feature 属性已从 8 个降至 0 个，baseline/target guard 和完整 stable workspace matrix 均通过，
   R22 已关闭；R23 又以 Miri expected UB 拒绝长期保留 ArcMut，并以 Loom 2/2 固定 guarded replacement。
   R24 已固化 soak/SLO、dashboard/alerts/runbook、rollback 和 evidence index 工程合同，R20 已关闭
   container dynamic。继续按批准窗口删除 R18 compatibility facade，并执行真实 Kind/K3d、M10 固定硬件和
   Human Gate。
...
R24 soak/SLO 发布证据合同随 Issue #8649 完成以下工程交付：

- `architecture-production-readiness-policy.json` 固定 21600 秒 soak、60 秒采样、1% 缺样上限、五项
  semantic-registry metric 目标、两项 M11-11 fault 目标和七项 rollback assertion。
- Grafana dashboard 与 Prometheus alerts 只引用 telemetry semantic registry 登记的 `rocketmq_*` metric；
  collector outage 和 drain/rollback 继续由真实 fault artifact 证明，不虚构在线 metric。
- 英文 runbook 对五项 alerts、两个 fault-only 目标、promotion stop、baseline image/chart rollback、PVC/WAL
  保留和 evidence verification 给出精确操作。
- `run-architecture-slo-evidence.ps1` 先验证同一 candidate image map 的动态 M11-11 fault run，再对 production
  Prometheus API 执行六小时采样；失败运行不保留可被误读为 PASS 的 `run.json`。
- `architecture_slo_guard.py` 绑定当前 full Git SHA、五服务 digest、policy、fault snapshot、七项目标、
  rollback、unresolved alert/fault 与所有 generated/committed artifact SHA-256；fixture 必须显式 opt-in。
- R24 完成时，31 项最小审查清单为完成 19 项、剩余 12 项；其后 R20 已关闭。R21 动态环境、
  R25 四方签署和 R09/R18 compatibility 保持开放，正式进度仍为 75/82。
...
| `python scripts/architecture_slo_guard.py --evidence scripts/tests/fixtures/m11-slo/pass --allow-fixture` | PASS；fixture 明确非动态 |
| `python -m unittest scripts.tests.test_architecture_slo_guard -v` | 9/9 PASS，覆盖 opt-in、dynamic 冒充、短 soak、目标失败、fault image mismatch、artifact/hash 和未登记 metric |
| `.\scripts\run-architecture-slo-evidence.ps1 -Mode Validate` | PASS；只验证合同，不生成 dynamic PASS |
| 动态六小时 workflow | 当时未执行：本机无 Docker/Kind/K3d/Kubectl/Helm、真实镜像/Secret/Prometheus endpoint；container R20 已后续关闭，cluster/SLO 仍保留给 R21/R25 |

## R20 五服务容器动态闭环

Issue #8677 使用 [Container Foundation run `30011167537`](https://github.com/mxsm/rocketmq-rust/actions/runs/30011167537)
在 main commit `13d50e2d33ddfc1142bba63431b339d07704a4f7` 上关闭 R20：

- foundation 与 Broker、NameServer、Controller、Proxy、MCP 五服务 target 全部构建成功；
- 缺配置 fail-closed、数字 non-root、read-only rootfs、声明式 volume/tmpfs 与真实 SIGTERM 退出全部通过；
- 五服务 CycloneDX、Trivy 零 CRITICAL、Cosign SBOM bundle 与 provenance 全部生成，success/failure 路径均未保留 private key；
- artifact `container-foundation`（ID `8565842850`）archive digest 为
  `sha256:bc8172178a0527a049a79d7c6be0d0811501067acb7336df94f50b5447d32a7f`；
- 本地只读复核重算 15 个 service SBOM/Trivy/Sigstore 文件 hash，确认 service set 恰好为 5、source commit
  一致、CycloneDX 格式正确、CRITICAL 为 0、private key 不存在。

R20 完成后，31 项最小审查清单为完成 **20/31**、剩余 **11**；正式顶层工作包仍为完成 **75/82**、
剩余 **7**。R21 production digest/Secret/Kind-K3d fault/rolling、R19 固定硬件、R25 四方签署与
R09/R18 compatibility 不由本 run 替代。

## R20 验证

| Gate | 结果 |
|---|---|
| GitHub Actions | run `30011167537`，静态合同、runtime foundation、five service images、artifact upload 全部 `success` |
| Foundation provenance | pinned builder/runtime/toolchain/snapshot，UID/GID `10001:10001`，CRITICAL `0`，SBOM/Trivy/bundle hash 完整 |
| Five-service provenance | Broker、NameServer、Controller、Proxy、MCP 恰好 5 个，均为 `10001:10001`、`SIGTERM`、CRITICAL `0` |
| Artifact integrity | `R20_EVIDENCE_OK commit=13d50e2d33ddfc1142bba63431b339d07704a4f7 services=5 critical=0 private_keys=false` |
| 完整证据 | [`11-container-dynamic-evidence.md`](11-container-dynamic-evidence.md) |
