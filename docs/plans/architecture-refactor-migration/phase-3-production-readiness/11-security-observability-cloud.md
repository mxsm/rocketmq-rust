...
M11-12aw 已将 `ScheduleMessageService` root 改为标准 `Arc`，并以 Broker outer-owned EscapeBridge strong owner 与
BrokerRuntimeInner `Weak` 回边拆除强引用环；Schedule/POP/ACK/transaction 不再经 mutable EscapeBridge capability
跨 `.await`。每次启动创建 fresh generation TaskGroup child lease，任务只捕获 service `Weak` 并使用 cancellation-aware
等待；停止先封闭准入和重调度、取消/drain，再执行一次最终持久化。生产 load、周期/最终/peer 写入均由
BlockingExecutor 所有；legacy Builder 复用既有 audited scheduler root 并在其下创建 owned blocking child，peer 写盘
成功后才发布内存 snapshot。Broker shutdown 在 message-store 前停止 Schedule，
controller 降级先完成 Schedule 停止且失败会阻止 Store 角色切换。实际快照为 282 production/654 occurrences、
157 test/452 occurrences，Broker owner 为 160/346；compatibility 14/40 不增。下一子切片 M11-12ax 继续处理
Broker 其他 processor/transaction owner，完整候选快照与 HUMAN Gate 仍保持开放。

M11-12ax 已将 transaction service root 及 send/reply/end-transaction processor capability 改为标准 `Arc`，
op-batch worker 只持标准 `Weak` 回边；共享 service API 由显式 Tokio mutex 串行 bridge 可变操作。transaction check
service 直接注入 config、service 与 listener，不再通过 BrokerRuntimeInner `mut_from_ref` 获取服务；无状态
Broker2Client 改用标准 Arc。shutdown 按 check service/listener→service/batch 顺序 drain，batch ready snapshot 与失败
回退路径均在后续 I/O 前释放 delete-context 锁，避免同一 mutex 嵌套获取。实际快照为 274 production/634
occurrences、156 test/451 occurrences，Broker owner 为 152/326，transaction 子树为 5/8；compatibility 14/40
...
M11-12bc2 随 Issue #8408 将 Topic persistence、single/increment/full registration 与 shutdown unregister 收入同一有界
coordinator：文件/RocksDB I/O 经 `BlockingExecutor`，shutdown 先关闭 admission 并排空已接纳命令，只有最终版本稳定、
registration 静默、worker 与 blocking task 退出后才 unregister/detach。Broker shutdown typed report 已纳入
