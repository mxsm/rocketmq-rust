...
- **test(client):** Cover client log-filter TTL and operation-ID boundaries in `set_properties`/`restore_properties`: inclusive TTL limits, exact 128-byte operation-ID preservation and 129-byte rejection, empty/whitespace/control/unsupported-punctuation rejection in both entry points, and INFO-level lowercasing with a directive-free restore payload ([#10541](https://github.com/mxsm/rocketmq-rust/issues/10541)).
