...
任务组准入经历 open、closing、closed 和 shutdown-completed 状态。关闭会停止新注册、取消工作并缓存报告。创建请求与关闭发生竞态时，必须尊重准入结果，不能通过新建子作用域逃避父截止时间。

## 阻塞工作实际退出前一直占用容量

子上下文提供受管理的 `storage_io`、`metadata_io` 和 `cpu_crypto` 执行器。通道具有各自的并发/排队策略，并共享所有者的全局阻塞准入预算。每个通道按到达顺序准入等待者，每次释放最多唤醒一个等待者。克隆通道不会创建新池。
...
短工作边界拒绝 `BlockingKind::LongRunning`。长期阻塞循环需要具有停止和 join 行为的显式线程或领域服务所有者。`BlockingExecutor::new` 创建隔离的执行器，其独立预算不会纳入根受管理通道。
...
`ScheduledTaskGroup` 是唯一的调度器，同时持有驱动和每次运行。`schedule(config, policy, task)` 的时间模式由配置决定：固定延迟在一次运行结束后等待；固定频率按绝对时刻触发，按策略要么从不重叠，要么最多允许 `n` 次运行并发。所有执行槽都被占用时，由漏触发策略决定跳过、保留最新一次或有限补偿。漂移指标记录每次触发的延迟。
...
`ShutdownReport::is_healthy` 检查失败、panic、超时、泄漏、仍运行的阻塞任务以及子报告。单独的 `aborted` 计数不会使该谓词为假。因此，健康谓词是其契约范围内的有效证据，不能证明所有任务自然结束或所有外部效果均已提交。

还需要检查组件专用报告：Store 可以报告刷盘、租约和文件退役状态，通用任务报告无法推断这些信息。运维 API 应使用有界、脱敏的 `RuntimeDiagnosticsViewV1`，避免暴露原始任务名、参数或配置；API 调用方仍负责认证。

配置契约违反与操作性 `RuntimeError` 是不同错误通道。适配启动错误时应保留区别，而不是将全部失败转为 panic。应按 `RuntimeError::kind()`（例如 `Closed` 或 `Poisoned`）分支，而不是按操作标签推断原因。

## 兼容路径与后续阅读

`RuntimeContext` 用于现有 Tokio 运行时内的迁移或测试，不持有也不关闭宿主运行时，并采用宽松测试预算，不能描述为与生产 `RuntimeOwner` 组合完全相同。原有的执行器服务、调度器和线程适配器已经删除，替代方式见[运行时迁移说明](https://github.com/mxsm/rocketmq-rust/blob/main/rocketmq-runtime/MIGRATION.md)。
