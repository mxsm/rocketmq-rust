...
//             .parse()
//             .map_err(|_| DeserializeHeaderError(format!("Parse field {} error",
// Self::QUEUE_ID)))?;         self.queue_offset = self
...
//             .parse()
//             .map_err(|_| DeserializeHeaderError(format!("Parse field {} error",
// Self::SYS_FLAG)))?;         self.commit_offset = self
//             .get_and_check_not_none(fields,
// &CheetahString::from_static_str(Self::COMMIT_OFFSET))?             .parse()
//             .map_err(|_| {
