        ...
        if let Some(exception_inner) = exception {
            match exception_inner {
                RocketmqError::AbortProcessError(code, message) => {
                    ...
                    tokio::select! {
                        result =self.channel.0.connection.send_command(response.set_opaque(opaque)) => match result{
                            Ok(_) =>{},
                            Err(err) => {
                                match err {
...
impl<RP: RequestProcessor + Sync + 'static + Clone> ConnectionListener<RP> {
    async fn run(&mut self) -> anyhow::Result<()> {
        ...
        loop {
            ...
            let channel = Channel::new(Connection::new(socket), local_addr, remote_addr);
...
pub async fn run<RP: RequestProcessor + Sync + 'static + Clone>(
    ...
    let handler = RemotingGeneralHandler {
        ...
        rpc_hooks: rpc_hooks,
        response_table: HashMap::with_capacity(512),
    };
