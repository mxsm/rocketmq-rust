...
"$python_command" distribution/transfer_candidate.py import --bundle "$candidate_control_bundle" --output "$control_import"
candidate="$control_import/CANDIDATE_RUN.json"
...
run_route H03-DRAFT-NO-REMOTE "$worker" "$context" "$event_root" "$python_command" scripts/no_remote_publication_guard.py --candidate-manifest "$candidate" --phase 6 --audit-point handoff-draft --context-root "$context_root" --event-root "$event_root" --handoff "$staging" --output "$source_import/evidence/H03-DRAFT-NO-REMOTE.json"
...
run_route H04-FINAL-SEMANTIC "$worker" "$context" "$event_root" "$python_command" distribution/verify_publication_handoff.py --handoff "$final" --candidate-manifest "$candidate" --candidate-root "$source_import" --source-root "$repository_source" --final-pre-ready --result-id H04-FINAL-SEMANTIC --output "$source_import/evidence/H04-FINAL-SEMANTIC.json"
run_route H05-FINAL-NO-REMOTE "$worker" "$context" "$event_root" "$python_command" scripts/no_remote_publication_guard.py --candidate-manifest "$candidate" --phase 6 --audit-point handoff-final --context-root "$context_root" --event-root "$event_root" --handoff "$final" --output "$source_import/evidence/H05-FINAL-NO-REMOTE.json"
