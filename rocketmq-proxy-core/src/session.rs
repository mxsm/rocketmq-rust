...
pub struct ClientSessionRegistry<C = ()> {
    ...
    remoting_channels: Arc<DashMap<String, C>>,
...
impl<C> Default for ClientSessionRegistry<C> {
    fn default() -> Self {
        Self {
            ...
            remoting_channels: Arc::default(),
            pending_telemetry_commands: Arc::default(),
            thread_stack_traces: Arc::default(),
            verify_message_reports: Arc::default(),
            pending_lite_unsubscribe_notices: Arc::default(),
        }
    }
}

impl<C> ClientSessionRegistry<C> {
    pub fn upsert_from_context<P>(&self, context: &ProxyContextWithPrincipal<P>) {
        self.upsert_from_context_with_client_type(context, None);
    }

    pub fn upsert_from_context_with_client_type<P>(
        &self,
    ...
    pub fn update_membership_from_remoting_heartbeat<P>(
        &self,
        context: &ProxyContextWithPrincipal<P>,
        client_id: &str,
    ...
    pub fn update_settings_from_telemetry<P>(
        &self,
        context: &ProxyContextWithPrincipal<P>,
    ...
    pub fn bind_remoting_channel(&self, client_id: impl Into<String>, channel: C) {
        self.remoting_channels.insert(client_id.into(), channel);
    }

    pub fn remoting_channel(&self, client_id: &str) -> Option<C>
    where
        C: Clone,
    {
...
mod tests {
    ...
    type ClientSessionRegistry = super::ClientSessionRegistry<()>;
