...
const PROPERTY_TIMER_OUT_MS: &str = "TIMER_OUT_MS";
const PROPERTY_TRACE_CONTEXT: &str = "TRACE_CONTEXT";
const PROPERTY_TRANSACTION_PREPARED: &str = "TRAN_MSG";
const PROPERTY_TRANSIENT_PREFIX: &str = "__RMQ.TRANSIENT.";
const PROPERTY_UNIQ_CLIENT_MESSAGE_ID: &str = "UNIQ_KEY";
...
pub fn build_query_assignment_response(
    ...
    let assignments: Vec<v2::Assignment> = plan
        ...
        .queue_datas
        .iter()
        .filter(|queue_data| is_readable(queue_data.perm) && queue_data.read_queue_nums > 0)
...
fn build_rocketmq_message(
    ...
    let mut rocketmq_message = ProxyMessage::new(topic.to_string(), message.body.clone());
    rocketmq_message.put_property(PROPERTY_UNIQ_CLIENT_MESSAGE_ID, client_message_id);

    if let Some(tag) = system.tag.as_deref() {
        let tag = tag.trim();
        if !tag.is_empty() {
            rocketmq_message.put_property(PROPERTY_TAGS, tag);
        }
    }

    if !system.keys.is_empty() {
        rocketmq_message.put_property(PROPERTY_KEYS, system.keys.join(KEY_SEPARATOR));
    }

    if let Some(trace_context) = system.trace_context.as_deref() {
        let trace_context = trace_context.trim();
        if !trace_context.is_empty() {
            rocketmq_message.put_property(PROPERTY_TRACE_CONTEXT, trace_context);
        }
    }

    for (key, value) in &message.user_properties {
        if is_system_reserved_user_property(key) {
            return Err(ProxyError::message_property_conflict(format!(
                "message user property '{key}' is reserved by the system"
            )));
        }
        rocketmq_message.put_property(key, value);
    }
...
fn apply_message_type(
    ...
    match message_type {
        ...
        ProxyTopicMessageType::Fifo => {
            ...
            message.put_property(PROPERTY_SHARDING_KEY, message_group);
            Ok(())
        }
...
fn build_message_from_ext(message: &ProxyMessageExt, invisible_duration: Option<Duration>) -> v2::Message {
    v2::Message {
...
fn build_message_system_properties(
    ...
    v2::SystemProperties {
        tag: message_ext.property(PROPERTY_TAGS).map(ToOwned::to_owned),
        keys,
        ...
        receipt_handle: message_ext.property(PROPERTY_POP_CK).map(ToOwned::to_owned),
        ...
        message_group: message_ext.property(PROPERTY_SHARDING_KEY).map(ToOwned::to_owned),
...
fn build_user_properties(message: &ProxyMessageExt) -> HashMap<String, String> {
    message
        .properties()
        .iter()
...
fn is_system_reserved_user_property(key: &str) -> bool {
    matches!(
        key,
        "TRACE_ON"
            | "MSG_REGION"
            ...
            | "TIMER_GENERATION"
            | "E_T"
            | "TIMER_FORMAT_VERSION"
            | "TIMER_POLICY_FINGERPRINT"
            | "TIMER_DELAY_LEVEL"
            | "__BORNHOST"
            | "BORN_TIMESTAMP"
            | "DLQ_ORIGIN_TOPIC"
...
fn is_reserved_message_property(key: &str) -> bool {
    is_system_reserved_user_property(key)
        || key.starts_with(PROPERTY_TRANSIENT_PREFIX)
        || key == PROPERTY_SHARDING_KEY
        || key == PROPERTY_TRACE_CONTEXT
}

fn dead_letter_queue(message: &ProxyMessageExt) -> Option<v2::DeadLetterQueue> {
    let topic = message.property(PROPERTY_DLQ_ORIGIN_TOPIC)?;
    let message_id = message.property(PROPERTY_DLQ_ORIGIN_MESSAGE_ID)?;
...
fn received_message_type(message: &ProxyMessageExt) -> v2::MessageType {
    ...
    if message.property(PROPERTY_TIMER_OUT_MS).is_some() || message.property(PROPERTY_TIMER_DELIVER_MS).is_some() {
        return v2::MessageType::Delay;
    }
    if message.property(PROPERTY_TRANSACTION_PREPARED).is_some() {
...
fn gen_message_queue_from_queue_data(
    ...
    if is_writeable(queue_data.perm) && is_readable(queue_data.perm) {
    ...
    } else if is_writeable(queue_data.perm) {
        write_only = queue_data.write_queue_nums;
    } else if is_readable(queue_data.perm) {
...
fn is_readable(permission: u32) -> bool {
    permission & 0b100 == 0b100
}

fn is_writeable(permission: u32) -> bool {
    permission & 0b010 == 0b010
}

fn convert_permission(perm: u32) -> i32 {
    if is_readable(perm) && is_writeable(perm) {
        return v2::Permission::ReadWrite as i32;
    }
    if is_readable(perm) {
        return v2::Permission::Read as i32;
    }
    if is_writeable(perm) {
...
fn permission_for_queue_id(queue_data: &QueueData, queue_id: i32) -> i32 {
    ...
    if is_readable(queue_data.perm) && is_writeable(queue_data.perm) {
...
mod tests {
    ...
    fn send_request(user_properties: HashMap<String, String>) -> v2::SendMessageRequest {
    ...
    #[test]
    fn endpoint_resolution_consumes_only_grpc_config() {
        let config = GrpcConfig {
            listen_addr: "127.0.0.1:8081".to_owned(),
            use_endpoint_port_from_request: false,
            ..GrpcConfig::default()
        };
        let request = v2::QueryRouteRequest {
            topic: Some(v2::Resource {
                resource_namespace: String::new(),
                name: "TopicA".to_owned(),
            }),
        ...
        let mapped = build_query_route_request(&config, &request).expect("route request");

        assert_eq!(mapped.endpoints[0].port, 8081);
    }
    ...
    fn send_message_maps_wire_properties_to_core_dto() {
        ...
        assert_eq!(message.property(PROPERTY_TAGS), Some("TagA"));
    ...
    #[test]
    fn send_message_rejects_reserved_user_property() {
        let error = build_send_message_request(
            &grpc_context(),
            &send_request(HashMap::from([("UNIQ_KEY".to_owned(), "spoofed".to_owned())])),
        )
        .expect_err("reserved user property must fail");

        assert!(matches!(error, ProxyError::MessagePropertyConflictWithType { .. }));
    }

    #[test]
    fn outbound_message_hides_system_properties() {
        let mut message = ProxyMessage::new("TopicA", b"hello".to_vec());
        message.put_property(PROPERTY_POP_CK, "receipt");
        message.put_property("custom", "value");
        let message_ext = ProxyMessageExt {
            message,
            msg_id: "msg-1".to_owned(),
            ..ProxyMessageExt::default()
        };

        let mapped = build_message_from_ext(&message_ext, None);

        assert_eq!(mapped.user_properties.get("custom").map(String::as_str), Some("value"));
        assert!(!mapped.user_properties.contains_key(PROPERTY_POP_CK));
        assert_eq!(
            mapped
                .system_properties
                .and_then(|properties| properties.receipt_handle),
            Some("receipt".to_owned())
        );
    }

    #[test]
    fn permission_bits_preserve_legacy_mapping() {
        assert_eq!(convert_permission(0b110), v2::Permission::ReadWrite as i32);
        assert_eq!(convert_permission(0b100), v2::Permission::Read as i32);
        assert_eq!(convert_permission(0b010), v2::Permission::Write as i32);
        assert_eq!(convert_permission(0), v2::Permission::None as i32);
    }
}
