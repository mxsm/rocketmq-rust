...
pub(super) fn stamp(file: &File) -> Result<FileStamp, PlatformFailure> {
    ...
    let length = u64::try_from(standard.EndOfFile)
        .map_err(|_| PlatformFailure::unsafe_namespace("file has a negative length"))?;
    let allocation_size = u64::try_from(standard.AllocationSize)
        .map_err(|_| PlatformFailure::unsafe_namespace("file has a negative allocation size"))?;
    Ok(FileStamp {
...
fn query_handle<T: Copy>(
    ...
    unsafe {
        GetFileInformationByHandleEx(
            ...
            u32::try_from(size_of::<T>()).map_err(|_| PlatformFailure::limit("handle info size"))?,
        )
        .map_err(|error| PlatformFailure::windows(context, error))?;
...
fn open_relative(parent: &File, name: &str, kind: EntryKind, absent_ok: bool) -> Result<Option<File>, PlatformFailure> {
    ...
    let byte_length = wide
        ...
        .ok_or_else(|| PlatformFailure::limit("relative component is too long"))?;
    ...
    let attributes = OBJECT_ATTRIBUTES {
        Length: u32::try_from(size_of::<OBJECT_ATTRIBUTES>())
            .map_err(|_| PlatformFailure::limit("OBJECT_ATTRIBUTES size"))?,
    ...
    let desired = match kind {
        ...
        EntryKind::Reparse | EntryKind::Other => {
            return Err(PlatformFailure::unsafe_namespace("unsupported relative entry kind"))
        }
    };
    ...
    if status == STATUS_OBJECT_NAME_NOT_FOUND {
        ...
        return Err(PlatformFailure::changed(format!("relative entry {name:?} disappeared")));
    }
    if !nt_success(status) {
        return Err(PlatformFailure::io("NtCreateFile", status_error(status)));
    }
...
pub(super) fn enumerate_directory(directory: &File, maximum: usize) -> Result<Vec<InventoryEntry>, PlatformFailure> {
    ...
    loop {
        ...
        if !nt_success(status) {
            return Err(PlatformFailure::io("NtQueryDirectoryFile", status_error(status)));
        }
        ...
        loop {
            if offset > used.saturating_sub(DIRECTORY_ENTRY_HEADER_LENGTH) {
                return Err(PlatformFailure::unsafe_namespace("directory entry header is truncated"));
            }
            ...
            let name_bytes = usize::try_from(info.FileNameLength)
                .map_err(|_| PlatformFailure::limit("directory entry name length"))?;
            if name_bytes % 2 != 0 {
                return Err(PlatformFailure::unsafe_namespace(
            ...
            let name_start = offset
                ...
                .ok_or_else(|| PlatformFailure::limit("directory entry offset overflow"))?;
            let name_end = name_start
                .checked_add(name_bytes)
                .ok_or_else(|| PlatformFailure::limit("directory entry name overflow"))?;
            if name_end > used {
                return Err(PlatformFailure::unsafe_namespace("directory entry name is truncated"));
            }
            ...
            if name != "." && name != ".." {
                ...
                let length = u64::try_from(info.EndOfFile)
                    .map_err(|_| PlatformFailure::unsafe_namespace("directory entry has a negative length"))?;
                let allocation_size = u64::try_from(info.AllocationSize)
                    .map_err(|_| PlatformFailure::unsafe_namespace("directory entry has a negative allocation size"))?;
                ...
                let stamp = if kind == EntryKind::Reparse {
                    ...
                    let opened = open_relative(directory, &name, kind, false)?.ok_or_else(|| {
                        PlatformFailure::changed(format!("entry {name:?} disappeared during enumeration"))
                    })?;
            ...
            let next = usize::try_from(info.NextEntryOffset)
                .map_err(|_| PlatformFailure::limit("directory next-entry offset"))?;
