...
impl TimelineManifestV1 {
    ...
    pub(crate) fn load(root: &Path) -> Result<Self, TimelineManifestError> {
        ...
        Err(TimelineManifestError::NoValidCopy)
    }
    ...
    pub(crate) fn publish_next(&self, root: &Path) -> Result<Self, TimelineManifestError> {
        ...
        next.generation = self
            .generation
            .checked_add(1)
            .ok_or(TimelineManifestError::GenerationExhausted)?;
        next.validate()?;
        ...
        match OpenOptions::new().create_new(true).write(true).open(&path) {
            ...
            Err(error) if error.kind() == std::io::ErrorKind::AlreadyExists => {
                if Self::decode(&std::fs::read(path)?)? == *self {
                    ...
                    Err(TimelineManifestError::ArchiveConflict)
                }
            }
    ...
    fn validate(&self) -> Result<(), TimelineManifestError> {
    ...
    fn encode(&self) -> Result<Vec<u8>, TimelineManifestError> {
        self.validate()?;
        ...
        let total_len = MANIFEST_HEADER_SIZE
            ...
            .ok_or(TimelineManifestError::LengthOverflow)?;
        ...
        output.extend_from_slice(
            &u64::try_from(total_len)
                .map_err(|_| TimelineManifestError::LengthOverflow)?
                .to_be_bytes(),
        );
    ...
    fn decode(bytes: &[u8]) -> Result<Self, TimelineManifestError> {
...
fn encode_run(output: &mut Vec<u8>, run: &TimelineRunDescriptor) -> Result<(), TimelineManifestError> {
...
fn decode_run(bytes: &[u8], cursor: &mut usize, end: usize) -> Result<TimelineRunDescriptor, TimelineManifestError> {
    ...
    let kind = match take_u8(bytes, cursor, end)? {
        ...
        _ => return Err(TimelineManifestError::InvalidRecord),
    };
...
fn encode_text(output: &mut Vec<u8>, value: &str) -> Result<(), TimelineManifestError> {
    let length = u16::try_from(value.len()).map_err(|_| TimelineManifestError::LengthOverflow)?;
    output.extend_from_slice(&length.to_be_bytes());
    output.extend_from_slice(value.as_bytes());
    Ok(())
}

fn take_text(bytes: &[u8], cursor: &mut usize, end: usize) -> Result<String, TimelineManifestError> {
    let length = usize::from(take_u16(bytes, cursor, end)?);
    let value = take(bytes, cursor, end, length)?;
    std::str::from_utf8(value)
        .map(str::to_owned)
        .map_err(|_| TimelineManifestError::InvalidRecord)
}

fn take<'a>(bytes: &'a [u8], cursor: &mut usize, end: usize, length: usize) -> Result<&'a [u8], TimelineManifestError> {
    let next = cursor
        .checked_add(length)
        .ok_or(TimelineManifestError::LengthOverflow)?;
    if next > end {
...
fn take_u8(bytes: &[u8], cursor: &mut usize, end: usize) -> Result<u8, TimelineManifestError> {
    Ok(take(bytes, cursor, end, 1)?[0])
}

fn take_u16(bytes: &[u8], cursor: &mut usize, end: usize) -> Result<u16, TimelineManifestError> {
...
fn take_u32(bytes: &[u8], cursor: &mut usize, end: usize) -> Result<u32, TimelineManifestError> {
...
fn take_i32(bytes: &[u8], cursor: &mut usize, end: usize) -> Result<i32, TimelineManifestError> {
...
fn take_u64(bytes: &[u8], cursor: &mut usize, end: usize) -> Result<u64, TimelineManifestError> {
...
fn take_i64(bytes: &[u8], cursor: &mut usize, end: usize) -> Result<i64, TimelineManifestError> {
...
fn u32_len(value: usize) -> Result<u32, TimelineManifestError> {
    u32::try_from(value).map_err(|_| TimelineManifestError::LengthOverflow)
}

fn read_array<const N: usize>(bytes: &[u8], offset: usize) -> Result<[u8; N], TimelineManifestError> {
    bytes
        .get(offset..offset.saturating_add(N))
        .and_then(|value| value.try_into().ok())
        .ok_or(TimelineManifestError::InvalidRecord)
}

fn read_u16(bytes: &[u8], offset: usize) -> Result<u16, TimelineManifestError> {
    Ok(u16::from_be_bytes(read_array(bytes, offset)?))
}

fn read_u32(bytes: &[u8], offset: usize) -> Result<u32, TimelineManifestError> {
    Ok(u32::from_be_bytes(read_array(bytes, offset)?))
}

fn read_u64(bytes: &[u8], offset: usize) -> Result<u64, TimelineManifestError> {
