...
impl DefaultHAClient {
    ...
    pub fn new(default_message_store: ArcMut<LocalFileMessageStore>) -> Result<Arc<Self>, HAClientError> {
    ...
    async fn dispatch_read_request(&self) -> bool {
        let mut read_buffer = self.byte_buffer_read.write().await;
        let mut dispatch_pos = self.dispatch_position.load(Ordering::SeqCst);

        loop {
            let available_data = read_buffer.len() - dispatch_pos;

            if available_data >= TRANSFER_HEADER_SIZE {
                // Create a cursor for reading from the buffer
                let mut cursor = Cursor::new(&read_buffer[dispatch_pos..]);
                ...
                if available_data >= TRANSFER_HEADER_SIZE + body_size {
                    let data_start = dispatch_pos + TRANSFER_HEADER_SIZE;
                    let data_end = data_start + body_size;

                    // Extract body data as Bytes for zero-copy
                    let bytes = read_buffer.clone().freeze().slice(data_start..data_end);
                    // Append to commit log
                    drop(read_buffer);
                    if let Err(e) = self
                        .default_message_store
                        .append_to_commit_log(master_phy_offset, bytes.as_ref(),data_start as i32,body_size as i32)
                    {
                    ...
                    dispatch_pos += TRANSFER_HEADER_SIZE + body_size;
                    self.dispatch_position.store(dispatch_pos, Ordering::SeqCst);
            ...
            if read_buffer.len() >= READ_MAX_BUFFER_SIZE {
                ...
                self.reallocate_byte_buffer().await;
                read_buffer = self.byte_buffer_read.write().await;
                dispatch_pos = self.dispatch_position.load(Ordering::SeqCst);
            }
