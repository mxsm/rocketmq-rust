...
use crate::queue::ConsumeQueueStoreTraitBak;
use crate::queue::ConsumeQueueTable;
use crate::queue::ConsumeQueueTraitBak;
...
#[allow(unused_variables)]
impl ConsumeQueueStoreTraitBak for ConsumeQueueStore {
    #[inline]
    fn start(&self) {
...
impl ConsumeQueueStore {
    ...
    fn put_consume_queue(
        ...
        topic: CheetahString,
        queue_id: i32,
        consume_queue: Box<dyn ConsumeQueueTraitBak>,
    ) {
        let mut consume_queue_table = self.inner.consume_queue_table.lock();
