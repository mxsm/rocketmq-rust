            ...
            |b, &size| {
                b.to_async(&rt).iter(|| async {
                    ...
                    for i in 0..100 {
                        ...
                        black_box(file.write_async(&data, offset).await.unwrap());
                    }
        ...
        group.bench_with_input(
            ...
            |b, &size| {
                b.iter(|| {
                    ...
                    for i in 0..100 {
                        let offset = (i * size) % (10 * 1024 * 1024 - size);
                        black_box(file.append_message_offset_length(&data, 0, size));
                    }
...
fn bench_flush_comparison(c: &mut Criterion) {
    ...
    group.bench_function("io_uring_flush", |b| {
        b.to_async(&rt).iter(|| async {
            ...
            // Flush
            black_box(file.flush_async().await.unwrap());
                ...
                b.to_async(&rt).iter(|| async move {
                    let file = Arc::new(
                        IoUringMappedFile::new(
                            CheetahString::from(format!("/tmp/bench_concurrent_io_uring_{}", num_threads)),
                            50 * 1024 * 1024, // 50MB
                        )
                        .await
                        .unwrap(),
                    ...
                    let _ = std::fs::remove_file(format!("/tmp/bench_concurrent_io_uring_{}", num_threads));
                });
            },
        );
    }
...
fn bench_sequential_throughput(c: &mut Criterion) {
    ...
    group.bench_function("io_uring_sequential", |b| {
        b.to_async(&rt).iter(|| async {
            ...
            for i in 0..10 {
                black_box(file.write_async(&data, i * data.len()).await.unwrap());
            }
    ...
    group.bench_function("default_sequential", |b| {
        b.iter(|| {
            ...
            let data = vec![0x55; 1024 * 1024]; // 1MB chunks
            for i in 0..10 {
                black_box(file.append_message_offset_length(&data, 0, data.len()));
            }
    ...
    group.bench_function("io_uring_read", |b| {
        b.iter(|| {
            ...
            for i in 0..100 {
                ...
                black_box(file.get_bytes(offset, 4096));
            }
        });
    });
