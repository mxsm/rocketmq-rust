...
fn run_lifecycle_probe() -> StoreKvCompactionLifecycleProbe {
    let runtime = tokio::runtime::Builder::new_multi_thread()
        ...
        .thread_name("rocketmq-store-kv-compaction-bench")
        .enable_all()
        .build()
        .expect("store KV compaction benchmark runtime should start");

    runtime.block_on(run_store_kv_compaction_lifecycle_probe())
}
...
fn write_store_kv_compaction_report_artifact() {
    ...
    let payload = serde_json::json!({
        "case": "store_kv_compaction_lifecycle",
        "generated_at_unix_ms": generated_at_unix_ms,
        "probe": output,
    });
    let path = output_dir.join("store-kv-compaction-lifecycle-report.json");
    fs::write(
        path,
        serde_json::to_vec_pretty(&payload).expect("store KV compaction benchmark artifact should serialize"),
    )
    .expect("store KV compaction benchmark artifact should be written");
}

fn bench_store_kv_compaction_lifecycle(criterion: &mut Criterion) {
    write_store_kv_compaction_report_artifact();

    criterion.bench_function("store_kv_compaction_lifecycle/no_overlap_shutdown", |bencher| {
        bencher.iter(|| {
            let output = run_lifecycle_probe();
            assert!(output.healthy, "{output:?}");
...
criterion_group! {
    ...
    targets = bench_store_kv_compaction_lifecycle
}
