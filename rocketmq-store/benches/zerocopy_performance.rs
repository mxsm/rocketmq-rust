...
fn create_test_message_with_encoding(body_size: usize) -> MessageExtBrokerInner {
    ...
    message.with_tags(CheetahString::from_static_str("TAG1"));
    
    let mut msg_inner = MessageExtBrokerInner::default_with_message(message);
    msg_inner.set_queue_id(0);
    msg_inner.set_born_timestamp(
            ...
            |b, &size| {
                ...
                b.iter(|| {
                    let result = callback.do_append(
                        black_box(0),
                        black_box(mapped_file.as_ref()),
                        black_box(1024 * 1024),
                        black_box(&mut message.clone()),
                        black_box(&context),
                    );
                    black_box(result);
                });
