...
async fn test_pagecache_warming_applied() {
    ...
    let mapped_file = result_rx.await
    ...
    assert!(elapsed.as_millis() < 10, "First write took {:?}, expected <10ms with PageCache warming", elapsed);
}
...
async fn test_pagecache_warming_reduces_page_faults() {
    ...
    println!("Improvement: {:.1}%", (1.0 - elapsed_warm.as_secs_f64() / elapsed_cold.as_secs_f64()) * 100.0);
...
async fn test_pagecache_warming_large_files() {
    ...
    println!("Large file (128MB) allocation + warming: {:?}", allocation_time);
    
    // Allocation + warming should complete in reasonable time
    assert!(allocation_time.as_secs() < 5, "Allocation took too long: {:?}", allocation_time);
...
async fn test_zero_copy_performance_improvement() {
    ...
    let mapped_file_std = DefaultMappedFile::new(
        CheetahString::from_string(file_path_std),
        10 * 1024 * 1024,
    );
    ...
    let mapped_file_zc = DefaultMappedFile::new(
        CheetahString::from_string(file_path_zc),
        10 * 1024 * 1024,
    );
    ...
    println!("Improvement: {:.1}%", (1.0 - elapsed_zc.as_secs_f64() / elapsed_std.as_secs_f64()) * 100.0);
...
async fn test_pagecache_warming_error_handling() {
    ...
    assert!(result.is_ok(), "Allocation should succeed even if madvise fails");
