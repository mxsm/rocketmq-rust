...
use rocketmq_store::log_file::mapped_file::factory::{MappedFileConfig, MappedFileFactory, MappedFileType};
...
async fn test_io_uring_mapped_file_creation() {
    ...
    assert!(result.is_ok(), "Failed to create IoUringMappedFile: {:?}", result);
...
async fn test_io_uring_async_flush() {
    ...
    mapped_file.write_async(&data, 0).await.expect("Write failed");
...
async fn test_io_uring_performance_advantage() {
    ...
    for i in 0..num_writes {
        ...
        uring_mapped.write_async(&data, offset).await.expect("Write failed");
    }
    let io_uring_duration = start.elapsed();
    
    println!("io_uring: {} writes in {:?}", num_writes, io_uring_duration);
    println!("io_uring throughput: {:.2} MB/s", 
...
async fn test_io_uring_large_file() {
    ...
    let positions = [0, file_size as usize / 4, file_size as usize / 2, file_size as usize - 8192];
