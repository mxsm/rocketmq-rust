...
impl TopicConfigManager {
    ...
    fn init(&mut self) {
        //SELF_TEST_TOPIC
        {
            self.topic_config_table.lock().insert(
                ...
                TopicConfig::with_queues(TopicValidator::RMQ_SYS_SELF_TEST_TOPIC, 1, 1),
            );
        }
        ...
        }
        {
            self.topic_config_table.lock().insert(
                ...
                TopicConfig::with_queues(TopicValidator::RMQ_SYS_BENCHMARK_TOPIC, 1024, 1024),
            );
        }
        {
            let topic = self
...

        {
            self.topic_config_table.lock().insert(
                ...
                TopicConfig::with_queues(TopicValidator::RMQ_SYS_OFFSET_MOVED_EVENT, 1, 1),
            );
        }

        {
            self.topic_config_table.lock().insert(
                ...
                TopicConfig::with_queues(TopicValidator::RMQ_SYS_SCHEDULE_TOPIC, 18, 18),
            );
        }

        {
            if self.broker_config.trace_topic_enable {
                ...
                self.topic_config_table
                    .lock()
                    .insert(topic.clone(), TopicConfig::with_queues(topic, 1, 1));
            }
        }
...

        {
            ...
            self.topic_config_table.lock().insert(
                topic.clone(),
                TopicConfig::with_perm(topic, 1, 1, PermName::PERM_INHERIT as u32),
            );
        }

        {
            self.topic_config_table.lock().insert(
                ...
                TopicConfig::with_queues(TopicValidator::RMQ_SYS_TRANS_HALF_TOPIC, 1, 1),
            );
        }

        {
            self.topic_config_table.lock().insert(
                ...
                TopicConfig::with_queues(TopicValidator::RMQ_SYS_TRANS_OP_HALF_TOPIC, 1, 1),
            );
        }
    }
    ...
    pub(crate) fn put_topic_config(&self, topic_config: TopicConfig) -> Option<TopicConfig> {
        self.topic_config_table
            .lock()
            .insert(topic_config.topic_name().cloned().unwrap(), topic_config)
    }
            ...
            let topic_config = if default_topic_config.is_some() {
                    ...
                    && self.broker_config.auto_create_topic_enable
                {
                    default_topic_config
                        .as_mut()
                        .unwrap()
                        .set_perm(PermName::PERM_READ | PermName::PERM_WRITE);
                }
                ...
                if PermName::is_inherited(default_topic_config.as_ref().unwrap().perm()) {
                    ...
                    ref_topic_config.set_write_queue_nums(queue_nums as u32);
                    ref_topic_config.set_read_queue_nums(queue_nums as u32);
                    let mut perm = default_topic_config.as_ref().unwrap().perm();
                    perm &= !(PermName::PERM_INHERIT as u32);
                    ref_topic_config.set_perm(perm);
                    ref_topic_config.set_topic_sys_flag(topic_sys_flag);
                    ref_topic_config.set_topic_filter_type(
    ...
    pub fn create_topic_in_send_message_back_method(
        ...
        let topic_config_result = if lock.is_some() {
            ...
            topic_config = Some(TopicConfig::new(topic));
            if let Some(ref mut config) = topic_config {
                config.set_read_queue_nums(client_default_topic_queue_nums as u32);
                config.set_write_queue_nums(client_default_topic_queue_nums as u32);
                config.set_perm(perm);
                config.set_topic_sys_flag(topic_sys_flag);
                config.set_order(is_order);
    ...
    pub fn update_topic_config(&mut self, topic_config: &mut TopicConfig) {
        ...
        let current_attributes = self.current(topic_config.topic_name().unwrap().as_str());
        let create = self
            .topic_config_table
            .lock()
            .get(topic_config.topic_name().unwrap().as_str())
            .is_none();
        let final_attributes =
            alter_current_attributes(create, ALL.clone(), new_attributes, current_attributes);
        topic_config.set_attributes(final_attributes);
        ...
        self.persist_with_topic(
            topic_config.topic_name().unwrap().as_str(),
            topic_config.clone(),
        );
    }
