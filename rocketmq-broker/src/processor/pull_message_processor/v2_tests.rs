...
impl RequestProcessorV2 for ArcHeldPullProcessor {
...
impl RequestProcessorV2 for TraitPullProcessor {
    async fn process(&mut self, request: &mut RemotingRequest) -> rocketmq_error::RocketMQResult<HandlerOutcome> {
        let mut processor = self.inner.lock().await;
        RequestProcessorV2::process(&mut *processor, request).await
    }
}
...
impl RunningServer {
    async fn finish(mut self) {
        ...
        let report = self.result.await.expect("Pull V2 server shutdown report");
    ...
    P: RequestProcessorV2 + Clone + Sync + 'static,
{
    ...
    let runner_context = owner.root_context().component("pull-message-v2.runner");
    ...
    runner_context
        .spawn_service("pull-message-v2-server", async move {
            let report = server
                ...
                .await
                .expect("Pull V2 server report");
            let _ = result_tx.send(report);
        })
        .expect("start Pull V2 server");
    let address = startup_rx
        .await
        .expect("Pull V2 startup result")
        .expect("Pull V2 startup address");
    let connection = Connection::new(TcpStream::connect(address).await.expect("connect Pull V2 client"));
    (
        connection,
        RunningServer {
            owner,
...
fn available_ha_port() -> usize {
    std::net::TcpListener::bind((std::net::Ipv4Addr::LOCALHOST, 0))
        .expect("reserve Pull V2 HA port")
        .local_addr()
        .expect("Pull V2 HA address")
        .port() as usize
}

async fn runtime(label: &str, broadcast: bool) -> (BrokerRuntime, PathBuf) {
    let root = temp_root(label);
    std::fs::create_dir_all(&root).expect("create Pull V2 test root");
    ...
    runtime.initialize().await.expect("initialize Pull V2 Broker runtime");
    runtime.seed_pop_topic_and_group_for_test("topic-a", "group-a");
    runtime
        .start_message_store_for_test()
        .await
        .expect("start Pull V2 message store");
    (runtime, root)
}
...
async fn send_and_receive(connection: &mut Connection, request: RemotingCommand) -> RemotingCommand {
    connection.send_command(request).await.expect("send Pull V2 request");
    connection
        .receive_command()
        .await
        .expect("read Pull V2 connection")
        .expect("Pull V2 inline response")
}
...
async fn wait_for_deferred(service: &PullDeferredService) {
    ...
    .await
    .expect("Pull V2 request should seal a deferred registration");
}

#[tokio::test]
async fn v2_immediate_reply_is_bound_to_the_original_wire_opaque() {
...
#[tokio::test]
async fn v2_arc_held_suspension_requires_injected_service_and_retains_one_sealed_lease() {
    ...
    deferred_client
        ...
        .await
        .expect("send deferred Pull V2 request");
...
fn stored_message() -> MessageExtBrokerInner {
    ...
    message.set_body(Bytes::from_static(b"v2-broadcast-pull"));
...
async fn v2_broadcast_offset_uses_the_canonical_network_session_identity() {
    ...
    for _ in 0..2 {
        let result = context
            ...
            .await
            .expect("append V2 broadcast message");
    ...
    assert_eq!(response.opaque(), OPAQUE);
    let observed_session = *sessions.lock().first().expect("observed Pull V2 session");
    assert_eq!(
