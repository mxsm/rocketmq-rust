...
impl<MS> RequestProcessorV2 for NotificationProcessor<MS>
where
...
mod tests {
    ...
    impl RequestProcessorV2 for ArcHeldNotificationProcessor {
        async fn process(&mut self, request: &mut RemotingRequest) -> rocketmq_error::RocketMQResult<HandlerOutcome> {
    ...
    #[test]
    fn notification_v2_normalizes_missing_and_zero_born_time_but_preserves_positive_value() {
    ...
    #[tokio::test]
    async fn notification_v2_arc_leaf_fails_closed_without_service_then_seals_zero_born_time_wait() {
        ...
        unavailable_client
            ...
            .await
            .expect("send Notification V2 request");
        let unavailable = unavailable_client
            .receive_command()
            .await
            .expect("read Notification V2 connection")
        ...
        assert!(processor
            ...
            .is_ok());
        let (mut deferred_client, deferred_server) = start_v2_leaf_server(
            "notification-v2-deferred",
        ...
        deferred_client
            ...
            .await
            .expect("send deferred Notification V2 request");
