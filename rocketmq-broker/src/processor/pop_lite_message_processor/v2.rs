...
impl<MS> RequestProcessorV2 for PopLiteMessageProcessor<MS>
where
...
mod tests {
    ...
    impl RequestProcessorV2 for ArcHeldPopLiteProcessor {
        async fn process(&mut self, request: &mut RemotingRequest) -> rocketmq_error::RocketMQResult<HandlerOutcome> {
    ...
    #[tokio::test]
    async fn pop_lite_v2_one_way_preflight_suppresses_frame_without_consuming_pending_event() {
        ...
        let (mut client, server) = start_v2_leaf_server(
            "pop-lite-v2-one-way",
