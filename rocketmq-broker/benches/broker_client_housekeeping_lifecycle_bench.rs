...
fn run_lifecycle_probe() -> BrokerClientHousekeepingLifecycleProbe {
    let runtime = tokio::runtime::Builder::new_multi_thread()
        ...
        .thread_name("rocketmq-broker-client-housekeeping-bench")
        .enable_all()
        .build()
...
fn write_broker_client_housekeeping_report_artifact() {
    ...
    let payload = serde_json::json!({
        "case": "broker_client_housekeeping_lifecycle",
        "generated_at_unix_ms": generated_at_unix_ms,
        "probe": output,
    });
    let path = output_dir.join("broker-client-housekeeping-lifecycle-report.json");
    fs::write(
        path,
        serde_json::to_vec_pretty(&payload).expect("broker client housekeeping benchmark artifact should serialize"),
    )
    .expect("broker client housekeeping benchmark artifact should be written");
}

fn bench_broker_client_housekeeping_lifecycle(criterion: &mut Criterion) {
    write_broker_client_housekeeping_report_artifact();

    criterion.bench_function("broker_client_housekeeping_lifecycle/no_overlap_shutdown", |bencher| {
        bencher.iter(|| {
            let output = run_lifecycle_probe();
            assert!(output.healthy, "{output:?}");
...
criterion_group! {
    ...
    targets = bench_broker_client_housekeeping_lifecycle
}
