...
fn run_lifecycle_probe() -> BrokerTopicQueueMappingCleanLifecycleProbe {
    let runtime = tokio::runtime::Builder::new_multi_thread()
        ...
        .thread_name("rocketmq-broker-topic-clean-bench")
        .enable_all()
        .build()
...
fn write_broker_topic_queue_mapping_clean_report_artifact() {
    ...
    let payload = serde_json::json!({
        "case": "broker_topic_queue_mapping_clean_lifecycle",
        "generated_at_unix_ms": generated_at_unix_ms,
        "probe": output,
    });
    let path = output_dir.join("broker-topic-queue-mapping-clean-lifecycle-report.json");
    fs::write(
        path,
        serde_json::to_vec_pretty(&payload)
            .expect("broker topic queue mapping clean benchmark artifact should serialize"),
    )
    .expect("broker topic queue mapping clean benchmark artifact should be written");
}

fn bench_broker_topic_queue_mapping_clean_lifecycle(criterion: &mut Criterion) {
    write_broker_topic_queue_mapping_clean_report_artifact();

    criterion.bench_function(
        "broker_topic_queue_mapping_clean_lifecycle/fixed_delay_shutdown",
        |bencher| {
            bencher.iter(|| {
                let output = run_lifecycle_probe();
                assert!(output.healthy, "{output:?}");
...
criterion_group! {
    ...
    targets = bench_broker_topic_queue_mapping_clean_lifecycle
}
