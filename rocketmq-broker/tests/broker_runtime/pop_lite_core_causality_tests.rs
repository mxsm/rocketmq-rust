...
#[tokio::test]
async fn pop_lite_resume_core_real_store_causality_advances_offset_once() {
    let mut runtime = new_lite_test_runtime("pop-lite-core-consume").await;
    ...
    let batch = runtime
        ...
        .expect("seam should reserve the dispatched event")
        .commit();
    ...
    let mut body = result.body.expect("shared core should return the stored message");
    let message =
        MessageDecoder::decode(&mut body, true, false, false, false, false).expect("decode shared core message");
...
#[tokio::test]
async fn pop_lite_real_store_preserves_attempt_order_and_requeues_until_drained() {
    let mut runtime = new_lite_test_runtime("pop-lite-attempt-requeue").await;
    seed_lite_query_state(&mut runtime);
    seed_lmq_message(&mut runtime, "child-a", b"lite-first").await;
    seed_lmq_message(&mut runtime, "child-a", b"lite-second").await;
    let lmq_name = CheetahString::from_string(to_lmq_name("parent-topic", "child-a").expect("child-a lmq"));
    let client_id = CheetahString::from_static_str("client-1");
    let group = CheetahString::from_static_str("group-a");
    ...
    let first = processor
        .execute_pop_lite_batch(
            ...
            dispatcher
                .reserve_pending_events(&client_id)
                .expect("reserve first event")
                .commit(),
        )
        .await;
    let mut first_body = first.body.expect("first attempt reads the first message");
    let first_message = MessageDecoder::decode(&mut first_body, true, false, false, false, false)
        .expect("decode first ordered message");
    assert_eq!(first_message.body(), Some(Bytes::from_static(b"lite-first")));
    assert_eq!(
        runtime
            .composition
            .state
    ...
    let blocked = processor
        .execute_pop_lite_batch(
            ...
            dispatcher
                .reserve_pending_events(&client_id)
                .expect("reserve event for blocked attempt")
                .commit(),
        )
        .await;
    ...
    let second = processor
        .execute_pop_lite_batch(
            ...
            dispatcher
                .reserve_pending_events(&client_id)
                .expect("reserve event for matching attempt")
                .commit(),
        )
        .await;
    let mut second_body = second.body.expect("matching attempt reads the second message");
    let second_message = MessageDecoder::decode(&mut second_body, true, false, false, false, false)
        .expect("decode second ordered message");
    assert_eq!(second_message.body(), Some(Bytes::from_static(b"lite-second")));
    assert_eq!(
        runtime
            .composition
            .state
...
#[tokio::test]
async fn pop_lite_real_store_applies_reset_and_consumes_empty_event() {
    let mut runtime = new_lite_test_runtime("pop-lite-offset-correction").await;
    seed_lite_query_state(&mut runtime);
    seed_lmq_message(&mut runtime, "child-a", b"lite-reset-first").await;
    seed_lmq_message(&mut runtime, "child-a", b"lite-reset-second").await;
    let lmq_name = CheetahString::from_string(to_lmq_name("parent-topic", "child-a").expect("child-a lmq"));
    ...
    runtime.composition.state.consumer_offset_manager().commit_offset(
        CheetahString::from_static_str("PopLiteResetConcurrentAdvance"),
        &group,
        &lmq_name,
        0,
        99,
    ...
    let reset = processor
        .execute_pop_lite_batch(
            ...
            dispatcher
                .reserve_pending_events(&client_id)
                .expect("reserve reset-offset event")
                .commit(),
        )
        .await;
    let mut reset_body = reset.body.expect("reset offset reads the second stored message");
    let reset_message =
        MessageDecoder::decode(&mut reset_body, true, false, false, false, false).expect("decode reset-offset message");
    assert_eq!(reset_message.body(), Some(Bytes::from_static(b"lite-reset-second")));
    assert_eq!(
        runtime
            .composition
            .state
    ...
    let empty = processor
        .execute_pop_lite_batch(
            ...
            dispatcher
                .reserve_pending_events(&client_id)
                .expect("reserve empty-store event")
                .commit(),
        )
        .await;
...
#[tokio::test]
async fn pop_lite_real_store_corrects_bad_offset_before_reread() {
    let mut runtime = new_lite_test_runtime("pop-lite-bad-offset-correction").await;
    seed_lite_query_state(&mut runtime);
    seed_lmq_message(&mut runtime, "child-a", b"lite-first").await;
    seed_lmq_message(&mut runtime, "child-a", b"lite-second").await;
    let lmq_name = CheetahString::from_string(to_lmq_name("parent-topic", "child-a").expect("child-a lmq"));
    ...
    let dispatcher = runtime.composition.state.lite_event_dispatcher().clone();
    runtime.composition.state.consumer_offset_manager().commit_offset(
        CheetahString::from_static_str("PopLiteBadOffsetTest"),
        &group,
        &lmq_name,
        0,
        99,
    ...
    let corrected = processor
        .execute_pop_lite_batch(
            ...
            dispatcher
                .reserve_pending_events(&client_id)
                .expect("reserve bad-offset event")
                .commit(),
        )
        .await;
...
#[tokio::test]
async fn pop_lite_real_store_initializes_missing_offset_from_retained_minimum() {
    let mut runtime = new_lite_test_runtime("pop-lite-missing-offset-minimum").await;
    seed_lite_query_state(&mut runtime);
    seed_lmq_offsets(&mut runtime, &[("child-a", 5)]);
    seed_lmq_message(&mut runtime, "child-a", b"lite-retained-minimum").await;
    let lmq_name = CheetahString::from_string(to_lmq_name("parent-topic", "child-a").expect("child-a lmq"));
    let client_id = CheetahString::from_static_str("client-1");
    let group = CheetahString::from_static_str("group-a");
    let direct = runtime
    ...
    let message =
        MessageDecoder::decode(&mut body, true, false, false, false, false).expect("decode retained-minimum message");
    assert_eq!(message.body(), Some(Bytes::from_static(b"lite-retained-minimum")));
    assert_eq!(
        runtime
            .composition
            .state
...
#[tokio::test]
async fn pop_lite_unavailable_store_consumes_event_without_offset_commit() {
    let mut runtime = new_lite_test_runtime("pop-lite-store-unavailable").await;
    seed_lite_query_state(&mut runtime);
    let lmq_name = CheetahString::from_string(to_lmq_name("parent-topic", "child-a").expect("child-a lmq"));
    let client_id = CheetahString::from_static_str("client-1");
    let group = CheetahString::from_static_str("group-a");
    ...
    let unavailable = processor
        .execute_pop_lite_batch(
            ...
            dispatcher
                .reserve_pending_events(&client_id)
                .expect("reserve unavailable-store event")
                .commit(),
        )
        .await;
