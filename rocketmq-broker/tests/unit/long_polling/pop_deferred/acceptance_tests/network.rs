...
#[path = "network/lifecycle_tests.rs"]
mod lifecycle_tests;
#[path = "network/provenance_tests.rs"]
...
impl RunningServer {
    ...
    async fn finish(mut self) {
        self.begin_shutdown();
        let report = self.result.await.expect("owned server result channel");
        assert_clean_shutdown("server", &report);
        let task_report = self.owner.shutdown_tasks().await;
        assert_clean_shutdown("runtime", &task_report);
        let final_report = self.owner.shutdown_background();
        assert_clean_shutdown("runtime finalization", &final_report);
    }
}
    ...
    P: RequestProcessor + Clone + Sync + 'static,
{
    ...
    runner_context
        .spawn_service("pop-deferred-server", async move {
            let result = server
                ...
                .await
                .expect("owned POP server shutdown report");
            let _ = result_tx.send(result);
        })
        .expect("spawn owned POP server");
    let address = startup_rx
        .await
        .expect("POP startup channel")
        .expect("POP server startup");
    let client = Connection::new(TcpStream::connect(address).await.expect("connect POP client"));
    (
        client,
        address,
        RunningServer {
...
async fn prepared_arrival_and_timeout_reexecute_then_write_one_bound_frame() {
    ...
    for (original_opaque, wake_reason) in [
            ...
            .spawn_service("pop-deferred-resume", async move {
                let result = service_for_resume
                    .resume_claimed_observed(
                        claim,
                        rocketmq_transport::api::DeferredResumeRetainedSize::new(257),
                        observer,
        ...
        let response = client
            .receive_command()
            .await
            .expect("POP connection remains open until the accepted write drains")
