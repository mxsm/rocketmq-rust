...
mod lifecycle_tests;
#[path = "acceptance_tests/p1_tests.rs"]
mod p1_tests;
#[cfg(windows)]
#[path = "acceptance_tests/session_close_tests.rs"]
...
impl RunningServer {
    async fn finish(mut self) {
        ...
        let report = self.result.await.expect("owned Pull server report");
        assert_clean_shutdown("Pull server", &report);
        let tasks = self.owner.shutdown_tasks().await;
        assert_clean_shutdown("Pull runtime", &tasks);
        let background = self.owner.shutdown_background();
        assert_clean_shutdown("Pull finalization", &background);
    }
}
    ...
    P: RequestProcessor + Clone + Sync + 'static,
{
    ...
    runner_context
        .spawn_service("pull-deferred-server", async move {
            let report = server
                ...
                .await
                .expect("owned Pull server shutdown report");
            let _ = result_tx.send(report);
        })
        .expect("spawn owned Pull server");
    let address = startup_rx
        .await
        .expect("Pull startup channel")
        .expect("Pull server startup");
    (
        address,
        RunningServer {
            owner,
    ...
    P: RequestProcessor + Clone + Sync + 'static,
{
    ...
    let client = Connection::new(TcpStream::connect(address).await.expect("connect Pull client"));
        ...
        running
            ...
            .spawn_service("pull-deferred-resume", async move {
                let result = service_for_resume
                    .resume_claimed(
                        claim,
                        rocketmq_transport::api::DeferredResumeRetainedSize::new(321),
...
async fn tcp_partial_write_drops_owner_once_without_retrying() {
    ...
    let connector = TcpSocket::new_v4().expect("create raw Pull socket");
    ...
    let raw_client = connector.connect(address).await.expect("connect raw Pull client");
