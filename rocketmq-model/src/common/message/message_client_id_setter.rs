...
use crate::codec::write_int;
use crate::codec::write_short;
...
static LEN: LazyLock<usize> = LazyLock::new(|| {
    let ip = get_ip().unwrap_or_else(create_fake_ip);
    ip.len() + 2 + 4 + 4 + 2
});
...
mod tests {
    ...
    fn create_fake_ip_generates_valid_ip() {
        ...
        assert!(fake_ip.iter().all(|&_byte| true));
    }
