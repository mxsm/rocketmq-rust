...
| 增加产品页面 | OpenAPI type + API client + desktop page | `rocketmq-sre/ui` |
...
```powershell
node .\rocketmq-sre\scripts\generate_phase5_openapi.mjs
npm --prefix .\rocketmq-sre\ui run generate:api
...
```powershell
cargo fmt --manifest-path .\rocketmq-sre\Cargo.toml --all -- --check
cargo clippy --manifest-path .\rocketmq-sre\Cargo.toml `
  --locked --workspace --all-targets --all-features -- -D warnings
cargo test --manifest-path .\rocketmq-sre\Cargo.toml `
  --locked --workspace --all-features

npm --prefix .\rocketmq-sre\ui run lint
npm --prefix .\rocketmq-sre\ui test -- --run
