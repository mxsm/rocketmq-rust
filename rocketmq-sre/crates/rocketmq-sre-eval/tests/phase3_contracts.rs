...
fn committed_phase_three_schemas_match_public_contracts() {
    ...
    assert!(
        generated
            .iter()
            .any(|(name, _)| *name == "execution-sli-query.schema.json")
    );
    assert!(
        generated
            .iter()
            .any(|(name, _)| *name == "execution-sli-observation.schema.json")
    );
    assert!(
        generated
            .iter()
            .any(|(name, _)| *name == "release-workflow.schema.json")
    );
    assert!(
        generated
            .iter()
            .any(|(name, _)| *name == "integration-delivery.schema.json")
    );
