...
fn validate_plan_request(request: &CreateDrPlanRequest) -> Result<(), ControlPlaneError> {
...
fn validate_checkpoint(
    request: &RecordRecoveryCheckpointRequest,
    definition: &rocketmq_sre_contracts::RecoveryCheckpointDefinition,
) -> Result<(), ControlPlaneError> {
...
fn action_item_transition_allowed(current: ActionItemStatus, next: ActionItemStatus) -> bool {
    current == next
        || matches!(
            (current, next),
            (
...
fn require_cluster(auth: &AuthContext, cluster_id: Option<ClusterId>) -> Result<(), ControlPlaneError> {
...
fn bound_evidence(evidence_ids: &[rocketmq_sre_contracts::EvidenceId]) -> Result<(), ControlPlaneError> {
