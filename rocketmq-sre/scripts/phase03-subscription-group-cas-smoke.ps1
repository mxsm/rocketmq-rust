...
Assert-PortAvailable ($NameServerPort + 1000)
Assert-PortAvailable ($NameServerPort + 1001)
...
$runRoot = [IO.Path]::GetFullPath(
    (Join-Path $TemporaryRoot "phase03-subscription-group-cas-$([Guid]::NewGuid().ToString('N'))")
)
...
brokerName = "sre-phase03-subscription-group-cas-broker"
brokerClusterName = "SrePhase03SubscriptionGroupCas"
brokerId = 0
...
$environmentNames = @(
    ...
    'ROCKETMQ_SRE_TEST_CONSUMER_GROUP',
    'RUST_LOG'
)
$savedEnvironment = @{}
...
try {
    ...
    ) 'RocketMQ Subscription Group CAS test-cluster build'
    ...
    if ($targetFreeGiB -lt 15 -or $targetFreePercent -lt 12) {
        ...
        throw (
            'The Subscription Group CAS build left insufficient Broker runtime reserve; ' +
    ...
    $adminBinary = Join-Path $CargoTargetDir 'debug/rocketmq-admin-cli.exe'
    foreach ($binary in @($namesrvBinary, $brokerBinary, $adminBinary)) {
        if (-not (Test-Path -LiteralPath $binary -PathType Leaf)) {
            throw "Expected Subscription Group CAS smoke binary is missing: $binary"
        }
    }
    ...
    $env:ROCKETMQ_SRE_TEST_CONSUMER_GROUP = $group
    Invoke-Native cargo @(
        '+1.95.0', 'test',
        '--manifest-path', (Join-Path $sreRoot 'Cargo.toml'),
        '--locked',
        '--package', 'rocketmq-sre-execution-agent',
        'real_subscription_group_version_cas_rejects_stale_write_and_rolls_back',
        '--',
        '--ignored',
        '--test-threads=1'
    ) 'real Subscription Group version-CAS smoke'
    $smokeSucceeded = $true
    Write-Host (
        "PHASE03_SUBSCRIPTION_GROUP_CAS_SMOKE_OK group=$group " +
        'version_advanced=true stale_rejected=true rollback_advanced=true permissions_unchanged=true'
    )
}
finally {
