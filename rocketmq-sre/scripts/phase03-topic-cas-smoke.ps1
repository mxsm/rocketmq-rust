...
brokerName = "sre-phase03-topic-cas-broker"
brokerClusterName = "SrePhase03TopicCas"
brokerId = 0
...
try {
    ...
    foreach ($binary in @($namesrvBinary, $brokerBinary, $adminBinary)) {
        if (-not (Test-Path -LiteralPath $binary -PathType Leaf)) {
            throw "Expected Topic CAS smoke binary is missing: $binary"
        }
    }
    ...
    $topic = "SRE_TOPIC_CAS_$([Guid]::NewGuid().ToString('N'))"
    Invoke-Native $adminBinary @(
        ...
        '-w', '4',
        '-p', '6'
    ) 'dedicated Topic bootstrap'

    $env:ROCKETMQ_SRE_TEST_DATABASE_URL = $DatabaseUrl
    $env:ROCKETMQ_SRE_TEST_NAMESRV_ADDR = "127.0.0.1:$NameServerPort"
    $env:ROCKETMQ_SRE_TEST_TOPIC = $topic
    Invoke-Native cargo @(
        '+1.95.0', 'test',
        '--manifest-path', (Join-Path $sreRoot 'Cargo.toml'),
        '--locked',
        '--package', 'rocketmq-sre-execution-agent',
        'real_topic_version_cas_rejects_stale_write_and_rolls_back',
        '--',
        '--ignored',
        '--test-threads=1'
    ) 'real Topic version-CAS smoke'
    $smokeSucceeded = $true
    Write-Host "PHASE03_TOPIC_CAS_SMOKE_OK topic=$topic version_advanced=true stale_rejected=true rollback_advanced=true"
}
finally {
