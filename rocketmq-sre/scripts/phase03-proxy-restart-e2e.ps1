...
foreach ($path in @($CargoHome, $CargoTargetDir, $TempDir)) {
    ...
    if ($resolved.StartsWith('C:\', [System.StringComparison]::OrdinalIgnoreCase)) {
        throw "Phase 03 build state must not use the C drive: $resolved"
    }
}

$env:KUBECONFIG = [System.IO.Path]::GetFullPath($Kubeconfig)
...
$proxyPods = kubectl -n rocketmq-system get pods -l app.kubernetes.io/name=rocketmq-proxy -o json |
    ConvertFrom-Json
if ($LASTEXITCODE -ne 0) {
    throw 'Unable to list the Kind Proxy pods'
}
$targetPod = $proxyPods.items |
...
$env:ROCKETMQ_SRE_PHASE3_PROXY_POD = $targetPod.metadata.name
$env:ROCKETMQ_SRE_PHASE3_PROXY_UID = $targetPod.metadata.uid
...
& cargo +1.95.0 test `
    ...
    --nocapture
if ($LASTEXITCODE -ne 0) {
    throw 'The formal Phase 03 Proxy restart E2E test failed'
}
