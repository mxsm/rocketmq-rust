...
try {
    ...
    Invoke-Native cargo @(
        ...
        '--lib',
        'proxy_restart_qualifies_through_shadow_supervised_and_autonomous',
        '--',
        '--ignored',
        '--test-threads=1'
    ) 'Proxy restart qualification lifecycle'
    ...
    Invoke-Native cargo @(
        ...
        '--test', 'execution_flow',
        'proxy_restart_autonomous_execution_uses_dynamic_safety_and_succeeds',
        '--',
        '--ignored',
        '--exact',
        '--test-threads=1'
    ) 'Proxy restart autonomous Executor flow'

    & (Join-Path $scriptDirectory 'phase03-proxy-restart-e2e.ps1') `
    ...
    if ($LASTEXITCODE -ne 0) {
        throw 'Unable to clear the prior bounded Proxy restart probe Job.'
    }
    Invoke-Native kubectl @(
        'apply',
        '-f', $probeManifest
    ) 'post-restart bounded message probe'
    Invoke-Native kubectl @(
        '-n', 'rocketmq-system',
        'wait',
    ...
    ) 'post-restart bounded message probe completion'
    $probeOutput = & kubectl -n rocketmq-system logs job/rocketmq-sre-phase03-proxy-restart-probe `
        -c bounded-probe
    if ($LASTEXITCODE -ne 0) {
        throw 'Unable to read the post-restart bounded message probe result.'
    }
    ...
    if (
        ...
        throw 'The post-restart probe did not prove an exact 10/10/10 message path.'
    }

    Write-Host (
        'PHASE04_PROXY_RESTART_AUTONOMY_SMOKE_OK ' +
