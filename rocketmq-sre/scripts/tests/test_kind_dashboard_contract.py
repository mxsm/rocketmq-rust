...
class KindDashboardContractTest(unittest.TestCase):
    def test_kind_runner_builds_loads_and_waits_for_dashboard(self) -> None:
        runner = (REPOSITORY_ROOT / "rocketmq-sre/scripts/kind.ps1").read_text(encoding="utf-8")
        ...
        supervisor = (REPOSITORY_ROOT / "rocketmq-sre/scripts/kind-port-forward.ps1").read_text(
            encoding="utf-8"
        )
    ...
    def test_kind_manifest_exposes_authenticated_dashboard_proxy(self) -> None:
        kustomization = (REPOSITORY_ROOT / "rocketmq-sre/deploy/kind/kustomization.yaml").read_text(
            encoding="utf-8"
        )
        manifest = (REPOSITORY_ROOT / "rocketmq-sre/deploy/kind/dashboard-stack.yaml").read_text(
            encoding="utf-8"
        )
    ...
    def test_kind_smoke_checks_dashboard_cluster_data(self) -> None:
        smoke = (REPOSITORY_ROOT / "rocketmq-sre/deploy/kind/smoke-job.yaml").read_text(encoding="utf-8")
