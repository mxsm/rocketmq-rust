...
async fn read_admin_startup_failure_retains_its_typed_source() {
    ...
    let runtime = RuntimeContext::from_current("topic-admin-startup-source");
    let pool = PgPoolOptions::new()
        .connect_lazy("postgres://unused:unused@127.0.0.1/unused")
        .expect("syntactically valid lazy PostgreSQL URL");
    let namesrv_addr = (0..65)
        .map(|port| format!("127.0.0.1:{}", 11_000 + port))
        .collect::<Vec<_>>()
        .join(";");

    let error = match ProductionTopicConfigPatchClient::start(
...
fn missing_live_topic_field_fails_closed_before_any_write() {
    ...
    assert_eq!(
        select_before_values(TopicConfigPatch::default(), &requested)
            .expect_err("missing topic configuration must fail the read")
            .stable_code(),
        "source_unavailable"
    );
}
