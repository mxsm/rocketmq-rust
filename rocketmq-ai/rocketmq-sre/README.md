...
Run Rust commands from `rocketmq-ai/rocketmq-sre/`:
...
Run the qualification from `rocketmq-ai/rocketmq-sre/`; build output stays on `F:` and the
...
`rocketmq-ai/rocketmq-sre/`. The complete run requires a clean committed revision and
...
Run from `rocketmq-ai/rocketmq-sre/` after bringing up the documented Kind environment:
...
Run from `rocketmq-ai/rocketmq-sre/`; build output stays on `D:` and `F:`, and the redacted
...
Run from `rocketmq-ai/rocketmq-sre/`; the harness creates and destroys its own cluster,
