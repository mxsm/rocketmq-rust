...
RUN --mount=type=cache,target=/usr/local/cargo/registry,sharing=locked \
    --mount=type=cache,target=/workspace/rocketmq-ai/rocketmq-sre/target,sharing=locked \
    cargo +1.95.0 build \
...
COPY --from=builder \
    /workspace/rocketmq-ai/rocketmq-sre/target/release/phase01-shadow-eval \
    /usr/local/bin/phase01-shadow-eval
COPY rocketmq-ai/rocketmq-sre/tests/fixtures /opt/rocketmq-sre/tests/fixtures
