...
function Resolve-RevisionManifest(
    [string]$SourceRoot,
    [string[]]$CandidatePaths,
    [string]$ProjectName
) {
    foreach ($candidatePath in $CandidatePaths) {
        $manifest = Join-Path $SourceRoot $candidatePath
        if (Test-Path -LiteralPath $manifest -PathType Leaf) {
            return $manifest
        }
    }
    throw "$ProjectName manifest was not found in revision root: $SourceRoot"
}
...
function Build-Revision(
    ...
    $sreManifest = Resolve-RevisionManifest $SourceRoot @(
        'rocketmq-ai\rocketmq-sre\Cargo.toml',
        'rocketmq-sre\Cargo.toml'
    ) 'RocketMQ SRE'
    $mcpManifest = Resolve-RevisionManifest $SourceRoot @(
