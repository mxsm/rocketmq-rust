...
def validate_manifest(manifest: dict[str, Any], sre_root: Path = SRE_ROOT) -> list[str]:
    ...
    repository_root = sre_root.parents[1]
    mcp_app = (repository_root / "rocketmq-ai" / "rocketmq-mcp" / "src" / "app.rs").read_text(
        encoding="utf-8"
    )
    mcp_server = (
        repository_root / "rocketmq-ai" / "rocketmq-mcp" / "src" / "protocol" / "server.rs"
    ).read_text(encoding="utf-8")
