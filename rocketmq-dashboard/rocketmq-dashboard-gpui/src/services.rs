...
use rocketmq_dashboard_common::{
    AdminSessionStatus, AdminSessionSummary, ConnectionScope, CredentialSourceKind, EndpointHealth, TransportSettings,
    add_endpoint, normalize_nameserver_address, normalize_proxy_address, remove_endpoint, switch_endpoint,
};
...
use crate::{
    infrastructure::{
        ...
        history_store::{HistoryLifecycle, HistoryStore},
...
impl AppServices {
    ...
    pub fn desktop(
        ...
        Self::desktop_inner(
            store,
            Arc::new(RealConnectionProvider(provider)),
            auth,
            runtime_context,
            history_context,
            monitor_context,
...
impl DesktopBackend {
    async fn reconcile_foundations(
        ...
        if foundations.history_enabled {
            let needs_start = self.history_lifecycle.lock().is_none();
            if needs_start {
                self.history_store.list().await.map_err(config_ui_error)?;
                let lifecycle = HistoryLifecycle::start(&self.history_context, true).map_err(config_ui_error)?;
            ...
            {
                return Err(UiError::new(
                    "The History lifecycle did not stop before its deadline.",
                    UiErrorCode::Configuration,
                    true,
                ));
            }
        ...
        *provider.switch_gate.lock() = Some(gate);
        ...
        runtime.block_on(async {
            ...
            let mut operation = Box::pin(
                services.mutate_with_progress(ConfigMutation::AddNameServer("localhost:9876".into()), progress),
            );
    ...
    fn unreachable_connection_projects_connecting_then_failed_with_real_health_semantics() {
        ...
        *provider.switch_gate.lock() = Some(gate);
        ...
        runtime.block_on(async {
            ...
            let (progress, mut updates) = tokio::sync::mpsc::unbounded_channel();
            let mut operation = Box::pin(
                services.mutate_with_progress(ConfigMutation::AddNameServer("unreachable:9876".into()), progress),
            );
