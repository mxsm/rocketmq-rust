...
Login retains the existing local-account and required-password-change behavior. Labels are associated with their inputs, and the sign-in button names its pending state. The login view restores the earlier blue-black control-console scene, cyan accents and illuminated sign-in card using the existing RocketMQ image. Its presentation is scoped to `auth.css` and `auth-scene.css`; authenticated pages keep their shared theme tokens. Decorative panels describe capabilities instead of claiming measured throughput or health before sign-in. Narrow windows put the form first, and reduced motion keeps a static scene. No animation is required to operate sign-in or restore a session.
