...
fn assert_shared_contract(canonical: Error, expected_cause: Option<&str>) {
    ...
    let wrapped = std::sync::Arc::clone(&shared);

    assert_eq!(wrapped.descriptor(), shared.descriptor());
    assert_eq!(wrapped.context(), shared.context());
    assert_eq!(wrapped.code(), shared.code());
    ...
    assert!(std::sync::Arc::ptr_eq(&shared, &wrapped));
}
...
fn multiple_carriers_reuse_the_original_canonical_snapshot() {
    ...
    let first = std::sync::Arc::clone(&shared);
    let second = std::sync::Arc::clone(&shared);
