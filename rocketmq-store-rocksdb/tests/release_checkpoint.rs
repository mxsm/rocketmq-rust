...
async fn release_checkpoint_restore_flushes_hashes_and_reopens_rocksdb_read_only() {
    ...
    let capacity_request = StoreReleaseCheckpointRequest {
        checkpoint_id: "rocksdb-a-generation-8".to_string(),
        checkpoint_set_id: "set-generation-8".to_string(),
        generation: 8,
        barrier_id: "barrier-78".to_string(),
        ..request.clone()
    };
    ...
    let capacity_service = RocksDbReleaseCheckpointService::new(
        store,
        RocksDbRuntimeScope::new(runtime.service_context("checkpoint-capacity")),
        checkpoint_root,
        capacity_request.storage_identity.clone(),
        1,
    );
