...
async fn example_dynamic_dispatch() {
...
async fn example_custom_strategy() {
    ...
    match strategy.authenticate(&context1).await {
    ...
    match strategy.authenticate(&context2).await {
    ...
    match strategy.authenticate(&context3).await {
...
async fn example_strategy_composition() {
    ...
    for strategy in &strategies {
        if strategy.authenticate(&context).await.is_ok() {
            authenticated = true;
...
async fn main() {
    ...
    example_allow_all_strategy().await;
    example_dynamic_dispatch().await;
    example_custom_strategy().await;
    example_strategy_composition().await;
