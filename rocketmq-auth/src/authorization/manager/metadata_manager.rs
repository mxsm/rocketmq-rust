...
mod tests {
    ...
    /// Helper to create a test manager with local provider
    fn create_test_manager() -> AuthorizationMetadataManager {
        // Create a test config
        let config = AuthConfig {
            auth_config_path: std::env::temp_dir()
                .join("test_auth_metadata_manager")
                .to_str()
                .unwrap()
                .to_string()
                .into(),
        ...
        // Create local provider and initialize it
        let mut provider = LocalAuthorizationMetadataProvider::new();
        provider.initialize(config, None).unwrap();

        // Create manager
        AuthorizationMetadataManager::with_providers(Arc::new(provider), None)
    }

    /// Helper to build a test ACL
    fn build_test_acl(subject: &str, resource_name: &str, _action: &str) -> Acl {
        let resource = Resource::of_topic(resource_name);
        let actions = vec![Action::Pub];
        let environment = Environment::of("192.168.0.1");

        let entry = PolicyEntry::of(resource, actions, environment, Decision::Allow);
        let policy = Policy::of_entries(PolicyType::Custom, vec![entry]);

        Acl::of(subject, SubjectType::User, policy)
    }
    ...
    async fn test_create_and_get_acl() {
        ...
        // Create ACL
        let acl = build_test_acl("user:test1", "Topic:test-topic", "PUB");
        manager.create_acl(acl.clone()).await.unwrap();

        // Get ACL
        let subject_wrapper = UserSubjectWrapper {
            key: "user:test1".to_string(),
            subject_type: SubjectType::User,
        };
        let retrieved = manager.get_acl(&subject_wrapper).await.unwrap();

        assert!(retrieved.is_some());
        let retrieved_acl = retrieved.unwrap();
        assert_eq!(retrieved_acl.subject_key(), "user:test1");
        assert_eq!(retrieved_acl.policies().len(), 1);
    }

    #[tokio::test]
    async fn test_update_acl() {
        let manager = create_test_manager();

        // Create initial ACL
        let acl1 = build_test_acl("user:test2", "Topic:topic1", "PUB");
        manager.create_acl(acl1).await.unwrap();

        // Update with new policy
        let acl2 = build_test_acl("user:test2", "Topic:topic2", "SUB");
        manager.update_acl(acl2).await.unwrap();

        // Verify updated ACL has both policies
        let subject_wrapper = UserSubjectWrapper {
            key: "user:test2".to_string(),
            subject_type: SubjectType::User,
        };
        let retrieved = manager.get_acl(&subject_wrapper).await.unwrap().unwrap();

        // After merge, should have entries for both resources
        assert!(!retrieved.policies().is_empty());
    }
    ...
    async fn test_delete_acl() {
        ...
        // Create ACL
        let acl = build_test_acl("user:test3", "Topic:test-topic", "PUB");
        manager.create_acl(acl).await.unwrap();

        // Delete ACL
        let subject_wrapper = UserSubjectWrapper {
            key: "user:test3".to_string(),
            subject_type: SubjectType::User,
        };
        manager.delete_acl(&subject_wrapper).await.unwrap();

        // Verify deleted
        let result = manager.get_acl(&subject_wrapper).await.unwrap();
        assert!(result.is_none());
    }

    #[tokio::test]
    async fn test_list_acl() {
        let manager = create_test_manager();

        // Create multiple ACLs
        let acl1 = build_test_acl("user:alice", "Topic:topic1", "PUB");
        let acl2 = build_test_acl("user:bob", "Topic:topic2", "SUB");
        manager.create_acl(acl1).await.unwrap();
        manager.create_acl(acl2).await.unwrap();

        // List all ACLs
        let all_acls = manager.list_acl(None, None).await.unwrap();
        assert!(all_acls.len() >= 2);

        // Filter by subject
        let filtered = manager.list_acl(Some("alice"), None).await.unwrap();
        assert!(!filtered.is_empty());
    }

    #[tokio::test]
    async fn test_validate_acl_empty_policies() {
        let manager = create_test_manager();

        // Create ACL with no policies
        let acl = Acl::of_with_policies("user:test", SubjectType::User, vec![]);

        // Should fail validation
        let result = manager.create_acl(acl).await;
        assert!(result.is_err());
        match result {
            Err(AuthorizationError::InvalidContext(msg)) => {
    ...
    #[tokio::test]
    async fn test_validate_policy_empty_entries() {
        let manager = create_test_manager();

        // Create policy with no entries
        let policy = Policy::of_entries(PolicyType::Custom, vec![]);
