...
impl LocalAuthenticationMetadataProvider {
    ...
    pub fn with_config(config: &AuthConfig) -> AuthServiceResult<Self> {
    ...
    where
        F: FnOnce(&mut HashMap<String, User>) -> AuthServiceResult<()>,
    {
...
async fn read_users_snapshot(path: &Path) -> AuthServiceResult<HashMap<String, User>> {
...
fn read_users_snapshot_blocking(path: &Path) -> AuthServiceResult<HashMap<String, User>> {
...
fn decode_users_snapshot(_path: &Path, bytes: &[u8]) -> AuthServiceResult<HashMap<String, User>> {
...
async fn write_users_snapshot(path: &Path, content: Vec<u8>) -> AuthServiceResult<()> {
...
mod tests {
    ...
    async fn create_user_rejects_duplicate_without_overwriting_existing_secret() {
        ...
        let error = provider
            ...
            .await
            .unwrap_err();
        assert_eq!(error.operation(), AuthOperation::ManageMetadata);
        assert_eq!(error.kind(), AuthFailureKind::Conflict);
