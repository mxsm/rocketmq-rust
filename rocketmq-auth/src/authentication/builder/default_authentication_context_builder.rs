...
mod tests {
    ...
    #[test]
    fn test_build_from_remoting_with_credentials() {
        use rocketmq_remoting::protocol::command_custom_header::CommandCustomHeader;
        ...
        let builder = DefaultAuthenticationContextBuilder::new();
        let mut request = RemotingCommand::create_request_command(1, EmptyHeader);

        // Add authentication fields
        request.add_ext_field(
            CheetahString::from(ACCESS_KEY),
            CheetahString::from("testuser"),
        );
        request.add_ext_field(
            CheetahString::from(SIGNATURE_FIELD),
            CheetahString::from("test-signature"),
        );
        request.add_ext_field(
            CheetahString::from("someField"),
            CheetahString::from("someValue"),
        );
        ...
        assert_eq!(result.username(), Some(&CheetahString::from("testuser")));
        assert_eq!(
            result.signature(),
            Some(&CheetahString::from("test-signature"))
        );
