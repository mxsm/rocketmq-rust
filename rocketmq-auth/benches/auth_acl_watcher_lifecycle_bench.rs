...
fn run_lifecycle_probe() -> AuthAclWatcherLifecycleProbe {
    let runtime = tokio::runtime::Builder::new_multi_thread()
        ...
        .thread_name("rocketmq-auth-acl-watcher-bench")
        .enable_all()
        .build()
...
fn write_auth_acl_watcher_report_artifact() {
    ...
    let payload = serde_json::json!({
        "case": "auth_acl_watcher_lifecycle",
        "generated_at_unix_ms": generated_at_unix_ms,
        "probe": output,
    });
    let path = output_dir.join("auth-acl-watcher-lifecycle-report.json");
    fs::write(
        path,
        serde_json::to_vec_pretty(&payload).expect("auth ACL watcher benchmark artifact should serialize"),
    )
    .expect("auth ACL watcher benchmark artifact should be written");
}

fn bench_auth_acl_watcher_lifecycle(criterion: &mut Criterion) {
    write_auth_acl_watcher_report_artifact();

    criterion.bench_function("auth_acl_watcher_lifecycle/reload_shutdown", |bencher| {
        bencher.iter(|| {
            let output = run_lifecycle_probe();
            assert!(output.healthy, "{output:?}");
...
criterion_group! {
    ...
    targets = bench_auth_acl_watcher_lifecycle
}
