...
pub use rocketmq_proxy_core::status::*;
use rocketmq_proxy_core::ProxyError;
use tonic::{Code as TonicCode, Status as TonicStatus};

pub trait ProxyStatusMapperExt {
    fn to_tonic_status(error: &ProxyError) -> TonicStatus;
    fn grpc_status_to_tonic_code(status: GrpcStatusCode) -> TonicCode;
}

impl ProxyStatusMapperExt for ProxyStatusMapper {
    fn to_tonic_status(error: &ProxyError) -> TonicStatus {
...
mod tests {
    ...
    use super::{ProxyStatusMapper, ProxyStatusMapperExt};
