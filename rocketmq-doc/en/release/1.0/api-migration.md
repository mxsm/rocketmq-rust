...
### API-0.9-010: Runtime outcome, contract, and plan APIs

Issue #10013 replaces the pre-1.0 mixed runtime error channel with explicit
contracts, normal outcomes, and catalog-backed operational failures. It is an
...
classification. The error is redaction-safe and maps to the canonical Filter
descriptor when it crosses an operational boundary.
...
`Filter::compile`, the local string `FilterError`, and the legacy adapter were
removed in the direct canonical cutover. Custom `Filter` implementations must
implement the typed method:
