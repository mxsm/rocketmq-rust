# Remoting V2 performance baseline

## Scope and interpretation

This report records the FIN-03 engineering evidence for the V2 processor and
...
plus the FIN working-tree changes. Measurements in this report were collected
...
Compile all three new groups without running them:
...
|---|---|---|
| `transport_processor_dispatch` | legacy-shaped materialized contract reference; canonical embedded V2 inline dispatch with 0 B, 128 B, and 4 KiB requests | absolute channel-free dispatch latency and throughput; materialization reference is not a full V1 dispatcher |
...
adding response binding, complete-frame encoding, or body access to the V2
public API.
...
| `transport_processor_dispatch/canonical_v2_embedded_inline` | 0 B | `[2.7989, 2.8500, 2.8958] us` | `[345.33, 350.87, 357.29] Kelem/s` |
...
| `transport_processor_dispatch/canonical_v2_embedded_inline` | 128 B | `[2.9934, 3.0584, 3.0990] us` | `[322.68, 326.97, 334.07] Kelem/s` |
...
| `transport_processor_dispatch/canonical_v2_embedded_inline` | 4 KiB | `[2.9568, 3.0075, 3.0661] us` | `[326.15, 332.50, 338.20] Kelem/s` |
...
| V2 request-local legacy `ChannelInner`, pending owner, and `Arc<Mutex<Connection>>` are zero | `public_reply_is_channel_free_zero_copy_bound_and_observed_once_after_one_admitted_clone` exercises the public embedded V2 boundary and verifies one admitted processor clone and body pointer preservation. V2 public types expose no channel or connection snapshot. | Meets for embedded V2. Network V2 reuses its canonical session writer and does not create a processor-local legacy channel snapshot. |
...
| Deferred wait/execution permits and retained bytes return to zero | Typed V2 server close tests assert writer queued items/bytes and `DeferredAdmissionSnapshot::retained_bytes()` are zero. Deferred registry acceptance tests assert `invariant_failures() == 0`. | Meets. |
...
## V1/V2 comparison conclusion

The compatibility processor layer is deliberately removed in FIN-05. The
same-revision `legacy_materialized_contract_reference` therefore measures only
the old materialized command shape; it is not presented as a complete V1
dispatcher latency baseline. The response-plan and Pull/Pop groups do provide
valid same-revision legacy-materialized versus canonical ownership/encode
comparisons.

Because no complete V1 dispatcher exists in the FIN revision, and Criterion
does not produce request P99 values, this quick run cannot approve the “no more
than 2% throughput/P99 regression” budget for the full dispatch path. A release
owner must either:

1. collect a cross-revision Criterion baseline from the immediate pre-FIN
   commit on the same host and add a histogram-capable end-to-end load run for
   P99; or
2. explicitly approve the missing historical comparison while relying on the
   exact allocation/encode/copy/writer/lifecycle evidence above.

This is an evidence limitation, not a claim that the budget passed. No runtime
regression or failure should be inferred from the absence of a comparable V1
sample.
