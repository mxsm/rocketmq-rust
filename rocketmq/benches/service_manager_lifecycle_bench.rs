...
use rocketmq_rust::task::run_service_manager_lifecycle_probe;
use rocketmq_rust::task::ServiceManagerLifecycleProbe;

fn run_lifecycle_probe() -> ServiceManagerLifecycleProbe {
    let runtime = tokio::runtime::Builder::new_multi_thread()
        .worker_threads(2)
        .max_blocking_threads(4)
        .thread_name("rocketmq-service-manager-bench")
        .enable_all()
        .build()
        .expect("service manager benchmark runtime should start");

    runtime.block_on(run_service_manager_lifecycle_probe())
}
...
fn write_service_manager_report_artifact() {
    ...
    let payload = serde_json::json!({
        "case": "rocketmq_service_manager_lifecycle",
        "generated_at_unix_ms": generated_at_unix_ms,
        "probe": output,
    });
    let path = output_dir.join("rocketmq-service-manager-lifecycle-report.json");
    fs::write(
        path,
        serde_json::to_vec_pretty(&payload).expect("service manager artifact should serialize"),
    )
    .expect("service manager artifact should be written");
}
...
criterion_group! {
    ...
    targets = bench_service_manager_lifecycle
}
