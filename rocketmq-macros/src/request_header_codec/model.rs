...
impl HeaderModel {
    ...
    pub(super) fn parse(input: DeriveInput) -> syn::Result<Self> {
        ...
        model.ok_or_else(|| syn::Error::new(span, "RequestHeaderCodec could not construct a header model"))
    }
    ...
    pub(super) fn parse_partial(input: DeriveInput) -> (Option<Self>, Option<syn::Error>) {
        ...
        let type_id = attrs.type_id.unwrap_or_else(|| {
            combine_error(
                ...
                syn::Error::new(
                    input.ident.span(),
                    "RequestHeaderCodec requires header(type_id = \"...\")",
                ),
            );
        ...
        let named = match input.data {
            Data::Struct(data) => match data.fields {
                ...
                Fields::Unnamed(fields) => {
                    combine_error(
                        &mut errors,
                        syn::Error::new(fields.span(), "RequestHeaderCodec requires a named struct"),
                    );
                    None
                }
                Fields::Unit => {
                    combine_error(
                        &mut errors,
                        syn::Error::new(input.ident.span(), "RequestHeaderCodec does not support unit structs"),
                    );
                    None
                }
            },
            Data::Enum(data) => {
                combine_error(
                    &mut errors,
                    syn::Error::new(data.enum_token.span(), "RequestHeaderCodec does not support enums"),
                );
                None
            }
            Data::Union(data) => {
                combine_error(
                    &mut errors,
                    syn::Error::new(data.union_token.span(), "RequestHeaderCodec does not support unions"),
                );
                None
            }
        };
        ...
        if let Some(named) = named {
            for (source_order, field) in named.into_iter().enumerate() {
                let Ok(source_order) = u16::try_from(source_order) else {
                    combine_error(
                        &mut errors,
                        syn::Error::new(field.span(), "RequestHeaderCodec supports at most 65536 fields"),
                    );
                    continue;
                };
...
impl FieldModel {
    fn parse(field: syn::Field, generic_types: &HashSet<String>, source_order: u16) -> syn::Result<Self> {
        ...
        let ident = field
            .ident
            .clone()
            .ok_or_else(|| syn::Error::new(span, "RequestHeaderCodec requires named fields"))?;
