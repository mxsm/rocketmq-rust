...
fn expand(input: proc_macro::TokenStream) -> syn::Result<proc_macro2::TokenStream> {
    ...
    let Some(model) = model else {
        return Err(errors
            .unwrap_or_else(|| syn::Error::new(input_span, "RequestHeaderCodec could not construct a header model")));
    };
    ...
    let warnings = model.fields.iter().filter(|field| field.legacy_required).map(|field| {
        ...
        quote_spanned! {span=>
            const _: () = {
                ...
                const __REQUEST_HEADER_CODEC_LEGACY_REQUIRED: () = ();
                let _ = __REQUEST_HEADER_CODEC_LEGACY_REQUIRED;
            };
        }
    });
    quote!(#(#warnings)*)
...
mod tests {
    ...
    #[test]
    fn serde_metadata_does_not_change_wire_metadata() {
        let input = syn::parse_quote! {
