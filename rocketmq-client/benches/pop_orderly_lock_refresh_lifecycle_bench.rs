...
use rocketmq_client_rust::test_support::PopOrderlyLockRefreshLifecycleProbe;
...
fn write_pop_orderly_lock_refresh_report_artifact() {
    ...
    let payload = serde_json::json!({
        "case": "client_pop_orderly_lock_refresh_lifecycle",
        "generated_at_unix_ms": generated_at_unix_ms,
        "probe": output,
    });
    let path = output_dir.join("client-pop-orderly-lock-refresh-lifecycle-report.json");
    fs::write(
        path,
        serde_json::to_vec_pretty(&payload).expect("pop orderly lock refresh artifact should serialize"),
    )
    .expect("pop orderly lock refresh artifact should be written");
}

fn bench_pop_orderly_lock_refresh_lifecycle(criterion: &mut Criterion) {
    write_pop_orderly_lock_refresh_report_artifact();

    criterion.bench_function(
        "client_pop_orderly_lock_refresh_lifecycle/fixed_delay_self_stop_shutdown",
        |bencher| {
            bencher.iter(|| {
                let output = run_lifecycle_probe();
                assert!(output.healthy, "{output:?}");
                ...
                black_box(output.task_count_after_self_stop);
...
criterion_group! {
    ...
    targets = bench_pop_orderly_lock_refresh_lifecycle
}
