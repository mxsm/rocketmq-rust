...
fn write_consumer_stats_manager_report_artifact() {
    ...
    let payload = serde_json::json!({
        "case": "client_consumer_stats_manager_lifecycle",
        "generated_at_unix_ms": generated_at_unix_ms,
        "probe": output,
    });
    let path = output_dir.join("client-consumer-stats-manager-lifecycle-report.json");
    fs::write(
        path,
        serde_json::to_vec_pretty(&payload).expect("consumer stats manager artifact should serialize"),
    )
    .expect("consumer stats manager artifact should be written");
}

fn bench_consumer_stats_manager_lifecycle(criterion: &mut Criterion) {
    write_consumer_stats_manager_report_artifact();

    criterion.bench_function(
        "client_consumer_stats_manager_lifecycle/task_group_shutdown",
        |bencher| {
            bencher.iter(|| {
                let output = run_lifecycle_probe();
                assert!(output.healthy, "{output:?}");
...
criterion_group! {
    ...
    targets = bench_consumer_stats_manager_lifecycle
}
