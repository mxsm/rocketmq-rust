...
fn write_trace_worker_report_artifact() {
    ...
    let payload = serde_json::json!({
        "case": "client_trace_worker_lifecycle",
        "generated_at_unix_ms": generated_at_unix_ms,
        "probe": output,
    });
    let path = output_dir.join("client-trace-worker-lifecycle-report.json");
    fs::write(
        path,
        serde_json::to_vec_pretty(&payload).expect("trace worker benchmark artifact should serialize"),
    )
    .expect("trace worker benchmark artifact should be written");
}

fn bench_trace_worker_lifecycle(criterion: &mut Criterion) {
    write_trace_worker_report_artifact();

    criterion.bench_function("client_trace_worker_lifecycle/task_group_shutdown", |bencher| {
        bencher.iter(|| {
            let output = run_lifecycle_probe();
            assert!(output.healthy, "{output:?}");
            black_box(output.task_count_before_shutdown);
            black_box(output.remaining_tasks_after_shutdown);
            black_box(output.cancelled);
...
criterion_group! {
    ...
    targets = bench_trace_worker_lifecycle
}
