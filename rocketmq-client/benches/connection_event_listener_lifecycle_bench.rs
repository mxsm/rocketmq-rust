...
fn run_lifecycle_probe() -> ConnectionEventListenerLifecycleProbe {
    ...
    let output = runtime.block_on(run_connection_event_listener_lifecycle_probe(runtime.client_runtime()));
    runtime.shutdown();
    output
}

fn run_heartbeat_probe() -> HeartbeatRouteIndexProbe {
    let runtime = support::BenchClientRuntime::new("heartbeat-route-index");
    let output = run_heartbeat_route_index_probe(runtime.client_runtime(), 1_000, 256, 512);
    runtime.shutdown();
    output
}
...
fn write_connection_event_listener_report_artifact() {
    ...
    fs::write(
        path,
        serde_json::to_vec_pretty(&payload).expect("connection event listener artifact should serialize"),
    )
    .expect("connection event listener artifact should be written");
}

fn bench_connection_event_listener_lifecycle(criterion: &mut Criterion) {
    write_connection_event_listener_report_artifact();

    criterion.bench_function(
        "client_connection_event_listener_lifecycle/task_group_shutdown",
        |bencher| {
            bencher.iter(|| {
                let output = run_lifecycle_probe();
                assert!(output.healthy, "{output:?}");
...
criterion_group! {
    ...
    targets = bench_connection_event_listener_lifecycle
}
