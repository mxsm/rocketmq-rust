...
fn write_produce_accumulator_guard_report_artifact() {
    ...
    let payload = serde_json::json!({
        "case": "client_produce_accumulator_guard_lifecycle",
        "generated_at_unix_ms": generated_at_unix_ms,
        "probe": output,
    });
    let path = output_dir.join("client-produce-accumulator-guard-lifecycle-report.json");
    fs::write(
        path,
        serde_json::to_vec_pretty(&payload).expect("produce accumulator guard artifact should serialize"),
    )
    .expect("produce accumulator guard artifact should be written");
}

fn bench_produce_accumulator_guard_lifecycle(criterion: &mut Criterion) {
    write_produce_accumulator_guard_report_artifact();

    criterion.bench_function(
        "client_produce_accumulator_guard_lifecycle/task_group_shutdown",
        |bencher| {
            bencher.iter(|| {
                let output = run_lifecycle_probe();
                assert!(output.healthy, "{output:?}");
...
criterion_group! {
    ...
    targets = bench_produce_accumulator_guard_lifecycle
}
