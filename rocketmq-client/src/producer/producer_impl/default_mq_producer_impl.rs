 ...
 impl DefaultMQProducerImpl {
     ...
     async fn send_kernel_impl(
         ...
         let mut send_message_context = if self.has_send_message_hook() {
             ...
             self.execute_send_message_hook_before(Some(&send_message_context));
             Some(send_message_context)
         } else {
             None
         };
             ...
             .starts_with(mix_all::RETRY_GROUP_TOPIC_PREFIX)
         {
             ...
             if let Some(value) = reconsume_times {
                 request_header.reconsume_times =
                     value.parse::<i32>().map_or(Some(0), |value| Some(value));
             ...
             if let Some(value) = max_reconsume_times {
                 request_header.max_reconsume_times =
                     value.parse::<i32>().map_or(Some(0), |value| Some(value));
                 MessageAccessor::clear_property(msg, MessageConst::PROPERTY_MAX_RECONSUME_TIMES);
             }
         }
         ...
         let send_result = match communication_mode {
             CommunicationMode::Async => {
                 unimplemented!("Async send message not implemented");
             }
         ...
         if self.has_send_message_hook() {
             send_message_context.as_mut().unwrap().send_result = Some(&send_result);
             self.execute_send_message_hook_after(send_message_context.as_ref());
         }
         Ok(send_result)
     }
