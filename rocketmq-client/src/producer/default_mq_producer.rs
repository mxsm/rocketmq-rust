...
impl MQProducer for DefaultMQProducer {
    async fn start(&mut self) -> rocketmq_error::RocketMQResult<()> {
        ...
        if self.client_config.enable_trace {
            ...
            default_mqproducer_impl.register_send_message_hook(Box::new(SendMessageTraceHookImpl::new(dispatcher.clone())));
            default_mqproducer_impl.register_end_transaction_hook(Arc::new(Box::new(EndTransactionTraceHookImpl::new(dispatcher))))
        }
