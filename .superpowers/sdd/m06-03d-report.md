...
3. Mutation-resistant ownership contract
   ...
   - GREEN: the final target exits 0 with 33/33 tests. It proves one canonical owner, the exact three storage
     ...
     comments, strings, aliased field types, and misplaced `libc` dependencies.
...
- The governed ArcMut baseline remains 1,232 identities and 3,377 occurrences. Five changed fingerprints are
  explicit one-for-one relocations with no new occurrence:
...
- `python scripts/tests/test_m06_store_local_contract.py` - exit 0; 33/33.
