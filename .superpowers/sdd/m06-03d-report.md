...
3. Mutation-resistant ownership contract
   ...
   - GREEN: the final target exits 0 with 41/41 tests. It proves one canonical owner, the exact three storage
     ...
     generic/default type aliases, and misplaced `libc` dependencies without misclassifying `AtomicU64`.
...
- Intermediate GREEN: the 39/39 contract resolved the reviewed direct, import-as, and recursive non-generic alias
  cases. The second review replaced this partial alias parsing with the exact syntax policy below.
- Both mapped-file creation sites now carry immediate, operation-specific `// SAFETY:` comments covering the
  completed pre-map `set_len`, DefaultMappedFile's own no-resize/no-truncate behavior, `ArcMut`/clone-owned mapping
  lifetime independent of file-handle rename/reopen/close, legacy caller obligation, and the lazy lock's limited
  role of serializing initialization. The `MappedFile::get_file` Rustdoc records that this safe compatibility
  accessor cannot enforce the size invariant; repository uses remain metadata and `try_clone`. Every public API
  in the Local file module has production Rustdoc, including applicable `# Errors` and `# Panics` contracts and
  rename/reopen failure ordering.
...
- Second-review RED: the contract ran 41 tests with two expected failures because a brace import containing
  `File as SegmentHandle` and a generic/default type alias could bypass the intermediate resolver.
- Final GREEN: the 41/41 contract applies an exact allowlist to every active alias/brace `use` and every active
  type alias in `default_mapped_file_impl.rs`. It permits only the existing `mlock`/`munlock` aliases, the
  normalized Windows `VirtualQuery` brace use, and `type Target = [u8]`; all other syntax is rejected without
  claiming to parse arbitrary Rust use trees or generic aliases. Resolved-field checks remain for fully-qualified
  paths, direct non-aliased imports, and exact plain `u64`; the AtomicU64 positive fixture includes the complete
  allowlisted syntax.
...
- `python scripts/tests/test_m06_store_local_contract.py` - exit 0; 41/41.
...
- `$env:RUSTDOCFLAGS='-D warnings'; cargo doc -p rocketmq-store-local -p rocketmq-store --no-deps --all-features`
  - exit 1 only on four pre-existing Store Rustdoc findings outside this slice: unquoted `Vec<u8>` in
  `message_arriving_listener.rs`, two unquoted `Arc<DefaultMappedFile>` occurrences in `mapped_file_queue.rs`, and
  an unmatched `</pre>` in `single_consume_queue.rs`. The changed `MappedFile::get_file` documentation emitted no
  finding.
- `cargo doc -p rocketmq-store --no-deps --all-features` - exit 0 with those same four existing warnings; the
  updated compatibility contract rendered successfully.
