...
envelope. Transport now owns bounded frame processing, adaptive buffers, TCP/TLS primitives, pending request
correlation, count-plus-byte admission, security projections, and owner-scoped client/server lifecycles.
...
- Wire commands remain `rocketmq-protocol` types; no schema, request code, response mapping, or body logging was
  introduced.
- ArcMut promotion is monotonic: 1,266 identities / 3,430 occurrences became 1,233 / 3,381. The 14 exact
...
- `cargo test -p rocketmq-remoting` (112 unit tests plus integration and doc tests); focused TLS ownership test was
  rerun after the final lifecycle adapter correction
...
- ArcMut guard, 24 fixtures, and 63 unit tests
