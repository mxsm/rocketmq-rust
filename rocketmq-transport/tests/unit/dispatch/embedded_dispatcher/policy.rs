...
async fn supplied_principal_and_peerless_origin_are_authoritative_over_command_headers() {
    let fixture = EmbeddedFixture::new("embedded-security-origin");
    ...
    assert!(matches!(
        dispatcher
            .dispatch_embedded(&fixture.task_group, Principal::new("trusted-broker"), None, command)
            .await
...
async fn embedded_security_denial_is_peerless_exactly_once_and_development_exemption_fails_closed() {
    let fixture = EmbeddedFixture::new("embedded-security-deny");
    let (processor, state, _) = TestProcessor::new(Behavior::Reply);
    ...
    let (processor, state, _) = TestProcessor::new(Behavior::Reply);
    let development = AuthorizedCommandDispatcher::new(
        processor,
        Vec::new(),
...
#[tokio::test]
async fn embedded_never_invokes_legacy_address_hooks() {
    let fixture = EmbeddedFixture::new("embedded-no-address-hook");
    let (processor, state, _) = TestProcessor::new(Behavior::Reply);
