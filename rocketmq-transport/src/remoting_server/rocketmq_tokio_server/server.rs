...
impl<P> TransportServer<P>
where
    P: RequestProcessor + Clone + Sync + 'static,
{
    /// Creates a server that will move `request_processor` into its dispatcher at startup.
    #[must_use]
    ...
    /// Creates a server with an explicit lifecycle owner.
    #[must_use]
    ...
    /// Creates a server bound to one transport recorder.
    #[must_use]
    ...
    /// Creates a server using an already-authorized dispatcher.
    #[must_use]
    ...
    /// Publishes session lifecycle into a composition-owned registry.
    #[must_use]
    pub fn with_session_registry(mut self, registry: Arc<crate::session_registry::SessionRegistry>) -> Self {
    ...
    #[must_use]
    pub fn with_authorized_dispatcher(mut self, dispatcher: Arc<AuthorizedCommandDispatcher<P>>) -> Self {
        ...
        S: Future,
    {
        ...
        serve(listener, shutdown, Some(broadcast::channel(100).0), prepared).await
    }
    ...
    async fn prepare(mut self) -> Result<PreparedServer<P>, TransportError> {
        ...
        if self.dispatcher.is_none() && self.request_processor.is_none() {
            return Err(configuration_message(
                "request_processor",
                "automatic  startup has no processor source",
            ));
        }
        ...
        let security = match (self.dispatcher.as_ref(), self.transport_security.take()) {
            ...
            (None, None) => {
                warn!(" remoting server has no security profile; using development-insecure fallback");
        ...
        let dispatcher = match self.dispatcher.take() {
            ...
            None => {
                ...
                Arc::new(
                    AuthorizedCommandDispatcher::try_new_with_telemetry_and_budget(
                        processor,
                        Vec::new(),
                        security,
                        admission,
    ...
    P: RequestProcessor + Clone + Sync + 'static,
{
    ...
    let route = Arc::new(ConnectionHandler {
    ...
    tokio::select! {
        result = transport.run_authorized(route) => {
            if let Err(error) = result {
                error!(cause = %error, " remoting listener stopped with an error");
            }
        }
        _ = shutdown => {
            info!("Shutting down  remoting server");
        }
    }
