...
#[tokio::test]
async fn deferred_permit_is_independent_from_real_session_execution_and_v1_snapshots() {
    ...
    let v1_before = controller.snapshot();
    let size = retained(5);
    ...
    assert_eq!(controller.snapshot(), v1_before);
    ...
    assert_eq!(after_first, v1_before);
...
fn dispatcher_is_fail_closed_until_explicit_configuration_and_then_shares_owner() {
    ...
    impl crate::runtime::processor_v2::RequestProcessorV2 for Processor {
        async fn process(
            &mut self,
    ...
    let controller = Arc::new(controller(&process));
    let dispatcher = crate::dispatch::AuthorizedCommandDispatcherV2::new(
        Processor,
        Vec::new(),
