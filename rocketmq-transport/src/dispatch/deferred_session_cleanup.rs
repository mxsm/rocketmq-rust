...
impl DeferredSessionCleanupOwner {
    ...
    #[cfg(test)]
    pub(crate) fn set_close_acquired_checkpoint(&self, checkpoint: TestCloseCheckpoint) {
        *self.registration.coordinator.close_acquired_checkpoint.lock() = Some(checkpoint);
    }
...
mod tests {
    ...
    fn panicking_first_target_does_not_skip_later_registry_cleanup() {
        ...
        let (sink, _receiver) = crate::dispatch::ResponseSink::local();
        let responder = sink
