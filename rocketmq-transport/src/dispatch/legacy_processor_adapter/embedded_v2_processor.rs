...
//! Channel-free embedded execution for the explicit V2 processor wrapper.
    ...
    P: RequestProcessorV2 + Clone + Sync + 'static,
{
    ...
    pub(crate) async fn process_embedded(
        ...
        match processed {
            Ok(Ok(outcome)) => Ok(InternalProcessorCandidate::success(InternalProcessorOutcome::V2(
                outcome,
            ))),
